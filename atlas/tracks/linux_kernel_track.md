...
5. 沿[kref阅读地图](../../knowledge/linux/object_lifetime/kref/大纲.md#第1章_kref_引用计数机制章节大纲)建立“谁保地址、谁持份额、谁关业务”的模型。先读P01～P05的责任与接口，再读P06～P10的退出、交付、查找、锁和RCU；P11区分私有对象与框架对象，P12用[诊断练习](../../knowledge/linux/object_lifetime/kref/P12_典型错误模式与调试线索.md#12.10.2_最小审查流程)检验推理。随后从[P13工程路线](../../knowledge/linux/object_lifetime/kref/大纲.md#1.14_工程模板)按创建、集合、异步、关闭、分层持有顺序阅读P16～P31，返回P13总表，再沿[P14实验路线](../../knowledge/linux/object_lifetime/kref/大纲.md#1.15_源码阅读实验)进入P32～P40基础观察、交付、回收和诊断实验，最后回到[P15实际模板验收](../../knowledge/linux/object_lifetime/kref/P15_最终验收标准.md#15.8_最终代码模板验收)。源码问题从[固定版本索引](../../research/source_reading/kref/navigation/P01_Linux_6.12_kref源码阅读索引.md#1.2_按问题进入已落地证据)定位，不按本地实验提交推断上游行为。
6. [devres资源账本与分组](../../knowledge/linux/object_lifetime/devres/大纲.md#1.1_从退出责任进入资源接口)：先运行六条C回滚路径，再查具体资源接口。
...
2. 沿[内存顺序](../../knowledge/linux/synchronization_and_asynchrony/synchronization/memory_ordering/大纲.md)先运行[一次发布](../../knowledge/linux/synchronization_and_asynchrony/synchronization/memory_ordering/P01_READ_WRITE_ONCE_与_SMP_内存顺序原语.md#1.2.2_先运行一个有明确语言契约的发布程序)，区分标准线程实验与Linux模型证据，再进入[锁机制](../../knowledge/linux/synchronization_and_asynchrony/synchronization/locks/大纲.md)，先区分可睡与不可睡上下文，再沿[锁源码总阅读索引](../../research/source_reading/locking/navigation/P01_Linux_6.12_锁源码总阅读索引.md#1.6_建议阅读顺序)核对 spinlock、mutex 与 rwsem；随后用 [Lockdep 专题](../../knowledge/linux/synchronization_and_asynchrony/synchronization/lockdep/大纲.md#1.1_专题定位)和[源码索引](../../research/source_reading/lockdep/navigation/P01_Linux_6.12_Lockdep源码导读.md#1.6_建议阅读顺序)把锁序、IRQ 上下文和持锁前置条件转成动态验证证据。
