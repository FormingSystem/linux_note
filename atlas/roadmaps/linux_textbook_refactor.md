...
| B05 并发与事件：synchronization_and_asynchrony 125 篇 | 单个操作及对象生命期 | 从两条交错路径推出同步、等待、通知、延迟执行与回收；以具体状态完成证明 | [同步与异步总纲](../../knowledge/linux/synchronization_and_asynchrony/大纲.md)；已完成执行路径先修、总纲及内存顺序P01～P03，余下正文继续按依赖拆批；kernel_subsystems/irq 当前无正式文件，不建立占位入口 |
