...
**2026-09-25 质量结论更正：此前批次中的“完成”、`validated` 与“剩余328项”保留为历史工作记录，暂停作为教材质量完成度使用。** 开发者指出class_sysfs的卡片化问题后，跨批次抽查又发现Maple游标的字段/状态集中引入、Lockdep成本章的工具与核对清单堆叠等讲解断点。全量清单现增加独立的质量复审要求，覆盖全部条目并按文档职责判断；已通过的源码和技术验证保留，不能代替可学性验收。先修复class_sysfs，再回查此前已标完成批次。尚未逐篇复核，不能给出新的教材完成百分比或把全部旧文一概判坏。
...
| B05 并发与事件：synchronization_and_asynchrony 125 篇 | 单个操作及对象生命期 | 从两条交错路径推出同步、等待、通知、延迟执行与回收；以具体状态完成证明 | [同步与异步总纲](../../knowledge/linux/synchronization_and_asynchrony/大纲.md)；已完成执行路径先修、总纲及内存顺序P01～P10及其大纲，余下正文继续按依赖拆批；kernel_subsystems/irq 当前无正式文件，不建立占位入口 |
...
B05k完成[P10诊断参考](../../knowledge/linux/synchronization_and_asynchrony/synchronization/memory_ordering/P10_子系统边界_误用诊断与选型.md#10.1.1_有发布取得为什么仍会拼出两轮字段)及[内存顺序大纲](../../knowledge/linux/synchronization_and_asynchrony/synchronization/memory_ordering/大纲.md#1.2_递进阅读路线)，区分跨轮覆盖、长期持有、设备门铃与完成，十章知识主线作者审查收束但版本导读仍pending。清单682项297 validated、367 pending、18 supporting_pending；继续内存顺序版本导读，见[B05k](../../governance/migration/repository_textbook_refactor.md#1.4.166_B05k子系统边界与阅读路线收束)。

B05l新增[单次访问模块导读](../../research/source_reading/memory_ordering/navigation/P02_单次访问与类型边界导读.md#2.1_从一个读取现场进入头文件)及[rwonce唯一实现](../../research/source_reading/memory_ordering/source_explanations/include/asm-generic/rwonce.h.md#1.1_沿读取现场核对实现)，源码总索引迁入navigation并同步真实调用方。清单684项299 validated、367 pending、18 supporting_pending；总索引的屏障与模型部分仍pending，下一项按固定模型继续展开。见[B05l工作记录](../../governance/migration/repository_textbook_refactor.md#1.4.167_B05l单次访问模块与唯一实现)。

B05m完成[公共SMP屏障配置模块](../../research/source_reading/memory_ordering/navigation/P03_SMP屏障的配置与调用层次.md#3.1_同一个调用为什么走两条路径)与[三种入口唯一实现](../../research/source_reading/memory_ordering/source_explanations/include/asm-generic/barrier.h.md#1.1_从公共调用找到真正执行的分支)，八组宿主替身路径区分UP/SMP、公共覆盖与内部覆盖，未执行目标顺序验证。清单686项301 validated、367 pending、18 supporting_pending；源总索引仍pending，继续发布取得与辅助原语。见[B05m记录](../../governance/migration/repository_textbook_refactor.md#1.4.168_B05m公共屏障配置与检测边界)。

B05n完成[发布取得源码模块](../../research/source_reading/memory_ordering/navigation/P04_发布取得的访问与配置导读.md#4.1_把屏障绑定到一次访问)并扩展[barrier唯一实现](../../research/source_reading/memory_ordering/source_explanations/include/asm-generic/barrier.h.md#1.6_发布取得的内部回退)，七定义与固定源码一致，八配置组合的节选/完整头对照及求值、保存值、类型门槛检查通过。清单687项302 validated、367 pending、18 supporting_pending；源码总索引仍pending，继续store_mb与atomic辅助。见[B05n记录](../../governance/migration/repository_textbook_refactor.md#1.4.169_B05n发布取得与类型求值边界)。

B05o完成[存储后屏障与原子强化模块](../../research/source_reading/memory_ordering/navigation/P05_存储后屏障与原子强化导读.md#5.1_屏障位于写前还是写后)及[九项唯一实现](../../research/source_reading/memory_ordering/source_explanations/include/asm-generic/barrier.h.md#1.9_存储后屏障与atomic辅助的内部回退)，十组节选/完整头配置对照通过，分开顺序方向、RMW适用性及间隙访问。清单688项303 validated、367 pending、18 supporting_pending；源码总索引仍pending，继续条件加载与架构边界。见[B05o记录](../../governance/migration/repository_textbook_refactor.md#1.4.170_B05o存储后屏障与原子强化)。

B05p完成[条件加载模块](../../research/source_reading/memory_ordering/navigation/P06_条件加载与控制依赖导读.md#6.1_一次取得还不能等待条件成立)与[三项唯一实现](../../research/source_reading/memory_ordering/source_explanations/include/asm-generic/barrier.h.md#1.12_条件加载与控制依赖补强)，核对一次指针求值、每轮VAL、退出后补强与保存返回值，二十四组节选/固定头检查区分宏返回与夹具停止。清单689项304 validated、367 pending、18 supporting_pending；版本总索引仍pending，继续ARM与其他顺序域边界。见[B05p记录](../../governance/migration/repository_textbook_refactor.md#1.4.171_B05p条件加载与退出样本)。
...
B05r重写总导读的[模型输入单元](../../research/source_reading/memory_ordering/navigation/P08_LKMM输入_分类与工具配置.md#8.1.1_用一次发布示例核对地址和标签)与工具配置解释，用完整MP材料区分共享地址、局部结果、事件标签和候选谓词。静态匹配输入及固定定义，不冒充herd7结果；总导读仍pending，692项中307 validated、367 pending、18 supporting_pending不变。后续继续bell/cat/lock模型，见[B05r记录](../../governance/migration/repository_textbook_refactor.md#1.4.173_B05r模型输入与配置边界)。

B05s完成[bell分类与读侧配对单元](../../research/source_reading/memory_ordering/navigation/P08_LKMM输入_分类与工具配置.md#8.2_linux_kernel_bell_给事件分类)，补足类别/标签、嵌套匹配、SRCU数据流及Marked边界。cat公理与锁模型尚待处理，总导读仍pending；清单692项、307 validated、385项待办不变。见[B05s记录](../../governance/migration/repository_textbook_refactor.md#1.4.174_B05s事件分类与读侧配对)。

B05t完成[公理检查](../../research/source_reading/memory_ordering/navigation/P09_LKMM公理_锁关系与验证边界.md#9.1.1_先问候选执行违反了哪一种约束)和[锁候选关系](../../research/source_reading/memory_ordering/navigation/P09_LKMM公理_锁关系与验证边界.md#9.2.1_从取得成功与观察失败分开读)正文，区别原子性、顺序、传播、诊断及加锁资格。总导读的模型单元已逐步展开，下一步按阅读职责整理模块并做整体收束；清单仍692项307 validated、385项待办。见[B05t记录](../../governance/migration/repository_textbook_refactor.md#1.4.175_B05t公理检查与锁候选关系)。
...
B05aa完成[完成量与C令牌实验](../../knowledge/linux/synchronization_and_asynchrony/synchronization/waiting_notification/P02_completion_完成量.md)、[令牌调用链](../../knowledge/linux/synchronization_and_asynchrony/synchronization/waiting_notification/P06_completion令牌状态与调用链.md)和[swait与退出选择](../../knowledge/linux/synchronization_and_asynchrony/synchronization/waiting_notification/P07_swait生命周期调试与选型.md)，补足串轮反例、取消与完成依赖、两类广播边界。698项324 validated、356 pending、18 supporting_pending，剩余374项；继续wait_event、独占及版本化源码。见[B05aa记录](../../governance/migration/repository_textbook_refactor.md#1.4.182_B05aa完成量轮次与swait边界)。

B05ab完成[等待调用链](../../knowledge/linux/synchronization_and_asynchrony/synchronization/waiting_notification/P04_wait_event入队与唤醒调用链.md)、[独占扫描C模型](../../knowledge/linux/synchronization_and_asynchrony/synchronization/waiting_notification/P05_独占等待批量唤醒与公平性.md)及[因果大纲](../../knowledge/linux/synchronization_and_asynchrony/synchronization/waiting_notification/大纲.md)。等待知识侧七章和大纲完成本轮作者审查，五篇版本化源码仍继续独立处理。698项327 validated、353 pending、18 supporting_pending，剩余371项。见[B05ab记录](../../governance/migration/repository_textbook_refactor.md#1.4.183_B05ab等待宏与独占扫描)。

B05ac完成[普通等待源码模块](../../research/source_reading/waiting_notification/navigation/P02_Linux_6.12_普通等待队列模块源码概念导读.md#2.2.1_队列头与栈上entry)及按上游位置组织的wait.h/wait.c实现，补齐真实分支与正常/信号时序，9块源码静态比对通过。699项330 validated、351 pending、18 supporting_pending，剩余369项；继续completion源码与总索引。见[B05ac记录](../../governance/migration/repository_textbook_refactor.md#1.4.184_B05ac普通等待源码分层)。

B05ad完成[completion源码模块](../../research/source_reading/waiting_notification/navigation/P03_Linux_6.12_completion模块源码概念导读.md#3.2_状态所有权)、头文件与C实现和总索引，补足try消费与done观察边界，11块固定源码比对通过。700项334 validated、348 pending、18 supporting_pending，剩余366项；继续seqcount一致快照专题。见[B05ad记录](../../governance/migration/repository_textbook_refactor.md#1.4.185_B05ad完成量源码与总索引)。

B05ae完成[seqcount入门](../../knowledge/linux/synchronization_and_asynchrony/synchronization/sequence_counters/P01_seqcount_seqlock_读重试快照机制.md)与[一致快照证明](../../knowledge/linux/synchronization_and_asynchrony/synchronization/sequence_counters/P02_一致快照的证明模型.md)，补初始化及局部副本交付，完整C模型双编译器确认35/70条交错的接受边界。700项336 validated、346 pending、18 supporting_pending，剩余364项；继续读写路径、latch及关联锁。见[B05ae记录](../../governance/migration/repository_textbook_refactor.md#1.4.186_B05ae一致快照入门与证明)。

B05af完成[读写顺序及raw差异](../../knowledge/linux/synchronization_and_asynchrony/synchronization/sequence_counters/P03_seqcount读写路径与内存顺序.md)与[latch双副本](../../knowledge/linux/synchronization_and_asynchrony/synchronization/sequence_counters/P04_seqcount_latch双副本状态机.md)，210条C交错揭示最低位验证错收21条候选。700项338 validated、344 pending、18 supporting_pending，剩余362项；继续关联锁、生命周期及源码独立审查。见[B05af记录](../../governance/migration/repository_textbook_refactor.md#1.4.187_B05af读写顺序与latch双副本)。
...
B05aj完成[自旋入门](../../knowledge/linux/synchronization_and_asynchrony/synchronization/locks/P01_自旋锁.md)，以完整采样辅助代码推导互斥、本地IRQ重入与RT差异。701项347 validated、336 pending、18 supporting_pending，剩余354项；内核实例未编译运行，继续P04实现边界。见[B05aj记录](../../governance/migration/repository_textbook_refactor.md#1.4.191_B05aj自旋入门与中断重入)。
...
B05bb完成[依赖环与IRQ规则](../../knowledge/linux/synchronization_and_asynchrony/synchronization/lockdep/P05_递归_依赖环_IRQ与读写规则.md)，补间接中断闭包和递归读不阻塞反例。705项368 validated、319 pending、18 supporting_pending，剩余337项；继续查询接入，未执行目标实验。见[B05bb记录](../../governance/migration/repository_textbook_refactor.md#1.4.209_B05bb强路径与IRQ闭包)。
...
B05bu补初始化的两个完成点、链桶配置分支和修改反例，并增加身份模块精确入口。整页仍pending，剩余328项；未执行内核启动与配置矩阵。见[B05bu记录](../../governance/migration/repository_textbook_refactor.md#1.4.228_B05bu启动初始化的两个完成点)。
