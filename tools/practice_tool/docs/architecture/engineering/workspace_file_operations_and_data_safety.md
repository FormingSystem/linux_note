...
普通保存只修改当前文档。写入前验证能力、文件身份和内容摘要；写入后验证结果并推进 `saved_revision`。D1-SAVE 已实现这条安全替换主线，但 D2 本地历史尚未实现，因此当前只允许可原子替换且元数据可保真的文件；不得用原地写入扩大范围。
...
如果写入策略可能改变硬链接、ACL、扩展属性或所有权，必须先选择能够保持语义的策略；不能保证时拒绝并提供另存为。D2 完成后，保存失败还可依靠恢复备份与本地历史恢复；在此之前失败必须保持 Dirty 并明确说明没有恢复副本，不能通过重复强制覆盖“修好”。
