...
/sessions/:sessionId/learning/:itemId  # 学习导引
...
刷新页面后通过会话 ID 恢复状态。路由守卫必须处理会话不存在、会话已回收、阶段未解锁、题目不属于会话快照、知识来源暂不可用和 URL 参数无效。错误页提供回到大厅、返回合法当前位置和查看诊断信息入口，不自动创建或猜测会话。
