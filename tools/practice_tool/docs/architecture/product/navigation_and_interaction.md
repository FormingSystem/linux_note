...
训练库采用“左侧单元目录 + 右侧训练单元卡片”的双栏工作区。顶部主导航固定显示当前页面的撤销和重做操作，目录栏在桌面端随页面滚动保持可见。
...
保存草稿和校验发布属于整本电子书的目录级动作，不进入全局应用导航。“全书目录”采用顶部工具栏、中间独立滚动章节列表、底部操作栏三段式结构：顶部的全选、确认统计和排序操作固定，底部的状态、保存与发布操作固定，只有中间章节列表滚动。底部操作栏仅在已导入章节后显示，状态单独占一行，保存与发布等宽并排排列，并严格继承目录列的左右边界。操作栏不得脱离目录面板固定到视口，也不得横跨正文或遮挡产品导航。手机宽度下继续收敛在目录面板内部。
