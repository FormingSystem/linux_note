...
C++ 服务只负责工作区能力、文件语义、备份、监听、索引与搜索，不实现 Markdown DOM、Mermaid 或 Renderer 状态。第一阶段使用 4 字节大端长度前缀的 UTF-8 JSON 协议，控制帧上限 1 MiB，拒绝未知字段、未知方法、超长帧和版本不匹配。
...
- 首个第三方依赖为 `nlohmann/json 3.12.0`，通过官方发行归档和固定 SHA-256 获取。
