---
id: knowledge.kernel_subsystems.fundamentals.running_linux_kernel.p03_内核引导和初始化
title: "内核引导和初始化"
kind: publication
status: evolving
domains:
...
##### 3)\_TPL\_(Tertiary\_Program\_Loader)
...
#### (4)\_U-Boot\_启动阶段详细笔记(第二部分)

---

##### 1)\_SPL\_(Small\_Program\_Loader)
...
##### 2)\_U-Boot\_proper\_(Main\_U-Boot)
...
#### (5)\_U-Boot\_启动阶段详细笔记(第三部分\_CPU\_地址视角)
...
##### 5)\_衔接回\_SPL/U-Boot\_proper\_的运行
...
#### (6)\_U-Boot\_启动阶段详细笔记(第四部分\_重定位机制)
...
#### (8)\_U-Boot\_启动阶段全景笔记(含重定位流程)

------

##### 1)\_启动阶段对照表(含\_relocation)
...
##### 2)\_U-Boot\_main\_阶段的内存管理

###### a)\_常见情况\_开\_MMU\_但只做\_身份映射\_(VA=PA)
...
##### 3)\_Linux\_内存管理\_用户/内核空间分割

###### a)\_虚拟地址空间(32\_位\_CPU)
...
###### a)\_ARM\_LPAE(Large\_Physical\_Address\_Extension)
...
###### b)\_x86\_PAE\_(Physical\_Address\_Extension)
...
##### 3)\_Windows\_vs\_Linux\_32\_位地址空间对比表
...
#### (12)\_内存管理学习笔记(第三批\_32\_位大型游戏的内存架构)
...
#### (13)\_内存管理学习笔记(第四批\_32\_位\_vs\_64\_位对比)
...
#### (15)\_U-Boot\_2018.03\_crt0\_64.S(arm64)启动入口\_main\_全面笔记
...
##### 5)\_时间轴\_x\_资源可用性\_x\_调用归属(细粒度对照)
...
##### 6)\_board\_init\_f()\_/\_board\_init\_r()\_谁实现\_在哪里
...
###### b)\_board\_init\_f()\_谁来干\_在哪儿\_会调什么
...
###### c)\_board\_init\_r()\_谁来干\_在哪儿\_会调什么
...
###### f)\_SPL\_与\_Proper\_的\_board\_init\_f\_/\_board\_init\_r\_对比速查表
...
###### g)\_SPL\_与\_Proper\_的\_board\_init\_f\_/\_board\_init\_r\_调用链对比图
...
##### 7)\_小结\_从\_SPL\_过渡到\_U-Boot\_proper(main)的执行链
...
##### 8)\_重定位与返回地址(LR)修补\_两段平移(Δ1,\_Δ2)
...
##### 9)\_relocate\_code()\_做了什么\_(ARCH\_层)
...
#### (16)\_补充笔记\_board\_init\_f()\_/\_board\_init\_r()\_明细\_+\_板级目录结构
...
##### 1)\_board\_init\_f()\_阶段(早期\_SRAM/锁缓存\_BSS\_未就绪)
...
##### 3)\_board\_init\_r()\_阶段(重定位后\_DRAM\_BSS\_清零\_完整\_C\_环境)
...
##### 6)\_数据关系(从\_f\_to\_中间\_to\_reloc\_to\_r)
...
7. 函数boot_selected_os调用函数do_bootm_linux，这是第二次调用函数do_bootm_linux，参数flag是BOOTM_STATE_OS_GO。函数do_bootm_linux(flag = BOOTM_STATE_OS_GO)调用函数boot_jump_linux，该函数跳转到内核的入口，第一个参数是扁平设备树二级制文件的起始地址，后面3个采纳数现在没有使用。
   ...
   ![image](../../../assets/images/kernel/linux内核深度解析/boot_jump_linux函数.png)
...
#### (1)\_NOR\_Flash\_vs\_NAND\_Flash\_vs\_eMMC\_vs\_DDR\_内存\_深度对比
...
#### (1)\_笔记\_为什么\_U-Boot\_proper\_要在\_DDR\_中搬家
...
#### (2)\_笔记\_为什么\_U-Boot\_proper\_要在\_DDR\_中搬家\_(第二部分)

------

##### 1)\_为什么\_SPL/BootROM\_不直接放到高端
...
##### 2)\_为什么\_U-Boot\_proper\_要搬到高地址
...
#### (3)\_笔记\_为什么\_U-Boot\_proper\_要在\_DDR\_中搬家\_(第三部分)
...
#### (4)\_笔记\_为什么\_U-Boot\_proper\_要在\_DDR\_中搬家\_(第四部分)
...
#### (5)\_笔记\_为什么\_U-Boot\_proper\_要在\_DDR\_中搬家\_(第五部分)

------

##### 1)\_relocate\_code()\_的核心职责
...
#### (6)\_笔记\_为什么\_U-Boot\_proper\_要在\_DDR\_中搬家\_(第六部分)
...
#### (7)\_笔记\_U-Boot\_启动流程总览(带内存位置与\_gd\_演变)
