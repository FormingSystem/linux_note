...
仓库根目录的 `practice.cmd` 和 `practice.sh` 是当前知识库提供的集成快捷入口，只转发到上述工具启动脚本，不承载环境准备、依赖安装或服务启动逻辑。第一次启动由工具自身检查 Node.js：现有版本达到最低兼容线时直接使用，否则从 Node.js 官方最新 LTS 开始按 `24 → 22 → 20 → 18` 寻找当前平台可用的最高版本。Windows 优先使用 `winget` 并以官方 ZIP 兜底，普通 Linux 把校验后的官方归档安装到工具 `.local/runtime`，不替换系统 Node.js。依赖安装完成、Vite 开始监听后才打开浏览器。
