...
对象生命周期从[kref专题阅读地图](knowledge/linux/object_lifetime/kref/大纲.md#第1章_kref_引用计数机制章节大纲)开始：先建立引用责任和查找窗口，再处理异步交付、资源退出与框架边界；工程应用由[P13模板入口](knowledge/linux/object_lifetime/kref/P13_工程模板.md#13.1_本章导读_把规则落成可复制模板)按依赖组织P16～P31。已有代码出错时可查[诊断流程](knowledge/linux/object_lifetime/kref/P12_典型错误模式与调试线索.md#12.10.2_最小审查流程)，核对具体实现则进入[固定版本源码索引](research/source_reading/kref/navigation/P01_Linux_6.12_kref源码阅读索引.md#1.2_按问题进入已落地证据)。验证范围与未执行项保存在[重构工作记录](governance/migration/repository_textbook_refactor.md)，不把宿主或前端检查当作目标运行结果。
