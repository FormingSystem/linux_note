...
对象生命周期的完整实验包括[动态对象与失败清理](knowledge/linux/object_lifetime/kref/P02_源码入口与结构定义.md#2.19_标准自定义引用对象模板)、[容器发布和撤下](knowledge/linux/object_lifetime/kref/P02_源码入口与结构定义.md#2.30.1_设计_A_容器持有引用)。P03 已收束引用、清理、责任表和业务关闭，[工作票据 C 模型](knowledge/linux/object_lifetime/kref/P03_kref_生命周期状态机.md#%287%29_所有权表要补充失败路径和取消路径)比较六条正常/拒绝/取消轨迹；[完整关闭模块](knowledge/linux/object_lifetime/kref/P03_kref_生命周期状态机.md#3.16_一个完整的生命周期模板)区分撤下入口和停止接纳业务；[三条规则的适用条件](knowledge/linux/object_lifetime/kref/P04_kref_三条核心规则.md#4.12_mutex/list_lookup_的最小模型)已补齐借用、转交与查找的差异；[条件取得模型](knowledge/linux/object_lifetime/kref/P05_基础_API_源码逐行讲解.md#5.7.2_kref_get_unless_zero%28%29_的使用场景)已补重试与异常返回边界；继续锁组合及后续应用单元。其他静态存储、快照及 C++ 对照入口见专题大纲和 Atlas 索引。
