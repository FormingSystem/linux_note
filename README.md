...
对象生命周期按[kref专题大纲](knowledge/linux/object_lifetime/kref/大纲.md)连续阅读；P01～P12已完成本轮作者审查，人工评审状态仍以评审地图为准，[创建与回滚模板](knowledge/linux/object_lifetime/kref/P16_对象创建与失败清理模板.md#16.2.2_模板二_alloc/init/get/put/release_分层模板)已提供完整失败阶段实验，[链表发布与重复撤下](knowledge/linux/object_lifetime/kref/P17_拥有型链表工程模板.md#17.1_拥有型链表的发布与撤下)已按完整服务契约收束，[拥有型哈希模板](knowledge/linux/object_lifetime/kref/P18_拥有型哈希与IRQ工程模板.md#18.1_拥有型哈希与IRQ上下文)已补齐碰撞、份额及IRQ上下文边界，[XArray与期待对象删除](knowledge/linux/object_lifetime/kref/P19_XArray身份与删除工程模板.md#19.1_整数索引与期待对象删除)已明确锁窗口和编号复用边界，四个单元现已独立为P16～P19，由[P13工程入口](knowledge/linux/object_lifetime/kref/P13_工程模板.md#13.1_本章导读_把规则落成可复制模板)组织阅读；[工作交付与关闭模板](knowledge/linux/object_lifetime/kref/P20_工作交付与关闭工程模板.md#20.1_先选择分享还是转交)已比较分享、转交及迟到投递，[定时器交付与同步关闭](knowledge/linux/object_lifetime/kref/P21_定时器交付与同步关闭模板.md#21.1_先把排队状态与对象责任分开)已补齐单次票据和退出，[完成事件与等待者](knowledge/linux/object_lifetime/kref/P22_完成事件与等待者工程模板.md#22.1_等待之前先取得合法对象)已复核输入资格、超时和广播边界，[分阶段失败与资源回滚](knowledge/linux/object_lifetime/kref/P23_分阶段失败与资源回滚模板.md#23.1_先列出失败时已经成立的责任)已完成十二条C轨迹，[删除入口与活动排空](knowledge/linux/object_lifetime/kref/P24_删除入口与活动排空工程模板.md#24.1_关闭业务不等于回收所有对象)已核对业务门、发布份额与资源期限，[RCU查找与业务关闭](knowledge/linux/object_lifetime/kref/P25_RCU查找与业务关闭工程模板.md#25.1_同一对象上有三组独立状态)已复核完整发布及回调退出，[弱缓存撤销与RCU回收](knowledge/linux/object_lifetime/kref/P26_弱缓存撤销与RCU回收模板.md#26.1_新读区为什么救不了旧缓存)已补齐清槽和替换协议，[父子桥接与非拥有链表](knowledge/linux/object_lifetime/kref/P27_父子桥接与非拥有链表模板.md#27.1_一条关系对应几份引用)已核对最终摘链与桥接归还，[文件实例与私有对象](knowledge/linux/object_lifetime/kref/P28_文件实例与私有对象持有模板.md#28.1_计数单位是文件实例)已补dup与失败交付边界，[引用封装与调试责任](knowledge/linux/object_lifetime/kref/P29_引用封装与调试责任模板.md#29.1_接口先交代谁拥有哪一份)已核对单次求值与日志开关，[状态观察与活动接纳](knowledge/linux/object_lifetime/kref/P30_状态观察与活动接纳模板.md#30.1_LIVE不是一张永久通行证)已用十条C路径核对排空边界，[接口契约与退出审查](knowledge/linux/object_lifetime/kref/P31_接口契约与退出审查模板.md#31.1_把成功和失败分别写进注释)已收束P13工程模板；继续P14源码实验。[责任账本与诊断工具](knowledge/linux/object_lifetime/kref/P12_典型错误模式与调试线索.md#12.2.1_调试工具先导)和[章末排查练习](knowledge/linux/object_lifetime/kref/P12_典型错误模式与调试线索.md#12.10.2_最小审查流程)把拥有关系、访问期限、资源退出与证据边界连成一条排错路线。代表性完整实验包括[动态对象与失败清理](knowledge/linux/object_lifetime/kref/P02_源码入口与结构定义.md#2.19_标准自定义引用对象模板)、[链表服务与锁](knowledge/linux/object_lifetime/kref/P09_kref_与锁的组合.md#9.6.1_一个完整的锁_+_kref_对象模板)、[RCU查找与回调退出](knowledge/linux/object_lifetime/kref/P10_kref_与_RCU.md#10.3.1_基础对象模型)、[命名对象撤下](knowledge/linux/object_lifetime/kref/P11_kref_refcount_t_kobject_的边界.md#11.3.1_kobject_不只是引用计数)和[独立会话与设备份额](knowledge/linux/object_lifetime/kref/P11_kref_refcount_t_kobject_的边界.md#11.5.4_一个典型的分层结构)。它们区分存储寿命、业务关闭和框架退出；各批验证与未执行项见[工作记录](governance/migration/repository_textbook_refactor.md#1.4.115_B04az诊断章末收束)，不将作者审查或编译前端结果等同于目标机运行。
