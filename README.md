...
评审状态使用三种标识：🔴 **AI 创建**（默认）、🟡 **人工评审中**、🟢 **评审完成**。这套状态是面向读者的内容可信度提示，与 Front Matter 中表示维护阶段的 `draft`、`evolving`、`maintained`、`archived` 不是同一套状态。

🔴 **AI 创建**（默认）：

- 是我从GPT处获得或者codex让它生成的查询和理解子模块的第一阶段，由于当时处于知识面窄，联想断裂阶段，所以知识上的准确率和观察视角会有偏差，读者需要小心处理这些模块。
- 我当时只是做了简单粗略的筛选和思考，甚至有些错误批注，为避免误导，请让ai再帮忙做矫正之后阅读。为了避免给读者造成误导，将该阶段标记为AI创建也没有啥问题。
- 起码能够保证不是纯AI文档，上库都是做过一次人工筛选的。
- 不够我想这些专题的观察视角也足够读者了解这些子模块和专题的内容了。正确率在基线上是可以保证的。
...
- 地图已经展开所有正式专题的真实 `PXX` 章节；当前只有 🟢 **kref** 的 15 章完成评审，🟡 **Lockdep** 的 P01～P03 正在评审，其余正式章节保持 🔴 默认状态。
