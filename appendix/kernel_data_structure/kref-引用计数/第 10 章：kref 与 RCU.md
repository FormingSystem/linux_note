...
### 10.2.2 为什么 RCU lookup 不能直接 kref_get()
...
### 10.2.3 kref_get_unless_zero() 解决什么，不解决什么
...
### 10.3.4 加入 dying 状态后的 lookup 模板
...
## 10.4 删除路径：取消发布、引用归零与延迟释放

### 10.4.1 remove 路径：先取消发布，再释放引用
...
### 10.4.3 list_del_rcu()、kref_put()、kfree_rcu() 的职责边界
...
### 10.4.4 remove 和 release 是否必须在同一个地方脱链

不一定。

有两种模型。

#### 10.4.4.1 模型 A：release 中脱链
...
#### 10.4.4.2 模型 B：remove 中脱链，release 只释放内存
...
## 10.5 读侧约束：临界区、字段一致性和子资源

### 10.5.1 RCU 读侧临界区内应该做什么
...
### 10.7.1 错误一：RCU lookup 后裸 kref_get
...
### 10.7.2 错误二：离开 RCU 后才 get_unless_zero
...
### 10.7.3 错误三：list_del_rcu 后直接 kfree
...
### 10.7.4 错误四：以为 list_del_rcu 后所有读者都看不到对象
...
### 10.7.7 错误七：release 中提前释放 RCU 子资源
