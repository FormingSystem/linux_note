...
## 5.6 普通引用 API：kref_get() 和 kref_put()
...
### 5.6.8 kref_put() 和 refcount_dec_and_test()
...
### 5.7.2 kref_get_unless_zero() 的使用场景
...
### 5.7.3 kref_get_unless_zero() 不解决裸指针有效性
...
### 5.8.4 kref_put_mutex/kref_put_lock 的共同边界
...
### 5.12.2 不要绕过 kref 直接操作 refcount
...
### 5.13.1 误用 1：把 kref_init 当 reset
...
### 5.13.2 误用 2：用 kref_read 判断对象是否可 get
...
### 5.13.3 误用 3：忽略 kref_get_unless_zero 返回值
...
### 5.13.4 误用 4：put 后继续使用返回值判断对象安全
...
### 5.13.5 误用 5：普通 release 用在 kref_put_lock
