...
## 12.3 基础引用错误：少 get、少 put、多 put

### 12.3.1 错误一：少 get，异步路径 UAF
...
### 12.3.3 错误三：多 put，提前 release 或 underflow
...
#### 12.3.3.4 错误代码：handoff 后当前路径又 put
...
## 12.4 lookup 与 handoff 错误：从指针到引用的窗口

### 12.4.1 错误五：lookup 后无保护 get
...
### 12.4.2 错误六：handoff 成功和失败路径归属不清
...
#### 12.4.2.5 正确写法三：submit_take_always
...
## 12.5 release 与 remove 错误：销毁边界要收干净

### 12.5.1 错误七：release 里重新发布对象
...
### 12.5.2 错误八：release 时对象仍挂在全局结构中
...
## 12.6 并发与 API 误用：kref 不是锁，也不是状态判断

### 12.6.1 错误九：把 kref 当锁，字段竞争仍然存在
...
### 12.6.3 错误十一：kref_read 用来判断能否访问对象
...
### 12.6.4 错误十二：kref_get_unless_zero 返回值不检查
...
## 12.7 remove、上下文和辅助 API：引用之外还有收尾协议

### 12.7.1 错误十三：remove 后已有引用没有 drain
...
### 12.7.3 错误十五：kref_put_lock / kref_put_mutex 使用误解
...
## 12.8 driver core 层次错误：device 引用和私有 kref

### 12.8.1 错误十六：device 对象上手动操作内部 kref
...
### 12.8.2 错误十七：私有 kref 和 device 引用混用
...
## 12.9 调试方法：从所有权表到 release 断言

### 12.9.1 调试方法一：写引用所有权表
