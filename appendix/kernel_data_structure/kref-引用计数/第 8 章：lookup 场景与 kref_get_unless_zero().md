...
### 8.2.1 lookup 为什么是 kref 最容易出错的场景
...
### 8.3.1 正确模型一：mutex/list lookup + `kref_get()`
...
### 8.3.3 remove/unlink 与 lookup 的顺序
...
## 8.4 `kref_get_unless_zero()`：防复活，不防悬挂指针
...
### 8.4.1 `kref_get_unless_zero()` 的定位
...
### 8.4.2 `kref_get_unless_zero()` 不解决什么
...
### 8.4.3 `kref_get_unless_zero()` 仍然需要锁或 RCU
...
### 8.4.4 什么时候用 `kref_get()`，什么时候用 `kref_get_unless_zero()`

#### 8.4.4.1 可以确认 refcount 一定非 0：用 `kref_get()`
...
#### 8.4.4.2 可能看到正在退出的对象：用 `kref_get_unless_zero()`
...
### 8.4.5 mutex + list + `kref_get_unless_zero()` 模板
...
### 8.6.1 `lookup_raw()` 和 `lookup_get()` 必须分开

建议工程里明确区分两类函数。

#### 8.6.1.1 `lookup_raw()`：只返回临时裸指针
...
#### 8.6.3.2 把 lookup 得到的引用直接转移给异步路径
...
### 8.6.4 lookup 函数不要返回“可能要 put”的对象
...
### 8.7.3 `DYING` 状态通常拒绝新的 lookup 引用
...
### 8.7.4 `kref_get_unless_zero()` 和对象状态不能互相替代
...
#### 8.8.1.2 以为 `kref_get_unless_zero()` 可以替代锁
...
#### 8.8.1.4 remove 时先 put 后 unlink
...
#### 8.8.1.6 lookup 得到引用后 handoff 失败忘记回滚
...
#### 8.8.2.5 RCU + `kref_get_unless_zero()` 模板预告
