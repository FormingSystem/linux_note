...
#### 8.2.1.2 `struct rb_node`：Linux rbtree 的节点骨架
...
#### 8.2.1.3 `__rb_parent_color`：父指针和颜色压缩存储
...
#### 8.2.1.4 `rb_left` 和 `rb_right`：左右孩子仍然是显式指针
...
##### Tip：`RB_ROOT` 对象式宏：把结构体默认初始化封装成可复用模板
...
###### 3. 为什么要写成带类型的 `(struct rb_root) { NULL, }`
...
#### 8.2.1.6 `struct rb_root_cached`：缓存最左节点的红黑树根
...
老传统了，这和内核链表操作时一个套路。参考[`container_of`：通过成员地址反推结构体地址](../../GNU_extension_of_C_language/C_language_extension.md#3.2 `container_of`：通过成员地址反推结构体地址)
...
#### 8.2.1.11 Linux 模型：红黑树是嵌入式节点维护工具
...
#### 8.2.1.12 空叶子差异：教材 NIL 节点 vs Linux NULL 指针
...
#### 8.2.1.13 根结构差异：完整 tree 对象 vs 极简 rb_root
