...
### 12.0.1 本章在 Linux rbtree 学习路线中的位置
...
### 12.1.3 `rb_insert_color_cached()` 的使用方式
...
### 12.1.4 `rb_erase_cached()` 如何更新最左缓存
...
### 12.2.2 `struct rb_augment_callbacks` 的三个回调
...
### 12.2.4 `RB_DECLARE_CALLBACKS()` 与 `RB_DECLARE_CALLBACKS_MAX()`
...
### 12.2.5 `rb_insert_augmented()` 的插入流程
...
### 12.2.6 `rb_erase_augmented()` 的删除流程
...
### 12.3.3 `WRITE_ONCE()` 在 rbtree 实现中的意义
...
### 12.3.5 lockless lookup 能保证什么，不能保证什么
...
### 12.5.5 如何验证 cached rbtree 的 `rb_leftmost`
...
### 12.5.6 如何验证 augmented rbtree 的增强信息
...
### 12.6.8 误以为 cached / augmented 会自动维护业务字段
...
### 12.7.5 VMA 历史上使用 rbtree 与后来转向 Maple Tree 的原因
...
### 12.7.6 interval tree 与 augmented rbtree 的关系
