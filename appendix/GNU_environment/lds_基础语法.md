...
# 第 2 部分：段的排序、过滤、内存分配、KEEP 保留机制

## 段的排序与过滤：`*(.text*)`、`KEEP()`、`SORT()`
...
## 精确控制输入文件和段：`*(filename.o)` 和 `(EXCLUDE_FILE)`
...
## 构造器与数组段：`.ctors`, `.init_array`, `.fini_array`
...
## U-Boot 2022.10 `u-boot.lds` 详解（结合上文的链接语法）
...
# 第 6 部分： `.rel.dyn`、`.bss`、`.u_boot_list`段与 `board_init_f()` 和 SPL 阶段 `.lds` 的协同机制

## `board_init_f()` 怎样协同对 `.bss`、`.rel.dyn`、`.u_boot_list` 段配合使用？
...
### SPL 不调用 `relocate_code()`，而主 U‑Boot 在 `board_init_f()` 中会：
...
# 第 7 部分：uboot的每个阶段的程序的运行逻辑

## U-Boot 多阶段启动总览（重点聚焦职责与角色）
...
###  阶段 1：TPL（Tiny Primary Loader）【可选】
...
### 阶段 2：SPL（Secondary Program Loader）
...
# 第 8 部分：u-boot启动阶段，soc所用到的对应的硬件

## 启动流程外设交互总览（SoC 与外设映射）
...
## 分阶段详细解析（含硬件交互与设备寄存器作用）

### BootROM 阶段（芯片内置 ROM 代码）
