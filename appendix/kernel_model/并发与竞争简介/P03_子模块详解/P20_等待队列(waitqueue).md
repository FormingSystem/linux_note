...
## 20.3_数据结构视角_waitqueue_head_与_wait_queue_entry
...
### 20.3.3_struct_wait_queue_entry
...
## 20.4_核心接口族_wait_event_/_wake_up_/_prepare_to_wait_详解
...
### 20.4.4_prepare_to_wait_与_finish_wait
...
## 20.6_等待队列的三视角分析_生产者_消费者与全局协作

------

### 20.6.1_设计动机_为什么_waitqueue_模型是_合理_的
...
#### (2)_wake_up()_vs_wake_up_one()_对比
...
## 20.8_低层等待接口族_prepare_to_wait*()_与_finish_wait()
...
### 20.8.3_典型使用模式(等价于_wait_event_循环)
...
## 20.9_任务状态语义_TASK_INTERRUPTIBLE_/_UNINTERRUPTIBLE_/_KILLABLE_等待模式
...
## 20.10_等待队列与_schedule_timeout_超时等待机制详解
...
### 20.10.3_schedule_timeout()_工作机制
...
### 20.10.4_wait_event_*_timeout()_内部流程
...
### 20.10.7_与_finish_wait()_任务状态的关系
...
## 20.11_等待队列_超时与信号交互_interruptible_与_killable_的实战差异

------

### 20.11.1_概念_信号与等待的交汇点
...
### 20.11.3_TASK_INTERRUPTIBLE_与_TASK_KILLABLE_的本质差别
...
## 20.12_completion_机制_单事件同步的最小原语

------

### 20.12.1_概念_等待队列的_单事件封装
...
### 20.12.9_completion_与_wait_queue_的差异
...
## 20.13_complete()_与唤醒机制_任务唤醒数量与状态传播原理
...
### 20.13.3_核心语义_唤醒数量由_nr_exclusive_参数控制
...
### 20.13.4_wake_up_common_lock()_的行为机制
...
## 20.14_completion_与_waitqueue_的混搭_多阶段同步模型
...
## 20.15_complete()_to_try_to_wake_up()_唤醒路径与调度切换详解
...
## 20.16_completion_的_done_计数机制与唤醒行为解耦原理
...
### 20.16.3_生产者行为_complete()_与_complete_all()
...
### 20.16.4_消费者行为_wait_for_completion()
...
### 20.16.6_reinit_completion()_状态复位机制
...
### 20.16.7_complete_all()_与_done_饱和行为
...
## 20.17_complete_all()_的广播模型与使用边界
...
## 20.18_wait_for_completion_interruptible()_与信号响应机制
...
### 20.18.6_与_wait_event_interruptible()_的对比
...
## 20.19_completion_同步模型在_probe/remove_中的设计模式
...
### 20.20.5_trace-cmd_与_KernelShark_可视化同步路径
...
### 20.20.6_/proc/sched_debug_与锁统计接口
...
#### (1)_错误_在中断上下文使用_wait_for_completion()
