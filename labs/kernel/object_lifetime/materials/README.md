...
[P13 XArray工程复用](../../../../knowledge/linux/object_lifetime/kref/P13_工程模板.md#13.4.3_模板五_xarray_+_kref_lookup_模板)使用同一完整模块，另给出同锁比较期待对象再__xa_erase的可选包装。三组新身份删除用例与六组既有用例通过，模块变体ARM前端通过；正式材料未改，未运行目标并发或真实节点算法。
...
[P13链表工程复用](../../../../knowledge/linux/object_lifetime/kref/P13_工程模板.md#13.4.1_模板三_list_+_mutex_+_kref_lookup_模板)使用同一note_kref_table.c，按接口契约及L0～L5解释一次发布、同ID拒绝与重复撤下；模块和既有六组夹具未改，本批不新增运行结论。
