...
### 12.1.1_fasync_vs_select/poll/epoll_适用范围与取舍
...
#### (6)_示例代码_同时支持.poll_与_fasync_的最小驱动_+_用户态对照
...
#### (8)_小结_在_通用事件接口_与_主动信号通知_之间做清晰分工
...
### 12.1.2_fasync_vs_input_子系统事件模型

#### (1)_引入_同样是_键盘/按键事件_为什么有两套方案
...
#### (2)_数据结构视角_event_count_vs_input_event_帧语义

##### 1)_fasync_字符设备的典型状态模型
...
##### 2)_input_子系统的事件模型_struct_input_event
...
#### (3)_开发者视角_写一个_裸_fasync_驱动_vs_接入_input_的差异

从内核驱动作者角度，对同一块按键硬件，两种路径的开发行为可以这样对比。

##### 1)_裸_fasync_字符驱动_完全自定义
...
#### (4)_用户/平台视角_evdev_的统一接口_vs_自定义协议
...
#### (6)_示例代码_同一_GPIO_按键的_fasync_版_vs_input_版(缩略)

为了对比清晰，这里给出两个**缩略示例**（省略 probe/资源申请/错误处理的大量细节），重点看“通知模型”。

##### 1)_示例一_fasync_字符设备按键驱动(概要)
...
#### (7)_调试与验证_fasync_字符驱动_vs_input_驱动的排错差异
...
#### (8)_小结_input_子系统是_规范化输入设备_的首选_fasync_更适合通用自定义
...
### 12.1.3_fasync_vs_netlink/kobject_uevent/sysfs_轮询

#### (1)_引入_设备_数据事件_vs_配置/拓扑变化事件
...
#### (2)_数据结构视角_per-fd_事件_vs_系统级广播

##### 1)_fasync_挂在具体_struct_file_上的通知链表
...
##### 2)_netlink_基于_struct_sock_的多播/单播消息通道
...
##### 3)_kobject_uevent_围绕_struct_kobject_的属性变化广播
...
#### (3)_开发者视角_哪些事用_fasync_哪些事用_netlink/uevent/sysfs
...
#### (4)_用户/平台视角_服务型进程_vs_系统守护进程

从用户态视角看，这几类通知机制面向的是不同角色的程序。

##### 1)_fasync/poll_面向_业务应用/服务进程
...
##### 2)_netlink/uevent_面向_系统管理/策略守护进程
...
#### (6)_示例代码_简单_netlink/uevent/sysfs_片段与_fasync_对照

本节只给“框架级”示例，避免展开完整子系统（那会非常长）。重点是感受“接口形态”。

##### 1)_netlink_内核端简单示例(自定义_family)
...
##### 2)_kobject_uevent_简单触发示例(属性改变时通知)
...
#### (7)_调试与验证_混用_fasync_与系统级通知时的边界检查
...
#### (8)_小结_fasync_是_设备事件级_机制_netlink/uevent/sysfs_是_系统级_机制
...
### 12.1.4_与_io_uring_/_AIO_等新型异步_I/O_机制的关系

#### (1)_引入_fasync_是_老接口_io_uring_是_新框架_它们怎么共存
...
#### (2)_数据结构视角_fasync_链表_vs_AIO/io_uring_上下文

先用结构对比的方式，把三者的核心抽象列出来（只取必要部分）。

##### 1)_fasync_fasync_struct_+_进程信号队列
...
##### 2)_传统_AIO_aio_context_+_kiocb_/_iocb
...
##### 3)_io_uring_SQ/CQ_环_+_io_uring_ctx
...
#### (3)_开发者视角_驱动要不要_特意支持_io_uring/AIO
...
##### 1)_最通用的做法_只实现.read/.write/.poll_让_io_uring_视你为_普通_fd
...
#### (4)_用户/平台视角_fasync_的进程模型_vs_io_uring_的队列模型

从用户态编程模型看，fasync 与 io_uring 差异非常明显。

##### 1)_fasync_以进程为中心的_信号唤醒模型
...
##### 2)_io_uring_以一次_io_uring_ctx_为中心的_操作队列模型
...
##### 3)_组合方式_用_io_uring_作为_事件循环内核_fasync_作为_某些设备的触发信号源
...
#### (5)_可视化_应用如何在同一个框架里用_io_uring_+_fasync_+_poll
...
#### (6)_示例代码_使用_io_uring_轮询字符设备_fd(依赖驱动.poll)
...
#### (7)_调试与验证_在_io_uring_+_fasync_混用场景下的注意点
...
#### (8)_小结_在现代异步_I/O_体系中_fasync_仍然是_简单_可组合_的一环
...
### 12.1.5_在实际工程中如何选择通知机制_决策矩阵与推荐组合

#### (1)_引入_从_我有一个设备_到_我该选哪一种通知机制
...
##### 2)_第二步_对于_数据面_先把.poll_+_waitqueue_写好
...
##### 5)_第五步_判断是否需要_netlink/uevent/sysfs_作为_控制面补充
...
##### 2)_模式_B_中等复杂度(多设备_+_多进程_+_系统守护)
...
#### (8)_小结_把_fasync_放回_通知机制家族_的工程化位置上
