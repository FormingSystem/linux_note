...
### 4.1.1 引入：为什么要从 `file->f_flags` 看起
...
### 4.1.2 数据结构视角：`struct file`、`f_flags` 与 `FASYNC`
...
### 4.1.3 开发者视角：内核中谁来修改 `f_flags` 和 `FASYNC`
...
#### 2）`fcntl(F_SETFL)` 路径中的 f_flags 修改
...
### 4.1.4 用户/平台视角：如何通过用户态控制 `FASYNC`
...
### 4.1.5 可视化：从 `O_ASYNC` 到 `file->f_flags.FASYNC` 的路径
...
### 4.1.6 示例代码：用户态 + 内核态如何围绕 `FASYNC` 协作

这一小节给出一组“能跑通”的最小示例片段，用于把 `file->f_flags` 与 `FASYNC` 的角色串起来。

#### 1）用户态：设置 FASYNC 并注册 SIGIO 处理函数
...
### 4.1.8 小结：`file->f_flags/FASYNC` 在 fasync 机制中的角色
...
### 4.2.1 为什么需要单独的 `struct fasync_struct`
...
### 4.2.2 `struct fasync_struct` 字段拆解
...
### 4.2.4 开发者视角：在驱动中如何持有 `struct fasync_struct *`
...
### 4.2.6 小结：`struct fasync_struct` 的角色
...
### 4.3.1 `.fasync` 在 file_operations 里的位置与作用
...
#### 1）使用全局 `struct fasync_struct *` 管理多个设备实例
...
#### 4）在中断上下文直接调用 `.fasync` / `fasync_helper()`
...
### 4.6.1 引入：为什么 fasync 还需要“owner”这一层
...
### 4.6.2 数据结构视角：`struct fown_struct` 的字段
...
### 4.6.3 内核开发者视角：`f_setown()` / `__f_setown()` 调用链
...
### 4.6.4 用户态视角：`F_SETOWN` / `F_SETSIG` 的行为规则
...
### 4.6.5 可视化：从 `F_SETOWN` 到 `send_sigio()` 的路径
...
### 4.7.1 顶层：`fcntl()` 与 `F_SETFL/F_SETOWN/F_SETSIG`
...
### 4.7.2 中层：`fasync_helper()` 与 `struct fasync_struct` 链表
