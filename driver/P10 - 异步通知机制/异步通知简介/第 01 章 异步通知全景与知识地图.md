...
### 1.1.6 示例代码：全书统一采用的最小 fasync 模板骨架
...
### 1.3.2 数据结构视角：fasync 的位置——fd 与信号之间的一条支路
...
### 1.3.3 开发者视角：哪类驱动“值得”做 fasync，哪类不必要
...
### 1.3.6 示例代码：同一驱动“有/无 fasync”改造的对比点

下面给出一个极简对比片段，强调“加 fasync 不改变原有 `read/poll` 路径”。

#### 1）原始版本：只支持 `read + poll`
...
### 1.3.7 调试与验证视角：如何确认问题在 fasync 边界内
