...
### 13.1.1 引入：从“按键中断”到“用户态 SIGIO 回调”
...
### 13.1.2 数据结构视角：`struct demo_async_key_dev` 如何承载完整状态
...
### 13.1.3 开发者视角：驱动实现的关键步骤（devm 版为主）

以 devm 版本为主线，从驱动作者角度，把实现拆成有明确顺序的步骤。

#### 步骤 1：解析设备树，获取 GPIO / 中断号 / 去抖参数
...
#### 步骤 5：`.open` / `.release` 中维护私有数据与 fasync 状态
...
### 13.1.4 用户/平台视角：如何从 SIGIO 看到真实按键事件
...
### 13.1.6 示例代码：devm 版完整驱动 + 用户态 SIGIO Demo
...
#### 13.1.6.1 内核驱动：`demo_async_key.c`（devm 版）
...
#### 13.1.6.2 用户态 SIGIO 示例：`demo_sigio_key.c`
...
### 13.1.7 调试与验证：从“设备不收信号”到“完整闭环”的排查步骤
...
### 13.1.8 小结：一个完整的“GPIO 中断 → fasync → SIGIO” 标准样例
...
### 13.2.1 引入：从“单个 SIGIO”到“多设备 + socket 的统一事件循环”
...
### 13.2.2 数据结构视角：从“每个 fd 一个 SIGIO”到“统一的 signalfd 事件源”
...
### 13.2.3 开发者视角：驱动侧几乎不用改，重点在用户态事件循环设计
...
### 13.2.4 用户/平台视角：signalfd + epoll 的统一处理流程

从用户/平台视角，完整流程可以概括为以下几步。

#### 13.2.4.1 步骤 1：创建 signalfd 并屏蔽普通信号
...
#### 13.2.4.2 步骤 2：打开各个 fasync 设备，并配置 F_SETOWN / O_ASYNC
...
#### 13.2.4.3 步骤 3：把 signalfd 与所有 fd 加入 epoll
...
#### 13.2.4.4 步骤 4：在 epoll 循环中统一处理事件
...
### 13.2.5 可视化：多设备 + signalfd + epoll 的整体拓扑
...
### 13.2.6 示例代码：多 fasync 设备 + signalfd + epoll 的用户态 demo
...
### 13.2.7 调试与验证：如何确认 signalfd 与 epoll 行为正确
...
### 13.2.8 小结：signalfd 是 fasync → 统一事件循环 的关键适配器
...
### 13.3.1 引入：从“用户态傻等”到“驱动主动通知”的演进路径
...
### 13.3.2 数据结构视角：从“裸全局变量”到“统一事件状态机”
...
#### 13.3.3.2 阶段 1：加入 waitqueue + `.poll`（但暂不引入 fasync）
...
#### 13.3.3.3 阶段 2：在 `.poll` 基础上引入 fasync + SIGIO
...
### 13.3.4 用户/平台视角：逐步迁移用户态程序的行为

改造驱动只是第一步，更关键的是 **用户态如何逐步迁移**，避免一次性大改出 bug。

一个合理的迁移顺序是：

#### 阶段 A：驱动已支持 `.poll`，用户态先从“死轮询”切换到 `poll`
...
#### 阶段 B：驱动加入 fasync 后，用户态增加 SIGIO 支持
...
### 13.3.5 可视化：轮询版 → `.poll` 版 → `fasync` 版的演进图
...
#### 13.3.6.2 阶段 1 核心差异（`.poll` + waitqueue）
...
#### 13.3.6.3 阶段 2 核心差异（在阶段 1 基础上加 fasync）
...
### 13.4.1 引入：同一个 fasync 驱动，为何要同时准备 devm / 非 devm 两套写法？
...
### 13.4.2 数据结构视角：devm 管的是“device 资源”，不是“file/fasync 资源”
...
### 13.4.3 开发者视角：同一个 fasync 驱动的 devm / 非 devm 差异点
...
#### 13.4.3.1 设备私有结构：`devm_kzalloc` vs `kzalloc`
...
#### 13.4.3.2 GPIO / IRQ 等资源：`devm_gpiod_get` / `devm_request_irq` vs 手动释放
...
#### 13.4.3.3 字符设备 / class / device 节点：devm 不会帮你管
...
#### 13.4.3.4 fasync 状态：devm 完全不介入，靠 `.release` 保证清理
...
### 13.4.4 用户/平台视角：devm 与非 devm 对用户态行为的影响（几乎为 0）
...
### 13.4.5 可视化：devm 与非 devm 在生命周期上的差异
...
### 13.4.6 示例代码：同一驱动的 devm / 非 devm 关键对比

这里给出一个压缩对照示例，仅保留 **资源分配/释放部分** 和 **fasync 部分**，方便在迁移时参考。

#### 13.4.6.1 devm 版骨架（示意）
...
### 13.4.7 调试与验证：从非 devm 迁移到 devm 时要重点检查什么？
...
### 13.4.8 小结：在 fasync 驱动里使用 devm 的原则与迁移建议
...
### 13.5.3 开发者视角：驱动实现过程中的“必写/可选/禁用”条目

这一节按主题拆分 checklist，你可以在写驱动时逐块对照。

#### 13.5.3.1 file_operations 与基本框架
...
### 13.5.4 用户/平台视角：驱动 checklist 如何映射到“可观测行为”
...
### 13.5.6 示例代码：内联注释版“最小合格 fasync 驱动骨架”
...
### 13.5.7 调试与验证：如何用 checklist 做代码评审与回归测试
...
### 13.6.1 引入：驱动已经“会叫人”，应用该怎么“听得懂、收得稳、退得干净”？
...
#### 13.6.2.2 signalfd + epoll 模式的状态结构
...
### 13.6.3 开发者视角：配置步骤与调用顺序的必选项 / 推荐项 / 禁用项

这一节按两种典型场景分别给出 checklist。

#### 13.6.3.1 场景一：传统 SIGIO + signal handler 模式
...
#### 13.6.3.2 场景二：signalfd + epoll 统一事件循环模式
...
### 13.6.4 用户/平台视角：多线程、多进程、退出路径的策略核对
...
#### 13.6.6.1 示例一：SIGIO + handler 最小模板
...
#### 13.6.6.2 示例二：signalfd + epoll 最小模板
...
### 13.6.7 调试与验证：如何用 checklist 排查用户态典型问题
...
### 13.6.8 小结：把“能收信号”变成“能稳定工作、能优雅退出”的规范
