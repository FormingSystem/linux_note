...
### 12.1.6 示例代码：同时支持 `.poll` 与 fasync 的最小驱动 + 用户态对照
...
#### 12.1.6.3 用户态 SIGIO 示例（与 epoll 对比）
...
### 12.1.7 调试与验证：如何确认四种机制在同一个驱动上行为一致
...
### 12.1.8 小结：在“通用事件接口”与“主动信号通知”之间做清晰分工
...
### 12.2.1 引入：同样是“键盘/按键事件”，为什么有两套方案？
...
### 12.2.2 数据结构视角：`event_count` vs `input_event` 帧语义

#### 12.2.2.1 fasync 字符设备的典型状态模型
...
#### 12.2.2.2 input 子系统的事件模型：`struct input_event`
...
### 12.2.3 开发者视角：写一个“裸 fasync 驱动” vs 接入 input 的差异

从内核驱动作者角度，对同一块按键硬件，两种路径的开发行为可以这样对比。

#### 12.2.3.1 裸 fasync 字符驱动：完全自定义
...
#### 12.2.3.2 接入 input 子系统：重用通用事件框架
...
### 12.2.4 用户/平台视角：evdev 的统一接口 vs 自定义协议

从用户态开发者的角度，差异更加明显。

#### 12.2.4.1 使用 evdev 的典型模式
...
#### 12.2.4.2 使用自定义 fasync 字符设备的典型模式
...
### 12.2.5 可视化：同一块 GPIO 按键的两种“上行路径”
...
### 12.2.6 示例代码：同一 GPIO 按键的 fasync 版 vs input 版（缩略）

为了对比清晰，这里给出两个**缩略示例**（省略 probe/资源申请/错误处理的大量细节），重点看“通知模型”。

#### 12.2.6.1 示例一：fasync 字符设备按键驱动（概要）
...
#### 12.2.6.2 示例二：input/evdev 按键驱动（概要）
...
### 12.2.7 调试与验证：fasync 字符驱动 vs input 驱动的排错差异
...
### 12.2.8 小结：input 子系统是“规范化输入设备”的首选，fasync 更适合通用自定义
...
### 12.3.1 引入：设备“数据事件” vs “配置/拓扑变化事件”
...
### 12.3.2 数据结构视角：per-fd 事件 vs 系统级广播

#### 12.3.2.1 fasync：挂在具体 `struct file` 上的通知链表
...
#### 12.3.2.2 netlink：基于 `struct sock` 的多播/单播消息通道
...
#### 12.3.2.3 kobject_uevent：围绕 `struct kobject` 的属性变化广播
...
#### 12.3.2.4 sysfs 轮询：基于属性文件的“慢速状态感知”
...
### 12.3.3 开发者视角：哪些事用 fasync，哪些事用 netlink/uevent/sysfs？
...
### 12.3.4 用户/平台视角：服务型进程 vs 系统守护进程

从用户态视角看，这几类通知机制面向的是不同角色的程序。

#### 12.3.4.1 fasync/poll 面向“业务应用/服务进程”
...
#### 12.3.4.2 netlink/uevent 面向“系统管理/策略守护进程”
...
### 12.3.6 示例代码：简单 netlink/uevent/sysfs 片段与 fasync 对照

本节只给“框架级”示例，避免展开完整子系统（那会非常长）。重点是感受“接口形态”。

#### 12.3.6.1 netlink 内核端简单示例（自定义 family）
...
#### 12.3.6.2 kobject_uevent 简单触发示例（属性改变时通知）
...
### 12.3.7 调试与验证：混用 fasync 与系统级通知时的边界检查
...
### 12.3.8 小结：fasync 是“设备事件级”机制，netlink/uevent/sysfs 是“系统级”机制
...
### 12.4.1 引入：fasync 是“老接口”，io_uring 是“新框架”，它们怎么共存？
...
### 12.4.2 数据结构视角：fasync 链表 vs AIO/io_uring 上下文

先用结构对比的方式，把三者的核心抽象列出来（只取必要部分）。

#### 12.4.2.1 fasync：`fasync_struct` + 进程信号队列
...
#### 12.4.2.2 传统 AIO：`aio_context` + `kiocb` / `iocb`
...
#### 12.4.2.3 io_uring：SQ/CQ 环 + `io_uring_ctx`
...
### 12.4.3 开发者视角：驱动要不要“特意支持 io_uring/AIO”？
...
#### 12.4.3.1 最通用的做法：只实现 `.read/.write/.poll`，让 io_uring 视你为“普通 fd”
...
#### 12.4.3.2 专用优化：实现 `->uring_cmd` 或配合块层框架
...
### 12.4.4 用户/平台视角：fasync 的进程模型 vs io_uring 的队列模型

从用户态编程模型看，fasync 与 io_uring 差异非常明显。

#### 12.4.4.1 fasync：以进程为中心的“信号唤醒模型”
...
#### 12.4.4.2 io_uring：以一次 io_uring ctx 为中心的“操作队列模型”
...
#### 12.4.4.3 组合方式：用 io_uring 作为“事件循环内核”，fasync 作为“某些设备的触发信号源”
...
### 12.4.5 可视化：应用如何在同一个框架里用 io_uring + fasync + poll
...
### 12.4.6 示例代码：使用 io_uring 轮询字符设备 fd（依赖驱动 `.poll`）
...
### 12.4.7 调试与验证：在 io_uring + fasync 混用场景下的注意点
...
### 12.4.8 小结：在现代异步 I/O 体系中，fasync 仍然是“简单、可组合”的一环
...
### 12.5.1 引入：从“我有一个设备”到“我该选哪一种通知机制”
...
### 12.5.2 数据结构视角：用关键维度构造“通知机制决策矩阵”
...
#### 第二步：对于“数据面”，先把 `.poll` + waitqueue 写好
...
#### 第五步：判断是否需要 netlink/uevent/sysfs 作为“控制面补充”
...
### 12.5.6 示例代码：一个“混合通知策略”模板（伪代码级别）
...
#### 12.5.6.2 用一个“配置结构”在内核注释中总结选型（示意）
...
### 12.5.8 小结：把 fasync 放回“通知机制家族”的工程化位置上
