# 第 2 章　能力与属性声明（`input_set_capability()` 与位图）
...
## 2.3　开发者视角（“四连问”：作用 / 场景 / 不写或写错的后果 / 驱动落点）

### 2.3.1 `input_set_capability()`
...
### 2.3.2 `__set_bit(bit, dev->propbit/evbit/...)`
...
### 2.3.3 `input_set_capability()` 使用说明
...
#### 2. input_set_capability 内核内部到底干了啥？
...
#### 3. 这些 capability 在哪里用？

##### 3.1 input_register_device 时匹配 handler
...
### 2.3.4 `input_set_capability()` 参数关系说明
...
#### 3. `input_set_capability` 一次可以设置多个bit类型吗？
...
#### 4. `input_set_capability(type, code)` 在底层到底做了什么？
...
#### 5. 运行时上报事件时，`type` 和 `code` 又如何配合？

##### 5.1 报告事件：`input_report_*` → `input_event`
...
#### 6. handler/驱动层面如何利用这对 `(type, code)`？

##### 6.1 handler 挂接时看 capability（只看位图）
...
#### 二、初始化/注册流程图：`input_set_capability()` 的位置
