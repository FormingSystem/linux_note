# 第5章 GPIO 与触发语义：电平、边沿、DTS 与“只来一次/一直来”为何出现
...
## 5.3 中断控制器的视角：它只管“怎么触发”，不管“你为什么要这样触发”
...
### 5.4.1 `interrupts = <...>` 说的是“中断控制器那一头要怎么触发”
...
### 5.4.2 `GPIO_ACTIVE_LOW` / `GPIO_ACTIVE_HIGH` 说的是“逻辑含义”
...
## 5.5 为什么驱动里还要再 `irq_set_irq_type()` 一次
...
### 5.8.4 示例二：threaded + IRQF_ONESHOT + printk（复现“卡死体感”）
...
### 5.8.5 示例三：threaded + trace_printk + 硬中断时间窗去抖（推荐）
