...
## **2.3 开发者视角：GPIO 控制器的 DTS 语法**

### **2.3.1 i.MX6ULL GPIO 控制器语法**
...
## **2.5 工程实例二：GPIO 控制电源（regulator 子系统）**

### **2.5.1 典型应用：3.3V 模块电源**
...
## **3.3 工程语法：pinctrl 节点结构与配置项**

### **3.3.1 i.MX6ULL 的 pinctrl 写法**
...
## **3.6 工程实例三：Wi-Fi 模块的引脚状态与上电时序**

### **3.6.1 RK3568 + AP6256 模块**
...
# **第 4 章：interrupt 子系统的工程语法与级联机制**

*(参考平台：NXP i.MX6ULL / Rockchip RK356x · Linux ≥ 6.1)*

------

## **4.1 主题引入：事件如何从脚到 CPU？**
...
## **4.2 数据结构视角：irqdomain 把不同控制器“装进同一地图”**
...
## **4.4 工程实例一：按键（gpio-keys）从 GPIO 域到 GIC**

### **4.4.1 i.MX6ULL 版本（GPIO1_IO05 为下降沿触发）**
...
## **4.6 工程实例三：I/O 扩展器（MCP23017 / PCA9539）级联中断**

扩展器自己是 **GPIO 控制器 + 中断控制器**，它的 **INT 引脚** 再接到主控的某个 GPIO。

### **4.6.1 i.MX6ULL 版本（PCA9539 举例）**
...
## **4.8 工程实例五：唤醒中断（Wakeup IRQ）与电气一致性**
...
## **5.3 工程实例一：Wi-Fi 模块完整组合（REG_ON + HOST_WAKE）**

### **5.3.1 i.MX6ULL 平台 DTS**
...
## **5.4 工程实例二：摄像头模块（PWDN + RESET + IRQ）**
...
# **第 7 章：调试与验证技巧——从引脚到中断的全链路追踪**

------

## **7.1 主题引入：驱动成功加载 ≠ 驱动真正工作**
...
## **7.3 数据结构视角：如何在运行时定位引脚信息**

### **7.3.1 设备树 → pinctrl 解析路径**
