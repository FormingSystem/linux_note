...
# 第 1 章：GPIO + pinctrl + interrupt 的设备树语法与应用
...
#### （1）GPIO 控制器描述结构：`struct gpio_chip`
...
#### （2）pinctrl 控制器描述结构：`struct pinctrl_desc`
...
## 1.3 开发者视角：设备树 Provider 与 Consumer 的关系
...
# 第 2 章：GPIO + pinctrl + interrupt 的组合应用与高级语法
...
## 2.2 数据结构视角：GPIO 与 pinctrl 的内部衔接

### 2.2.1 pinctrl state 的生命周期
...
### 2.3.4 pinctrl hog：自动固定 pinmux 状态
...
# 第 3 章：Linux 中断模型深度解析与设备树中的级联机制（第 3 部分）
...
## 3.2 数据结构视角：从硬件中断号到虚拟 IRQ

### 3.2.1 中断域 irqdomain：内核中的中断映射核心
...
### 3.2.2 GIC 中断模型与 `#interrupt-cells = 3`
...
#### 常见宏定义（来自 `<dt-bindings/interrupt-controller/arm-gic.h>`）
...
### 3.2.3 GPIO 控制器作为中断控制器：`#interrupt-cells = 2`
...
## 3.3 开发者视角：中断级联的设备树语法

### 3.3.1 GPIO → GIC 的级联定义
...
### 3.3.3 使用 `interrupts-extended` 同时声明多个中断源
