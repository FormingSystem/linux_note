...
## 9.2 ARM GIC 在层级结构中的角色（怎么放进去）

### 9.2.1 GIC 一定是根域（root irq_domain）
...
### 9.4.2 irq_domain_ops 在层级结构里的真实作用
...
### 9.5.3 第二层排查：看控制器驱动是否创建了 irq_domain
...
### 9.5.4 第三层排查：看 /proc/interrupts 的跳数
