...
### 2.2.2 从 DTB 到 `device_node`（解析阶段）
...
### 2.2.4 自动生成 `platform_device` 与填充 `resource[]`（调用链）
...
### 2.3.3 驱动 probe：从 device_node 到 resource
...
### 2.3.7 匹配表与 MODULE_DEVICE_TABLE
...
#### 2. struct of_device_id 的定义位置与字段
...
#### 1. `/proc` 与 `/sys` 的目录映射（从 DTS 到驱动绑定）
...
#### 2. 匹配与资源生成流程图（DTS → resource → probe）
...
#### 3. 模块自动装载与 probe 调用时序（含 modalias / alias）
