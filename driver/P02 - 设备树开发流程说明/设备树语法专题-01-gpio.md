...
### 1.3.3　`flags` 常用宏（来自 `<dt-bindings/gpio/gpio.h>`）
...
# 第2章　设备树中的 GPIO 中断与 gpio-keys 语法（Kernel 6.1+）
...
### 2.3.1　`interrupt-parent` 与 `interrupts`
...
### 2.6.2　结合 gpio-line-names 提升可读性
...
# 第3章　设备树中 GPIO 与 pinctrl 配合语法详解（Kernel 6.1+）
...
## 3.4　pinctrl 语法细化与可选属性

### 3.4.1　`pinctrl-names` 与 `pinctrl-0` … `pinctrl-N`
...
### 3.4.2　`fsl,pins` / `rockchip,pins` 属性结构
...
## 3.5　电气配置属性详解（Pin Configuration）

### 3.5.1　标准属性（Linux pinconf 通用）
