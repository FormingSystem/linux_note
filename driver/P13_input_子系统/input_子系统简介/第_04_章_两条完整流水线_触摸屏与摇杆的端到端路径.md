...
## 4.1 场景总览：两条流水线的共性与差异

### 4.1.1 是什么：两条流水线的抽象模型
...
## 4.2 LCD 电容触摸屏流水线（I²C，多点，DIRECT）
...
### 4.2.4 驱动落点：I²C 线程化 IRQ + MT-B 成帧（高层骨架）
...
#### 4.2.4.3 中断线程中的“采样 + 成帧”轮廓（不展开控制器寄存器）
...
### 4.2.5 用户态视角：libinput / evtest / `EVIOCGABS` 怎么用
...
#### 4.2.5.2 用 `evtest` 验证 MT-B 事件流（定性）
...
#### 4.2.5.3 用 `demo_evdev_absinfo` 定量验收 ABS 元数据
...
#### 4.2.5.4 最小用户态“MT 帧打印器”（按帧解析 MT-B）
...
### 4.3.4 驱动骨架：轮询型虚拟摇杆（EV_ABS + fuzz/flat）
...
#### 4.3.4.4 模块 init/exit：注册输入设备并启动轮询
...
### 4.3.5 fuzz / flat / res 的配置策略与用户态验收

#### 4.3.5.1 fuzz：抖动抑制建议值
