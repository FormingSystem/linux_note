...
### 5.2.3 怎么实现：典型模式对照（坏写法 vs 好写法）

#### 5.2.3.1 坏写法：硬中断里 I²C + 上报
...
### 5.2.4 workqueue / 轮询场景下的“采集可睡、上报不睡”
...
### 5.3.1 是什么：从 `input_report_*()` 到用户态的“设备级总顺序”
...
## 5.4 帧一致性与 TOCTOU：data-ready 检查 + 有限重读
...
### 5.4.1 是什么：帧一致性与 TOCTOU 在 Input 驱动中的含义

#### 5.4.1.1 帧一致性（frame consistency）
...
### 5.4.3 怎么实现（1）：帧号模式（frame_id 前后一致）
...
### 5.4.4 怎么实现（2）：data-ready + 有限重读模式
...
#### 5.4.6.3 反模式 3：在两个上下文中交替读写同一 FIFO
...
### 5.4.7 示例代码：I²C 触摸屏的 data-ready + 有限重读
...
## 5.5 关机 / 休眠顺序：`disable_irq_sync()` 与电源状态切换
...
### 5.5.1 引入：为什么“关机/休眠顺序”是 Input 驱动的高危区
...
### 5.5.2 数据结构视角：device / input_dev / IRQ / wakeup 的关系
...
### 5.5.4 用户 / 平台视角：系统关机 / 休眠中的 Input 驱动角色
...
### 5.5.5 可视化：标准顺序的流程图与时序图

#### 5.5.5.1 `.suspend()` / `.resume()` 推荐流程（设备内部视角）
...
#### 5.5.5.2 线程化 IRQ 与 `.suspend()` 的交互时序
...
### 5.5.6 示例代码：suspend/resume/shutdown 模板（含 devres/非 devres 对比）
...
#### 5.5.6.2 suspend / resume（非 devres/普通风格）
...
#### 5.5.6.3 devres 风格：devm_request_threaded_irq() 与 PM 的关系
...
### 5.5.8 小结：把 `disable_irq_sync()` 固定在模板里的位置
...
## 5.6 背压与丢帧：EV_SYN/SYN_DROPPED 与用户态自愈重同步
...
### 5.6.1 是什么：Input 里的“背压”和“丢帧提示”

#### 5.6.1.1 背压（backpressure）在这里的具体含义
...
#### 5.6.1.2 丢帧（drop）与 `EV_SYN / SYN_DROPPED`
...
### 5.6.5 用户态代码模板：处理 `SYN_DROPPED` 的最小循环
...
#### 5.6.5.3 主循环：检测 SYN_DROPPED 并调用重同步
...
### 5.6.6 对比 / 避坑：错误处理方式与后果

#### 5.6.6.1 反模式 1：完全忽略 `SYN_DROPPED`
...
#### 5.6.6.2 反模式 2：看到 `SYN_DROPPED` 后立即退出程序
