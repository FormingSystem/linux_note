...
## 12.4 带超时的事务执行（`completion`/`timeout` + 可选 `hrtimer` 看门狗）
...
## 12.5 NFC/按键类去抖（IRQ → `delayed_work`，DTS 接入）
