...
## 1.2　设计哲学：错误即对象（Error as Object）
...
## 2.1　错误值体系：MAX_ERRNO 与 errno 映射
...
## 3.6　PTR_ERR_OR_ZERO()：错误码与成功码的统一传播
...
## 4.1　“单返回通道”思想：函数的语义统一化

### 4.1.1　背景问题：传统返回语义的碎片化
...
# 第5章　错误指针机制在 driver core 中的传播路径
...
## 5.2　driver_probe_device() 的错误传播链
...
## 5.5　device_add() / device_register() 中的错误传播

### 5.5.1　device_register()
...
## 6.3　错误指针机制在 devres 中的意义

### 6.3.1　devm 系列接口的设计标准
...
### 8.2.1　实验模块：测试 ERR_PTR 与 IS_ERR
...
## 9.4　Unified Error API 提案（错误接口统一化）
...
### 9.5.2　从“错误即对象”到“错误即值（Error as Value）”
...
## B.3　`drivers/base/driver.c`（驱动核心）
...
## B.4　`drivers/base/devres.c`（资源管理）
...
## B.5　`drivers/gpio/gpiolib.c`（子系统示例）
...
## B.6　`include/linux/devm-helpers.h`（devres 协作接口）
