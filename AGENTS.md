...
类型只能使用：`feat`、`fix`、`refactor`、`perf`、`security`、`content`、`docs`、`test`、`build`、`ci`、`release`、`revert`、`chore`。`(<project>/<module>)` 整体可选，不设固定项目或语言列表；填写时按实际修改自由选择一到两级范围，中文和英文都合法，只禁止会破坏语法的空白、括号、冒号和额外斜杠。当前维护者习惯使用英文范围，但不得把个人习惯写成钩子的语言限制。结果说明必须包含中文。每次提交均须在标题后空一行，再填写至少一条 `- 描述` 明细；每条独占一行，说明实际变化、依据或已完成的验证，不得只重复标题、保留占位词或编造验证结果。
