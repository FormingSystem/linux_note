...
- `atlas/maps/knowledge_review_map.md` 是面向读者的唯一专题与章节评审状态台账，使用 🔴 **AI 创建**、🟡 **人工评审中**、🟢 **评审完成** 三态；新内容默认 🔴，只能依据开发者的实际人工阅读进度升级。该状态与 Front Matter 维护阶段相互独立，不得复制到 `README.md`、专题元数据、路线或索引中形成多份台账。
...
- 新增、移动、拆分、合并或删除知识正文、工程方法、平台记录、实验、源码研究、参考或出版物时，还必须同步 `atlas/maps/knowledge_review_map.md` 的对应节点和链接；新节点一律从 🔴 **AI 创建** 开始。章节已进入人工阅读时可以在章节下维护 🟡 小节评审点，未得到开发者明确确认不得把节点改为 🟢。
