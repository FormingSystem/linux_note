...
- 面向 C/C++ 开发者，讲解示例、教学模型和配套测试默认使用 C/C++；非必要不使用 Python。选择其他语言必须有具体的任务必要性，不能只因为作者编写方便。本轮已引入的 Python 教学示例也须按此要求逐项复核。
- Linux 教学正文按连续教材组织，落实《C Primer Plus》《C++ Primer Plus》所代表的循序渐进、完整实例、耐心讲解、动手观察、渐进练习与章末回顾；使用原创表达。已声明的先修知识用简短回顾唤起，不每章从头定义；参考页保留查询用途。先完成有代表性的正文样章，再按学习依赖扩展，不能只调整提纲和语气。全量批次见 `atlas/roadmaps/linux_textbook_refactor.md`。
- 零基础不止是解释缩写。每个模块、分类、平台、架构、产品、实现家族和代际都要先说明它是什么、为何出现、位于哪一层、与谁相连、和相邻对象按什么依据区分；不能把作者用于组织材料的最终分类当作读者前置知识。先独立建立第一套实现，再由新约束引出后续实现和同轴比较。
...
类型只能使用：`feat`、`fix`、`refactor`、`perf`、`security`、`content`、`docs`、`test`、`build`、`ci`、`release`、`revert`、`chore`。`(<project>/<module>)` 整体可选，不设固定项目或语言列表；填写时按实际修改自由选择一到两级范围，中文和英文都合法，只禁止会破坏语法的空白、括号、冒号和额外斜杠。当前维护者习惯使用英文范围，但不得把个人习惯写成钩子的语言限制。结果说明必须包含中文。每次提交均须在标题后空一行，再填写至少一条 `- 描述` 明细；每条独占一行，以修改总结说明解决的问题、关键改动、改后结果或已完成的验证；不得用文件路径清单、文件数量或增删行数代替总结，不得只重复标题、保留占位词或编造验证结果。
