...
- 长期书籍与文章通过 `publications/manifests` 编排；按月冻结的单专题聚焦电子书通过 `markbook/manifests` 编排。正文保留统一阅读序号，派生出版物不得成为第二份权威正文。
...
- 一个专题只维护一份 manifest。生成器从 manifest 展开真实目录，按“稳定机制与应用 → 版本化源码导航 → 唯一实现讲解 → 实验与交叉边界”等阅读职责编卷；不得为了出版把正文复制到另一套可编辑 Markdown 中。
- 常规月刊按 `Asia/Shanghai` 自然月编号为 `YYYY.MM`，发布日期固定为当月 1 日。统一月度任务只在每月 1 日运行一次，并一次处理全部启用的专题 manifest。
- GitHub Actions 月度任务只允许读取仓库并试生成、验证刊物，不得拥有 `contents: write`，不得在远端 runner 中执行 `git commit` 或 `git push`。正式发布必须从维护者本地已同步且干净的 `master` 开始：先确认 `master...origin/master` 为 `0 0`，再生成、验证、形成本地发布提交，最后普通 push。push 被拒绝时保留本地提交并先处理远端变化，禁止强推；不得通过远端自动提交制造本地尚不存在的 `master` 历史。
- 同一专题同一月份只能发布一个版本。目标版本目录或目录台账已经存在时，生成器必须在任何写入之前拒绝执行；重复调度、手工重跑和失败后的不明状态都不得静默覆盖。
- 首次建刊且尚无任何历史版本时，可以显式使用 `--initial` 在非 1 日生成当月首版；它不适用于已有刊物。只有开发者明确提出“覆盖更新”时才能使用 `--overwrite`，普通的“更新”“重新生成”或修正文案不能推定为覆盖授权。
- 已发布月份默认不可变。常规内容修订进入下一月版本；获准覆盖时仍必须重新生成完整来源台账，不得只替换局部 HTML 后伪装成一致快照。
- 每期至少固化 manifest 哈希、生成器版本、仓库 `HEAD`、源文件逐项 SHA-256、源文件当时的工作树状态、生成时间和产物哈希。工作树存在未提交修改时可以形成明确标记的快照，但不得把 `HEAD` 误写成这些修改的来源提交。
- 每期应提供专业封面、版权与版本说明、分卷目录、章节与上下篇导航、全文检索、阅读进度、源码路径回溯、打印样式和离线 Mermaid 渲染。代码块统一采用 VS Code Light 视觉语义并在构建期完成语法高亮，不得为了迁就正文配色改成深色代码块或依赖在线高亮服务。拥挤图必须使用独立大图查看器承载自适应、缩放、拖动、适屏和退出恢复，不得复用正文容器的缩放状态；正文图保留自然尺寸和局部横向滚动，打印时再收敛到纸张宽度。不得伪造出版社、ISBN、人工评审状态或源码版本。
- 书内链接优先跳到本期已收录章节的精确标题；未收录材料回链仓库权威路径。评审状态仍只由 `atlas/maps/knowledge_review_map.md` 管理，生成成功不等于内容已通过人工评审。
- 版本目录、目录台账和共享运行时不得包含本机盘符、UNC、用户名、IP、访问令牌或构建机路径。共享运行时必须版本化，旧刊不得引用会被原地替换的未版本资源。
- 修改 manifest、生成器、模板或月度任务后，至少执行 MarkBook 单元测试、目标刊物生成或验证、同月重复发布拒绝测试、链接检查和 `git diff --check`。生成器输出必须与源文件分开审查，不能用生成产物掩盖权威正文的意外改动。
