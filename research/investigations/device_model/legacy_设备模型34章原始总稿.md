...
### 1.2.1\_统一抽象\_struct\_device\_与\_struct\_device\_driver
...
## 2.6\_可视化\_设备模型中的\_kobject\_继承关系
...
### 3.3.2\_struct\_subsys\_private\_内部组成
...
# 第4章\_device\_与\_device\_driver\_的匹配与绑定过程
...
### 4.5.4\_of\_driver\_match\_device()
...
# 第5章\_of\_device\_id\_与\_of\_match\_table\_匹配机制详解
...
### 5.3.3\_MODULE\_DEVICE\_TABLE\_宏展开
...
# 第6章\_platform\_device\_与\_platform\_driver\_框架
...
### 6.3.1\_struct\_platform\_device
...
### 6.3.2\_struct\_platform\_driver
...
### 7.3.2\_struct\_device\_与\_class\_的关系
...
# 第8章\_device\_attribute\_与驱动属性文件机制
...
### 8.3.1\_struct\_device\_attribute
...
### 8.4.4\_批量属性文件\_attribute\_group
...
## 8.7\_示例\_在\_platform\_驱动中添加属性文件
...
### 9.3.4\_struct\_device\_与\_devres\_链表
...
### 9.4.4\_devm\_ioremap\_resource()\_示例
...
### 11.3.1\_struct\_dev\_pm\_info(设备电源状态信息)
...
### 11.3.2\_struct\_dev\_pm\_ops(驱动电源回调)
...
### 11.4.2\_系统级挂起/恢复(suspend/resume)
...
# 第12章\_sysfs\_与\_kobject\_内核对象模型
...
## 12.6\_可视化\_kobject\_与\_sysfs\_映射关系
...
# 第13章\_uevent\_与\_udev\_内核事件通知机制
...
### 13.3.1\_struct\_kobj\_uevent\_env
...
### 13.3.2\_struct\_kset\_uevent\_ops
...
### 13.5.3\_udev\_与\_modalias\_自动加载
...
## 13.6\_可视化\_uevent\_与\_udev\_协作流程
...
# 第14章\_device\_与\_subsystem\_层级管理机制
...
### 14.3.1\_struct\_device\_层级节点的核心
...
### 14.3.2\_struct\_bus\_type\_总线层
...
### 14.4.5\_subsystem\_register()\_机制
...
# 第15章\_设备模型与模块加载\_从\_insmod\_到\_probe\_的全流程
...
## 15.4\_驱动注册阶段\_platform\_driver\_register()
...
## 15.5\_driver\_register()\_驱动正式接入系统
...
## 15.6\_bus\_add\_driver()\_驱动与总线绑定
...
## 15.7\_device\_attach()\_遍历设备匹配驱动
...
## 15.8\_设备匹配阶段\_driver\_match\_device()
...
## 15.9\_设备绑定阶段\_driver\_probe\_device()
...
# 第16章\_driver\_core\_设备匹配机制详解\_from\_of\_match\_device()\_到\_of\_match\_node()
...
## 16.3\_第一层入口\_driver\_match\_device()
...
## 16.5\_第三层\_of\_driver\_match\_device()
...
### 16.7.1\_关键调用\_of\_device\_is\_compatible()
...
# 第17章\_设备树匹配的高级机制与多层继承(compatible\_列表与\_fallback\_匹配)
...
## 17.4\_of\_match\_node()\_的多值匹配逻辑
...
## 17.6\_使用\_of\_device\_get\_match\_data()\_提取匹配数据
...
## 17.7\_可视化\_多层\_compatible\_匹配逻辑
...
# 第18章\_从\_device\_node\_到\_platform\_device\_设备树节点在设备模型中的注册过程
...
## 18.4\_of\_platform\_populate()\_设备树扫描入口
...
## 18.5\_of\_platform\_bus\_create()
...
## 18.6\_of\_device\_add()\_从节点到\_platform\_device
...
## 18.7\_of\_platform\_device\_create\_pdata()
...
## 18.8\_platform\_device\_register\_full()
...
## 18.13\_of\_node\_与\_device\_的绑定机制
...
## 18.14\_of\_alias\_get\_id()\_别名\_ID\_的生成
...
# 第19章\_platform\_device\_与\_platform\_driver\_的绑定过程(device\_attach\_与\_probe\_调用)
...
## 19.4\_driver\_attach()\_驱动遍历设备的起点
...
## 19.5\_driver\_attach()\_匹配与\_probe\_调用调度点
...
## 19.6\_device\_attach()\_驱动与设备绑定入口
...
## 19.7\_driver\_probe\_device()\_执行真正的\_probe\_调用
...
## 19.8\_really\_probe()\_绑定与错误回滚的核心
...
## 19.9\_device\_bind\_driver()\_建立\_sysfs\_双向链接
...
# 第20章\_设备模型中的引用计数与生命周期管理(get\_device\_/\_put\_device\_/\_kref\_机制)
...
## 20.3\_数据结构视角\_kref\_与\_device

### 20.3.1\_struct\_kref(核心计数结构)
...
### 20.3.2\_struct\_device\_中的引用成员
...
## 20.4\_device\_initialize()\_初始化阶段
...
## 20.5\_get\_device()\_与\_put\_device()
...
## 20.9\_引用计数在\_driver\_core\_各阶段的使用位置
...
# 第21章\_设备模型中的资源管理与\_devm\_*\_接口机制
...
## 21.4\_devm\_kzalloc()\_最常用的资源申请接口
...
## 21.6\_devres\_release\_all()\_自动释放机制
...
## 21.9\_实际示例\_带\_GPIO\_和\_IRQ\_的\_LED\_驱动
...
# 第22章\_device\_link\_与电源依赖关系(Power\_Domain\_Dependencies)
...
## 22.3\_核心数据结构\_struct\_device\_link
...
## 22.4\_device\_link\_add()\_建立设备依赖关系
...
## 22.7\_device\_link\_del()\_删除依赖关系
...
## 22.8\_dev\_pm\_domain\_attach\_by\_id()\_自动建立\_link\_示例
...
## 22.9\_电源域\_suspend/resume\_顺序控制
...
## 22.10\_可视化\_suspend/resume\_调用依赖顺序
...
## 22.13\_device\_link\_的通用用途(除电源外)
...
# 第23章\_bus\_type\_与\_class\_的深入实现机制(从\_bus\_register\_到\_sysfs\_呈现)
...
## 23.3\_struct\_bus\_type\_总线类型核心结构
...
## 23.5\_struct\_class\_设备功能分类核心结构
...
## 23.6\_class\_register()\_注册功能类入口
...
## 23.7\_subsys\_system\_register()\_核心封装函数
...
## 23.8\_kset\_create\_and\_add()\_创建集合容器
...
## 23.9\_可视化\_bus/class/kobject\_层次关系
...
## 23.11\_device\_create()\_class\_层生成节点
...
## 23.12\_示例\_Platform\_总线与\_LED\_类
...
## 23.13\_driver\_core\_中\_bus/class\_的数据关系
...
# 第24章\_driver\_core\_与模块系统的交互机制(THIS\_MODULE\_与引用保护)
...
## 24.8\_remove()\_调用链中的\_module\_put()
...
# 第25章\_热插拔与设备动态创建机制(Hotplug\_&\_Dynamic\_Device)
...
# 第26章\_device\_create()\_与\_sysfs\_属性自动生成机制
...
## 26.4\_device\_add()\_与\_sysfs\_目录生成
...
## 26.6\_可视化\_device\_create()\_到节点生成流程
...
## 26.7\_DEVICE\_ATTR()\_宏\_创建\_sysfs\_属性文件
...
## 26.12\_驱动中\_device\_create()\_与\_cdev\_create()\_的区别
...
# 第27章\_sysfs\_属性机制的底层原理与实现(kobj\_attribute\_与属性访问路径)
...
## 27.4\_struct\_attribute(属性基础单元)
...
## 27.5\_struct\_kobj\_attribute(属性\_+\_操作)
...
## 27.6\_struct\_sysfs\_ops\_属性访问调度表
...
## 27.7\_device\_attr\_show()/store()\_调用路径
...
## 27.8\_kobj\_type\_与\_sysfs\_ops\_绑定
...
## 27.9\_sysfs\_文件创建函数\_sysfs\_create\_file()
...
## 27.10\_kobject\_create\_and\_add()\_对象层注册
...
## 27.11\_device\_create\_file()\_内部关系复盘
...
## 27.13\_sysfs\_emit()\_与\_sprintf()\_的区别
...
# 第28章\_class\_attribute\_与\_driver\_attribute\_的实现与差异
...
## 28.3\_struct\_class\_attribute
...
## 28.4\_struct\_driver\_attribute
...
## 28.5\_注册接口\_class\_create\_file()\_与\_driver\_create\_file()
...
### 28.6.1\_1️⃣\_CLASS\_ATTR\_RW(name)
...
### 28.6.2\_2️⃣\_DRIVER\_ATTR\_RW(name)
...
# 第29章\_bus\_attribute\_与子系统属性机制(总线级属性注册)

## 29.1\_主题引入\_总线级属性的角色
...
## 29.2\_struct\_bus\_attribute\_定义与成员说明
...
## 29.3\_总线属性注册函数\_bus\_create\_file()
...
## 29.5\_bus\_attribute\_宏定义(简化声明)
...
## 29.7\_示例\_为\_platform\_总线添加属性文件
...
## 29.8\_bus\_属性的\_show()/store()\_调用流程
...
## 29.9\_subsys\_private\_与\_bus\_attribute\_的关系
...
## 29.10\_subsys\_system\_register()\_与\_bus\_register()\_的关系
...
## 29.13\_小结\_bus\_attribute\_的语义与应用场景
...
# 第30章\_设备模型与模块依赖\_modalias\_与\_modprobe\_自动加载机制
...
## 30.5\_kobject\_uevent()\_与环境变量添加
...
## 30.7\_MODULE\_DEVICE\_TABLE\_宏的作用
...
## 31.4\_driver\_unregister()\_与\_bus\_remove\_driver()

### 31.4.1\_源码节选(drivers/base/driver.c)
...
## 31.6\_device\_release\_driver\_internal()\_解绑核心逻辑
...
## 31.11\_引用计数与\_kobject\_put()\_关系
...
# 第32章\_多总线协作与复合设备机制(Composite\_/\_MFD\_/\_Component\_框架)
...
## 32.2\_MFD\_框架(Multi-Function\_Device)
...
## 32.3\_mfd\_add\_devices()\_的内部实现
...
## 32.5\_Component\_框架示例\_显示子系统(DRM)
...
## 32.6\_Composite\_框架(复合\_USB\_设备)
...
# 第33章\_ACPI/OF\_抽象与\_fwnode\_handle\_框架
...
## 33.1\_主题引入\_统一\_ACPI\_与\_Device\_Tree\_的抽象需求
...
### 33.2.1\_定义(include/linux/fwnode.h)
...
## 33.3\_struct\_fwnode\_operations
...
## 33.5\_fwnode\_handle\_与\_device\_的关系
...
## 33.8\_fwnode\_get\_named\_gpiod()\_示例
...
## 33.9\_与\_Software\_Node\_的扩展关系
