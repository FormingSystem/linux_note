...
1. 先从 `rcupdate.h` 阅读 `rcu_assign_pointer()`、`rcu_dereference()` 和读侧封装，并标出功能语句与 Sparse/Lockdep 检查语句；`rcu_check_sparse()` 的 `__CHECKER__` 分支见[静态类型桥接实现](../source_explanations/P01_Linux_6.12_RCU_公共接口与检查机制源码详解.md#1.3.3_rcu_check_sparse静态类型桥接)，其他公共宏体从[接口与源码索引](../source_explanations/P01_Linux_6.12_RCU_公共接口与检查机制源码详解.md#1.2_接口与源码索引)进入。
2. 再沿[RCU Lockdep适配模块源码概念导读](P05_Linux_6.12_RCU_Lockdep适配模块源码概念导读.md#5.1_模块问题与实现所有权)分清通用 Lockdep 与 RCU 实例的所有权，再读 `update.c` 的四个 maps、静态 key、等待类型、`debug_lockdep_rcu_enabled()` 和 `rcu_read_lock_held()`；具体实现见[RCU Lockdep适配层源码实现](../source_explanations/P04_Linux_6.12_RCU_Lockdep适配层源码实现.md#4.2_源码符号覆盖账本)，公共告警宏体见 [`RCU_LOCKDEP_WARN()` 检查适配层](../source_explanations/P01_Linux_6.12_RCU_公共接口与检查机制源码详解.md#1.6_RCU_LOCKDEP_WARN检查适配层)。
...
7. 沿 `call_rcu()` 进入回调排队，再阅读 `rcu_segcblist` 的分段模型与 `rcu_do_batch()`；callback 真正执行时怎样登记 `rcu_callback_map`，见[callback 上下文标记](../source_explanations/P04_Linux_6.12_RCU_Lockdep适配层源码实现.md#4.7_rcu_callback_map怎样标记延迟动作上下文)。
