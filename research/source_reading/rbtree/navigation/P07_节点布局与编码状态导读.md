...
树的根槽和孩子槽保存的是嵌入成员地址。业务代码读到某个槽后，必须已经通过相应同步与寿命协议保证对象可访问，再用已知的外围类型和成员名还原业务对象。一个任务可分别以 by_deadline、by_id 两个成员进入独立索引；每个成员占据不同地址，取回对象时要配上实际搜索的那个索引成员。

源码顺序是 [rb_entry 转发](../source_explanations/include/linux/rbtree.h.md#1.11_父地址与业务地址的两种还原) → [container_of 的类型检查与偏移计算](../source_explanations/include/linux/container_of.h.md#1.1_一次还原中的求值与类型检查)。这一步不读取树链接，也不产生共享状态写入；T0、T2、T3 都可能使用它，但 T4 释放后的旧地址不能再使用。成员类型相同并不表示它们可互换，编译检查不识别同类型成员的身份错误。

需要保持 const 时，再读[只读入口](../source_explanations/include/linux/container_of.h.md#1.2_只读限定由哪个入口保留)。const、成员身份与寿命是三个独立条件：只读类型不阻止其他路径释放对象；取得引用也不修正传错成员名；普通宏丢失 const 不等于允许写只读对象。用[P09 双成员实验](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P09_Linux_6.12_内核_rbtree_嵌入式节点与使用者接口.md#%281%29_两个嵌入成员还原同一个任务)观察两个正确偏移和结构体复制，再按实际业务决定比较规则。

[kref 的回调地址场景](../../../../knowledge/linux/object_lifetime/kref/P02_源码入口与结构定义.md#2.9_container_of_是理解_kref_的关键)复用同一通用宏：工作回调从 work 成员还原请求，release 从非首成员 ref 还原同一请求。这里没有新增树状态或所有者查询机制；对象类型和成员身份仍由调用方负责。
