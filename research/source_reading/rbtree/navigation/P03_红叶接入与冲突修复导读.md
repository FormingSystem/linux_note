...
[P26 插入单元](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P26_Linux红叶接入与插入修复.md#26.2_rbtree_插入前半段_搜索落点与_rb_link_node%28%29)从查找变为修改：同一比较过程走到 NULL 后，不再只报告不存在，而是要把新对象写入那个空槽。接入红叶不增加路径黑数，却可能与红父冲突。本篇追踪这些责任在固定 Linux 文件中如何交接；红黑理论沿用 P07，本篇不重新推导整套平衡树。
...
[完整模块](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P26_Linux红叶接入与插入修复.md#26.3.15_在内核模块中观察五组插入)分别提交 LL、LR、RR、RL 三键输入，再增加叔红输入，输出根、父键和颜色。先预测根是否变化、红冲突是否靠染色上推，再读日志。该模块的节点与根只在初始化的一个私有函数内存活，没有其他读者，所以无须为这个封闭观察增加共享树锁；这不是省略生产代码同步责任的许可。

这些函数不含内存申请或锁获取失败分支。它们的危险边界来自前提：非法成员、坏父链、排序错误、多个写者和悬空对象。观察程序不能验证这些任意错误输入；实际依赖的编译和运行范围以教材及工作记录为准。

插入结束以后，取消对象会移除黑贡献，而不只是增加红冲突；下一步沿[对象摘除与缺黑修复](P04_对象摘除与缺黑修复导读.md#4.1_谁拥有地址和颜色)追踪同一组根槽与父色地址。Case 3 删除回调也有中间态，但颜色和游标前提要按删除流程重新核对。

返回[总索引](P01_Linux_6.12_rbtree源码阅读索引.md#1.2_按问题选择源码入口)或[大纲](../大纲.md#1.1_从查询承诺进入实现)。


读完 I4/I5 后，[普通旋转对照](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P29_普通旋转与Linux修复的完成边界.md#29.3_相同形状不等于相同中间状态)解释为何局部形状已改变而完整父链尚未闭合；本模块仍负责固定函数怎样协作。
