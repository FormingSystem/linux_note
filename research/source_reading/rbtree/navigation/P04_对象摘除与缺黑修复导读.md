...
成员标记还需与根入口区分：[P08 根值实验](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P08_Linux_6.12_内核_rbtree_基础结构与工程模型.md#2%29_观察根值复制与对象存活)说明清根或复制根并不遍历节点；[三个空状态宏](../source_explanations/include/linux/rbtree.h.md#1.8_游离标记不等于成员搜索)分别读根槽、读节点标记、写节点标记。不能由某次根为空推断对象已回收，也不能用节点标记判断其属于哪一棵树。

D4 结束树操作后怎样继续，见[P09 入口与引用 S0～S5](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P09_Linux_6.12_内核_rbtree_嵌入式节点与使用者接口.md#%281%29_两个入口关闭之后谁还在使用对象)：关闭一棵树只撤去一个发现入口，其他索引与合法持有者仍可能保留对象。游离标记只有在旧路径不再需要字段且业务采用该协议时才可改写；读侧退出与引用归零分别按真实回收协议判断，不能由 rb_erase 返回统一推出。该 C 实验是串行所有权模型，不是 RCU 并发验证。
