...
固定头文件的 64 位或 BUILD_VDSO32_64 分支给出 16/10 槽数组上限，普通 32 位分支给出 32/21；dense 节点是另一布局，不应用这组数字。字段使用 unsigned long，名字中的 64 不覆盖 C 类型与构建配置。有效数据终点和节点类型继续限制实际使用槽数，本模块不将数组上限当作恒定分支数。具体字节复用和 NULL 分区沿[节点布局模块](P04_节点布局与范围分区.md#4.2_按问题读取布局)核对。
...
普通 Maple 查询与高级 mas 操作的区分见[固定文档](../../linux/Documentation/core-api/maple_tree.rst)：前者封装常用操作，后者将状态与同步管理更多交给调用方。两种初始化及实际方向/写入包装进一步沿[VMA 适配模块](P09_VMA游标与边界适配.md#9.2_从地址空间到局部游标)核对，包括宏 last=0 与函数 last=addr 的区别。游标不是索引永久拥有的共享节点；只改变游标位置不等于改变 VMA 集合。
...
查询契约之后进入[撤销范围模块](P10_撤销范围与临时索引.md#10.2_两棵树沿S0到S5分工)：主树按地址索引，临时待处理集合可按序号索引，同样的 ma_state 类型不能证明索引单位相同。
