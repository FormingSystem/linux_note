...
目前落地的模块导读只有[范围契约与查询入口](P02_范围契约与查询入口.md#2.1_同一地址可以提出不同问题)。它先区分半开区间、闭区间与查询任务，再连接节点容量、调用方游标和返回指针的期限。
...
这些小封装没有完成 Maple 的节点搜索或分裂。完整 Maple API 与状态的进一步阅读由[已有 P15](../../../../knowledge/linux/data_structures/红黑树_rb-tree/P15_Linux_6.12_Maple_Tree_源码结构与_API_分层.md#15.1_本章涉及的源码文件)组织；下面两个实现文件只承担本次已经逐句核对的 VMA 查询封装，不把尚未展开的内层算法列成已完成模块。
