...
[打开回调失败与交付边界](../source_explanations/fs/open.c.md#1.3_打开回调失败与交付边界)进一步核对S0/S1之间的FMODE_OPENED写入点：驱动回调失败须自行回滚；回调成功而VFS后续失败仍进入已打开文件清理。[P28完整文件持有实验](../../../../knowledge/linux/object_lifetime/kref/P28_文件实例与私有对象持有模板.md#28.1_计数单位是文件实例)将这一边界用于共享服务的候选份额，不重复本导读的框架实现。
