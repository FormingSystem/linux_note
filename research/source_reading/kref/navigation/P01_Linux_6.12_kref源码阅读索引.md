...
| 阅读问题 | 模块与唯一实现 |
| --- | --- |
| 最终验收怎样落到完整可核对程序 | [表模块与责任账本](P02_普通引用与归零回调导读.md#2.34_最终验收回到实际表模块) → P15收束与实际验证边界 |
| 框架和工程退出如何验收 | [对象体系与工程题](P08_device引用与资源退出导读.md#8.8_最终验收中的框架责任) → 既有框架证据、错误责任与退出依赖 |
| 交付、查找与RCU组合如何构造反例 | [组合窗口验收](P03_条件取得与查找窗口导读.md#3.15_组合场景验收的地址与归还窗口) → 成功/拒绝、重复撤下与回调退出 |
| 基础验收怎样用前提和反例作答 | [六题与固定实现](P02_普通引用与归零回调导读.md#2.33_基础验收的前提与反例) → 初始份额、普通增加、最后回调 |
| 旧用户与父子关系如何验收，实验记录怎样限定结论 | [状态与父子实验](P02_普通引用与归零回调导读.md#2.32_实验路线的状态与父子回收) → P14完整实验路线 |
| 多put一定告警吗，正快照是否保活 | [责任与检测边界](P07_引用错误的动态诊断导读.md#7.7_额外归还与快照的检测界限) → 既有减法、收敛与读取实现 |
| 最后put为何会增加一条隐含锁顺序 | [归还上下文与依赖证据](P07_引用错误的动态诊断导读.md#7.6_最后归还与锁依赖证据) → Lockdep既有唯一实现 |
| 存储有效为何仍会丢更新，KCSAN是否与KASAN共用配置 | [字段与采样证据](P07_引用错误的动态诊断导读.md#7.5_KCSAN配置互斥与字段证据) → 固定KCSAN配置及完整对照 |
