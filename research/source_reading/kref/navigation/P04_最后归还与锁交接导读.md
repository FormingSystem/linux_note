...
[P12退出上下文](../../../../knowledge/linux/object_lifetime/kref/P12_典型错误模式与调试线索.md#12.7_remove_上下文和辅助_API_引用之外还有收尾协议)先区分资源活动排空与外壳引用归还，再检查最后回调继承的上下文。普通[最后归还](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)直接调用应用release；[接锁回调](../source_explanations/include/linux/kref.h.md#1.8_归零时把锁交给回调)还要求应用恰好接管一次解锁。固定[锁内重查](../source_explanations/lib/refcount.c.md#1.3_取得锁后再次减少判断)不会为普通spin_lock隐含增加irqsave，也不会替回调安排可睡眠执行者。

若延迟最终清理，需要独占清理责任维持外壳和回调代码；普通RCU回调不因此成为可阻塞工作线程。正文E0～E5描述应用层closing、active与等待通道的设计模型，不是kref内部新字段。它沿既有管理者等待和锁交接完整模块解释条件，没有新增硬件在途实现或目标运行结论。

[完整Lockdep实验](P07_引用错误的动态诊断导读.md#7.6_最后归还与锁依赖证据)用三种归还位置说明最后回调继承已有锁上下文；普通kref_put回调中自行取锁，与kref_put_mutex的归零交锁契约分别核对，不能互换。
