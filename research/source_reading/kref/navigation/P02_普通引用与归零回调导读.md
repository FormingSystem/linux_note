...
从[等待总索引](../../waiting_notification/navigation/P01_Linux_6.12_等待与完成量源码总阅读索引.md#1.5_建议阅读顺序)进入[completion 模块](../../waiting_notification/navigation/P03_Linux_6.12_completion模块源码概念导读.md#3.2_状态所有权)，再查[等待消费](../../waiting_notification/source_explanations/kernel/sched/completion.c.md#1.4_do_wait_for_common等待与消费)。工作退出另经[工作队列总索引](../../workqueue/navigation/P01_Linux_6.12_工作队列源码总阅读索引.md#1.6_建议阅读顺序)到[同步取消](../../workqueue/source_explanations/P03_Linux_6.12_worker_flush与取消源码实现.md#3.5_cancel_work_sync撤销与等待)。这些状态彼此独立，普通 kref 函数仍只完成计数和回调，不复制异步子系统的实现。
...
[P31工程查询页](../../../../knowledge/linux/object_lifetime/kref/P31_接口契约与退出审查模板.md#31.1_把成功和失败分别写进注释)以已有完整程序说明取得、转交、分享和借用的中文契约，区分成功失败的责任。P13总表与二十项审查问题已按拥有型/非拥有型、普通/条件取得、分享/转交/借用分别限定，不要求所有对象采用同一释放顺序。

普通[最后归还](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)仍可能在worker内同步执行清理，因此release销毁该worker所在队列可能自等。管理者应先停止来源，再用既有[工作取消与等待](../../workqueue/navigation/P04_Linux_6.12_flush取消与生命周期模块源码概念导读.md#4.4_cancel调用链)处理对应退出；对象kref不自动保代码或资源。节点为空、pending采样和WARN只检查局部状态，不能替代排空或弱入口撤销。

本批没有新增Linux函数体、内核材料或运行模型。P20/P23/P24/P25/P27材料保持不变，沿既有验证记录使用；新验证仅是正文冷读、契约对照与入口检查，不产生目标并发或装卸结论。

## 2.29\_基础实验的返回值与回调位置

[P32完整基础模块](../../../../knowledge/linux/object_lifetime/kref/P32_基础引用与源码对照实验.md#32.1_先分清读哪份源码和运行哪个内核)用holders=1/2比较初始份额与新增份额。B0对应[初始化](../source_explanations/include/linux/kref.h.md#1.2_建立初始引用)，B1对应[普通取得](../source_explanations/include/linux/kref.h.md#1.3_为独立使用追加引用)，B2/B3分别核对[put的回调与返回](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)。非最后返回0，正常最后归还在回调返回后返回1；回收是否延期仍由回调决定。

两槽位在同一初始化线程顺序处理，不是两线程实验。打印位置特意把最后put后的观测放在局部返回值与对象外release_calls，kref_read只在独占且持有的初始化窗口使用。六条宿主协议和ARM前端通过，354头中342非生成源码与官方固定dfaf2136无差异；未目标链接装卸、真实日志或并发运行。

## 2.30\_错误轨迹与实际引用的证据层次

[P33责任错误实验](../../../../knowledge/linux/object_lifetime/kref/P33_缺失引用与未归还责任实验.md#33.1_先确定删除的是哪一份)复用P12的完整C影子账本，以原六条和五条扩展输入检查缺份额、未归还、正确转交、完成顺序、候选回滚与记录不完整。它没有执行Linux计数、排队、释放、线程调度或KASAN，不能把invalid_owner和leftover直接称为真实UAF和内存泄漏报告。

映射到实际模块时，[普通get](../source_explanations/include/linux/kref.h.md#1.3_为独立使用追加引用)只追加计数，[正常最后put](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)不识别业务拥有者；少get可能通过正常归零提前触发清理，不一定先触发refcount异常。queue接纳、代码退出和借用期限继续按P20/P24完整协议核对，不能从sleep延迟或无告警推出正确。

十一条宿主C轨迹通过，既有ownership_audit、工作模块及上游证据未改。未新执行ARM、目标装卸、真实故障注入、动态检查器或硬件。

## 2.31\_工作实验的责任和实际投递

[P35实验7](../../../../knowledge/linux/object_lifetime/kref/P35_工作交付与关闭窗口实验.md#35.1_候选份额不等于已经交付)沿一次实例的预留、接纳、拒绝、取消和完成，对照独立worker份额与管理者覆盖借用。固定[普通取得](../source_explanations/include/linux/kref.h.md#1.3_为独立使用追加引用)和[最后归还](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)没有读写work的pending或应用stopping，不能用对象保活代替实际排队与关闭排序。

work_ticket六路径、note_kref_work_modes十例和note_kref_owned_work九组重新通过，三个既有材料及上游函数未改。模型和队列/原子替身不证明目标调度；全新单次work的拒绝分支由夹具注入，不是内核自然拒绝实测。本批没有新ARM、装卸、真实并发或内存序结果。
[Generic KASAN诊断分支](P07_引用错误的动态诊断导读.md#7.3_从应用回收到非法读取)把普通最后put、应用kfree与之后的非法字段读取分开；引用回调不自动等于任意类型物理回收，告警还依赖检测配置和实际访问。
[字段更新实验](P07_引用错误的动态诊断导读.md#7.5_KCSAN配置互斥与字段证据)为每个接纳的work另取一份，部分投递失败先放行已接纳者再等待；get/put不让plain的读改写不可分割。
[误用实验](P07_引用错误的动态诊断导读.md#7.7_额外归还与快照的检测界限)保留实际存储观察零后减法，并区分无告警消费别人份额与数值异常；计数原语没有调用者拥有表。

## 2.32\_实验路线的状态与父子回收

[P14实验15和16](../../../../knowledge/linux/object_lifetime/kref/P14_源码阅读实验.md#14.8_状态和父子关系实验_生命周期不等于业务可用)分别复用[业务接纳](#2.27_业务状态与引用原语的分工)与[父子桥接](#2.24_父子桥接与非拥有节点)的既有模块。旧reader保活不代表取得活动token，child保存父地址仍须由桥接份额保护；父管理者退出不消费child用户的份额。原短操作同锁检查与访问的价值保留，再由锁外活动引出登记和排空，不把整个生命周期等同于一个dying字段。

本批重编执行state_ownership十条C路径及note_kref_parent八组宿主协议；模型不实现真实锁、通知或硬件，父子夹具使用顺序替身。未新增ARM构建、目标装卸或实际并发，P27/P30完整正文和唯一实现保留。实验记录分开预测、观察与未执行，不能把框架存在或前端通过写成内核诊断完成。

## 2.33\_基础验收的前提与反例

[P15基础六题](../../../../knowledge/linux/object_lifetime/kref/P15_最终验收标准.md#15.3_核心概念验收_生命周期_指针和引用)以P32完整模块检验初始份额交付、两槽责任、最后归还及container_of外壳。六组宿主协议重新编译通过；不新增目标装卸或底层内存序证据。普通get须同时有地址及正数前提，mutex不自动保证后者；put后能否再访问由另一独立保护决定，不由变量名或非最后返回值决定。

沿[普通追加唯一实现](../source_explanations/include/linux/kref.h.md#1.3_为独立使用追加引用)、[最后归还](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)与[条件取得导读](P03_条件取得与查找窗口导读.md#3.8_按地址期限和正计数审查查找)分别核对，不把验收判断改写成新的原语定义。P15场景及工程部分继续逐项审查。

[组合场景验收](P03_条件取得与查找窗口导读.md#3.15_组合场景验收的地址与归还窗口)以分享/转交/管理者借用检验责任单位，普通最后回调与异步代码退出分别审查；不把失败净结果误读成从未准备候选份额。

## 2.34\_最终验收回到实际表模块

[P15最终模板题](../../../../knowledge/linux/object_lifetime/kref/P15_最终验收标准.md#15.8_最终代码模板验收)以现有note_kref_table替代发布归属不明且重复remove无条件put的旧片段。责任账本逐步解释初始、表、reader份额以及重复撤下，六组宿主协议本批重编通过；未新增目标运行。问答和总图按地址、正数、借用、业务接纳及回收条件分支，不按锁名机械选择get，也不让借用worker代还份额。

基础与场景沿已有普通/条件取得及锁交接导读回到唯一实现，本段不另抄函数。P15作者审查收束，后续仍须审查专题大纲及相邻devres和生命周期集成材料；本次完成不等于全仓完成。
