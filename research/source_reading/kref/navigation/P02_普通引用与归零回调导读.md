...
同文后续改用条件取得，允许普通 put 在锁外执行，清理回调再取 mutex 摘除；因此查找锁内可能遇到地址尚有效而计数已零的对象，必须处理取得失败。RCU 示例则把计数成员的存储保留到相应宽限期。此处只定位各协议差异，不展开尚未整理的条件函数体；普通 get/put 仍精确回到[追加引用](../source_explanations/include/linux/kref.h.md#1.3_为独立使用追加引用)和[归零回调](../source_explanations/include/linux/kref.h.md#1.4_最后归还调用清理)的唯一讲解。
