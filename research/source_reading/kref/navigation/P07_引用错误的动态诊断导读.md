...
[P40完整C模型](../../../../knowledge/linux/object_lifetime/kref/P40_多归还与失效窗口实验.md#40.1_计数为正仍可能归还了别人的份额)以具名责任、总数、逻辑存储和入口分别重放五场景。固定引用函数三条宿主对照确认：计数2连续归还两次可以无告警地归零，即使应用把第二次视为重复消费；栈存储仍在、零后再减才观察异常收敛；读取快照不产生份额。既有[减法分支](../source_explanations/include/linux/refcount.h.md#1.3_旧值决定归零与异常分支)、[异常收敛](../source_explanations/lib/refcount.c.md#1.1_告警之前先收敛到饱和)和[快照读取](../source_explanations/include/linux/kref.h.md#1.5_读取快照不新增责任)保持唯一实现。

六组note_kref_table宿主协议重跑通过，夹具包含当前模块，拥有型表在锁内摘除再归还表份额；release断言只诊断，不能修复漏摘除。非拥有索引与RCU须按各自退出协议解释。本批没有新ARM构建、目标故障、实际UAF或诊断器报告，不把数值对照称为完整责任检测。

[最终工程验收](P08_device引用与资源退出导读.md#8.8_最终验收中的框架责任)把UAF、未释放、underflow和字段竞争分别追踪到责任、期限与覆盖证据；告警和未告警都不替代原始记录，READ_ONCE也不自动修复复合更新。
