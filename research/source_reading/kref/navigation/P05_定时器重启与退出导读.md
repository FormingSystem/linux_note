...
[P21单次请求](../../../../knowledge/linux/object_lifetime/kref/P21_定时器交付与同步关闭模板.md#21.1_先把排队状态与对象责任分开)在原借用模型旁增加另一应用协议：管理者初始份额保持到T5，T1只接受一次启动并为timer另取一份，T2回调或T4取消者恰有一方归还。request.ticket是应用票据，issued保存本对象已经用过的启动机会；两者均不是timer.entry或base.running_timer的镜像。

按T1先读[改期入口](../source_explanations/kernel/time/timer.c.md#1.1_改期不等于追加一次回调)，返回零不能作为普遍失败码；按T3/T4再读[最终关闭](../source_explanations/kernel/time/timer.c.md#1.2_等待执行与关闭重启)，function置空防止新启动生效，但并不消费应用提前get的份额。因此应用gate必须把closing检查、get和实际mod_timer排序在一起，关闭者在gate外等待。原模板del_timer_sync的固定[旧名包装](../source_explanations/include/linux/timer.h.md#1.2_旧名转到同步删除)仍保留用于解释，本例最终退出选择shutdown且不重新初始化。

七组宿主协议检查使用固定普通引用链与显式定时器/锁替身，覆盖取消、执行、重复启动/关闭和门控拒绝；不是内核等待或软中断验证。ARM前端通过，354头中342非生成源码无固定提交差异；未目标链接装卸、真实竞争和硬件验证。原四轨迹借用/改期模型与唯一源码实现不变，不将这份单次模块外推成任意周期或timer/work互启协议。
