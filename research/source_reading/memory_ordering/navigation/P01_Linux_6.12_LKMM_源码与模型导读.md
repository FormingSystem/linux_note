...
沿MP坏结果核对：取得flag之后读到旧buf，产生从旧buf读通向新buf写的fr；发布侧顺序再把新buf写连接到flag发布，flag读从该发布取值。必须把这些边合成为符合定义的传播路径，才能回到取得侧形成hb回边。1.9将它与取得后的正向边合起来说明矛盾；不能省掉中间组合，只画一个未标出处的箭头。
