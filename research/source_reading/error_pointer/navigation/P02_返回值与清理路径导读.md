...
对于 S4，`devres_release_all` 先检查设备链状态，再在锁保护下通过 `remove_nodes` 摘出记录，锁外由 `release_nodes` 逆序调用释放回调并释放记录。组记录也由相应算法处理，本文不把完整分组算法改写成虚假的“从头逐个删除”。资源拥有者必须保证没有不受控的并发使用者跨越销毁点。
