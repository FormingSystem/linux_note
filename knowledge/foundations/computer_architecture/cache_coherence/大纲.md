...
本专题已经覆盖 Snooping 与 Directory、MESIF/MOESI、Inclusive/Exclusive LLC、NUMA 一致性域和 CXL.cache。Store Buffer、传播和屏障的硬件抽象由[体系结构内存顺序专题](../memory_ordering/大纲.md)维护；Linux 的 ONCE、SMP 屏障与 release/acquire 契约由 [Linux 内存顺序专题](../../../linux/memory_ordering/大纲.md)维护。DMA 映射接口、RCU 和锁的调用方式也保留在各自专题，通过链接关联，避免把硬件协议与软件 API 再次混写。
