...
本例选择同步等待，因此 `record_slot_replace()` 与 `record_slot_destroy()` 必须运行在允许睡眠的上下文。完整的读者、同步/异步更新者和模块退出模式见 [RCU 通用 API 与最小使用闭环](../../../linux/synchronization/rcu/P03_RCU_通用API与最小使用闭环.md#3.1.2_完整同步实现)；`__rcu`、Sparse 与动态保护条件怎样分工，继续看 [RCU 类型语义、Sparse 与 Lockdep](../../../linux/synchronization/rcu/P26_RCU_类型语义_Sparse与Lockdep.md#26.1.3_正确的完整代码闭环)。
