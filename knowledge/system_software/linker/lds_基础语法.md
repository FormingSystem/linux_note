...
# 第2章\_段的排序\_过滤\_内存分配\_KEEP\_保留机制

## 2.1\_段的排序与过滤\_*(.text*)\_KEEP()\_SORT()

### 2.1.1\_*(.text)\_vs\_*(.text*)
...
## 2.2\_精确控制输入文件和段\_*(filename.o)\_和\_(EXCLUDE\_FILE)
...
## 2.5\_分段绑定内存区\_使用\_AT\_和\_>

### 2.5.1\_将.text\_放入\_ROM
...
## 4.4\_构造器与数组段.ctors,.init\_array,.fini\_array
...
## 4.9\_U-Boot\_2022.10\_u-boot.lds\_详解(结合上文的链接语法)
...
### 4.9.2\_DISCARD\_区段\_丢弃无用符号(释放空间)
...
### 4.9.5\_安全启动区域(ARMv7\_TrustZone\_专用)
...
### 4.9.8\_动态链接和符号段(用于\_relocation)
...
### 5.3.1\_Q1\_为什么会有多个.lds\_不会产生段冲突和重复定义
...
### 5.3.2\_Q2\_为什么生成多个\_bin\_镜像文件\_它们的区别是什么
...
### 5.3.3\_Q3\_这些镜像文件如何运行\_是并行\_单选\_依次运行
...
# 第6章\_.rel.dyn.bss.u\_boot\_list段与\_board\_init\_f()\_和\_SPL\_阶段.lds\_的协同机制

## 6.1\_board\_init\_f()\_怎样协同对.bss.rel.dyn.u\_boot\_list\_段配合使用
...
### 6.1.2\_.rel.dyn\_段\_重定位\_ELF\_动态符号
...
### 6.1.3\_.u\_boot\_list\_段\_命令/驱动自动注册机制
...
## 6.2\_SPL\_阶段.lds\_构造与特性说明

### 6.2.1\_SPL.lds\_结构更精简
...
### 6.2.2\_SPL\_不调用\_relocate\_code()\_而主\_U‑Boot\_在\_board\_init\_f()\_中会
...
# 第7章\_uboot的每个阶段的程序的运行逻辑

## 7.1\_U-Boot\_多阶段启动总览(重点聚焦职责与角色)
...
### 7.2.2\_阶段\_1\_TPL(Tiny\_Primary\_Loader)(可选)
...
### 7.2.3\_阶段\_2\_SPL(Secondary\_Program\_Loader)
...
### 7.2.4\_阶段\_3\_U-Boot\_Main(完整功能)
...
### 7.2.5\_阶段\_4\_FIT\_镜像\_/\_Linux\_Kernel
...
# 第8章\_u-boot启动阶段\_soc所用到的对应的硬件

## 8.1\_启动流程外设交互总览(SoC\_与外设映射)
...
## 8.2\_分阶段详细解析(含硬件交互与设备寄存器作用)

### 8.2.1\_BootROM\_阶段(芯片内置\_ROM\_代码)
...
## 8.4\_SPL\_阶段(初始化内存\_+\_加载\_u-boot.bin)
...
## 8.5\_U-Boot\_Main\_阶段(功能最全\_用户交互)
...
## 8.6\_Kernel\_阶段(通过\_bootm/booti\_启动)
