...
`grep` 只用于从已保存日志中快速定位几个证据标题，不能替代阅读完整 dmesg。受安全启动、模块签名、容器能力或只读系统限制时，`insmod` 可能根本未执行目标路径；必须先解决或记录这些工程约束，不能把加载失败解释成“无告警”。
...
最小顺序反转实验用单任务依次执行两个方向，验证的是组件链闭包，不制造真实卡死。报告应被还原为本次新边、历史反向路径和 current 账本；配置、`debug_locks` 和路径覆盖缺一不可。一次严重报告后应在干净启动继续实验。
