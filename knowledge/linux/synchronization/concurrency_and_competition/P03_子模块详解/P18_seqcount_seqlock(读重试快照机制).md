...
# 第18章\_seqcount\_seqlock(读重试快照机制)

## 18.1\_seqcount\_/\_seqlock(读重试快照机制)
...
* `seqcount_t` 接口参考：[18.5　`seqcount_t` 接口与使用规则（纯顺序计数器）](#18.1.10_seqcount_t_接口与使用规则(纯顺序计数器))
* `seqlock_t`  接口参考：[18.6　`seqlock_t` 接口与使用规则（带写方互斥封装）](#18.1.11_seqlock_t_接口与使用规则(带写方互斥封装))
...
#### (3)\_seqcount\_t\_与\_seqlock\_t\_的区别
...
### 18.1.7\_seqcount\_t\_写入路径与序列号管理机制-一写多读
...
### 18.1.8\_读路径实现细节与\_read\_seqcount\_retry()\_判断机制
...
### 18.1.9\_seqlock\_t\_封装机制与写保护行为-多写多读
...
### 18.1.10\_seqcount\_t\_接口与使用规则(纯顺序计数器)
...
### 18.1.11\_seqlock\_t\_接口与使用规则(带写方互斥封装)
...
##### 1)\_为何没有数据结构定义\_只有\_DEFINE\_SEQLOCK()\_宏接口
...
#### (8)\_〔可视化\_写\_to\_写\_串行\_+\_读无锁重试〕
...
### 18.1.12\_seqcount\_t\_与\_seqlock\_t\_的模型对比与选型建议
...
##### 1)\_表\_18-7-2\_seqcount\_t\_与\_seqlock\_t\_的内存屏障与执行顺序对比
...
| `rcu_read_lock()` | 普通 RCU 保护对象生命周期，读侧不应主动睡眠   | 若保护区必须跨睡眠，选 SRCU       |
...
| 是否有可并发释放的指针/复杂结构？ | 若是 → 评估 RCU、锁或引用计数；并非出现指针就必须用 RCU | ☐ |
...
| 热插拔状态监控 | `mutex` + RCU/kref，或 SRCU | 写侧串行化；跨睡眠访问需 SRCU 或独立引用 |
...
### 18.1.13\_seqcount\_t\_与\_RCU\_的边界与取舍
...
`seqcount_t` 与 `RCU（Read-Copy-Update）` 是 Linux 内核中两种**典型的读多写少同步模型**。
 ...
 若前者像是一种“读方校验机制”，那么后者则是“读侧生命周期保护 + 发布/取消发布 + 延迟回收”的系统级方案。
...
| ------------ | ----------------------- | ---------------------------------------- |
| 设计目标     | 检测读期间是否遇到写入  | 保护并发可达性与对象生命周期             |
| 写者语义     | 就地更新（in-place）    | 发布/取消发布；常见但非强制采用副本替换  |
| 读侧行为     | 重读直到稳定            | 在保护区内取得并使用可达对象             |
| 数据结构要求 | 读操作可安全重试        | 访问、更新与回收遵循相应 RCU 协议        |
| 写者冲突处理 | 写者必须串行化          | RCU 不提供写写互斥，调用者另行串行化      |
| 内存顺序语义 | 使用 seqcount API 契约  | 使用 `rcu_assign_pointer()` / `rcu_dereference()` 等发布—取得契约 |
...
##### 1)\_seqcount\_t\_一致性验证(version\_check)
...
old = rcu_replace_pointer(ptr, new,
                          lockdep_is_held(&update_lock));
mutex_unlock(&update_lock);
kfree_rcu(old, rcu);
```

> 指针发布 + 旧版本延迟销毁 + 读侧对象生命期保护；对象字段是否构成一致快照需另行设计。
...
- 写操作替换指针；
- 读侧无需检测，只需在保护区内读取；
- `kfree_rcu()`/`call_rcu()` 可在宽限期后异步回收；`synchronize_rcu()` 只等待 GP，本身不释放对象。
...
| 读过程   | 读取字段并准备校验                     | 用 `rcu_dereference()` 取得指针并在保护区内使用 |
...
- `RCU`：读方获得对象的临时生命周期保护；对象内部字段是否构成一致快照仍需不变设计、锁或 seqcount；
- 两者都依赖各自 API 的内存顺序契约，不能把 `rcu_read_lock/unlock()` 本身等价成通用 acquire/release 屏障。
...
| 维度       | `seqcount_t`            | `RCU`                                |
| ---------- | ----------------------- | ------------------------------------ |
| 读路径开销 | 较低，但可能重试        | 通常较低；具体状态更新取决于 RCU 配置 |
| 写路径开销 | 原地写并维护序号        | 取决于更新方式，通常还要承担 GP 回收 |
| 一致性     | 成功校验后得到一致快照  | 保证协议内可达性/生命周期；业务快照需另行设计 |
| 数据复制   | 无                      | 视更新算法而定，常见副本替换但并非强制 |
...
| 必须跨睡眠的读侧保护区     | `SRCU` 或 RCU 内取得 kref 后退出 | 普通 RCU 读侧不应主动睡眠 |
...
| 在普通 RCU 读侧区间主动调用可睡函数 | 可能长期阻塞 GP，并违反普通 RCU 调用规则 | 缩短读侧区间、在区间内取得独立引用，或改用 SRCU |
| 未区分 seqlock 与 RCU 的职责就混用 | 时序难验证         | 只有在分别保护字段快照和对象生命周期时才组合 |

RCU 的硬件基础、CPU／任务通知机制和宽限期实现统一参见[RCU 专题大纲](../../rcu/大纲.md)，本节只维护 seqcount/seqlock 与 RCU 的选型边界。
...
| 是否包含指针或可释放对象？ → 用 `RCU`                        | ☐        |
| 保护区是否必须跨睡眠？ → 使用 SRCU，或在普通 RCU 内取得独立引用后退出 | ☐ |
| 是否要求读者立即看到新版本？ → RCU 不提供这种业务时效保证    | ☐        |
