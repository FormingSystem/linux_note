...
* [16.8　自旋锁常用接口与语义说明](#16.10_自旋锁常用接口与语义说明)
* [16.9　自旋锁在多核缓存一致性（MESI）下的传播机制与延迟分析](#16.11_自旋锁在多核缓存一致性(MESI)下的传播机制与延迟分析)
* [16.10　per-CPU 自旋锁与 qspinlock 实际代码结构](#16.12_per-CPU_自旋锁与_qspinlock_实际代码结构)
* [16.11　`spin_*` 与 `raw_spin_*` 的区别及 RT 内核行为](#16.13_spin_*_与_raw_spin_*_的区别及_RT_内核行为)
...
关于 `spin_` 和 `raw_spin_*` 的区别，请查阅 [16.11　`spin_*` 与 `raw_spin_*` 的区别及 RT 内核行为](#16.13_spin_*_与_raw_spin_*_的区别及_RT_内核行为)。
...
### 16.10.8\_原始自旋锁接口(raw\_spinlock)
...
## 16.11\_自旋锁在多核缓存一致性(MESI)下的传播机制与延迟分析

------

### 16.11.1\_背景\_为何要分析缓存一致性
...
### 16.11.7\_Linux\_的优化机制\_Queued\_Spinlock\_(qspinlock)
...
#### (2)\_图\_16-4\_Queued\_Spinlock\_等待链(简化版)
...
## 16.12\_per-CPU\_自旋锁与\_qspinlock\_实际代码结构

------

### 16.12.1\_Linux\_自旋锁的两条实现路线
...
#### (2)\_struct\_raw\_spinlock(RT前后语义不同)
...
#### (3)\_struct\_qspinlock(现代SMP实现核心)
...
### 16.12.3\_锁的排队结构\_struct\_mcs\_spinlock
...
#### (2)\_图\_16-1\_MCS/qspinlock\_等待链流程
...
### 16.12.6\_per-CPU\_自旋锁(local\_lock)
...
### 16.12.7\_qspinlock\_与\_local\_lock\_对比
...
## 16.13\_spin\_*\_与\_raw\_spin\_*\_的区别及\_RT\_内核行为
...
### 16.13.2\_什么是\_RT\_内核(PREEMPT\_RT)
...
### 16.13.3\_RT\_内核对\_spinlock\_的结构性改造
...
### 16.13.6\_raw\_spinlock\_t\_的意义
...
## 16.14\_rwlock\_t\_自旋型读写锁的机制与使用边界
...
### 16.14.8\_〔对比\_rwlock\_t\_vs\_rw\_semaphore〕

#### (1)\_表\_16-12-3\_两类读写锁特性对照
