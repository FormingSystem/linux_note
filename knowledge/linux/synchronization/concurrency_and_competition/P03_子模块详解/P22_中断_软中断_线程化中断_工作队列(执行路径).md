---
id: knowledge.linux.synchronization.concurrency_and_competition.p03_子模块详解.p22_中断_软中断_线程化中断_工作队列_执行路径
title: "中断 软中断 线程化中断 工作队列(执行路径)"
kind: mechanism
status: evolving
domains:
  - linux
...
# 第22章\_中断\_软中断\_线程化中断\_工作队列(执行路径)

## 22.1\_中断\_/\_软中断\_/\_线程化中断\_/\_工作队列(执行路径)
...
| 软中断（SoftIRQ）     | 批量延后     | `raise_softirq()` | ❌        | 高     | 网络、默认路径中的 RCU core/回调处理 |
...
| `RCU_SOFTIRQ`     | 推进每 CPU RCU core 与默认回调处理 | RCU 子系统；启用 nocb offload 时部分工作由专用线程承担 |
...
#### (8)\_驱动示例(可编译版)\_Top\_Half\_调度\_SoftIRQ(tasklet)
...
#### (5)\_Top\_Half\_+\_Threaded\_Bottom\_Half\_混合模式
