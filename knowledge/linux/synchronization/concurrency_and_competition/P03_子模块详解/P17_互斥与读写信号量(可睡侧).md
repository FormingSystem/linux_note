...
* mutex 详情请参考：
  ...
  * [17.8　`mutex` 接口与使用规则](#17.10_mutex_接口与使用规则)；
  * [17.9　`mutex_lock()` 与 `mutex_lock_interruptible()` 的区别与使用边界](#17.11_mutex_lock()_与_mutex_lock_interruptible()_的区别与使用边界)；



* semaphore 详情请参考 ：

  * [17.10　`semaphore` 接口与使用规则（计数型可睡锁）](#17.12_semaphore_接口与使用规则(计数型可睡锁))；
  * [17.11　`rw_semaphore` 接口与使用规则（读写信号量）](#17.13_rw_semaphore_接口与使用规则(读写信号量))；
  * [17.12　`downgrade_write()`：写锁降级为读锁的机制与使用场景](#17.14_downgrade_write()_写锁降级为读锁的机制与使用场景)。
...
## 17.11\_mutex\_lock()\_与\_mutex\_lock\_interruptible()\_的区别与使用边界
...
### 17.11.4\_为何\_可中断等待\_会破坏资源稳定性

#### (1)\_问题并非\_修改资源\_而是\_提前返回
...
## 17.12\_semaphore\_接口与使用规则(计数型可睡锁)
...
#### (2)\_表\_17-10-2B\_获取(down\_系列\_计数-1)
...
#### (3)\_表\_17-10-2C\_释放(up\_系列\_计数+1)
...
## 17.13\_rw\_semaphore\_接口与使用规则(读写信号量)
...
写->读降级详情请参考 [17.12　`downgrade_write()`：写锁降级为读锁的机制与使用场景](#17.14_downgrade_write()_写锁降级为读锁的机制与使用场景)。
...
## 17.14\_downgrade\_write()\_写锁降级为读锁的机制与使用场景
...
#### (1)\_表\_17-12-2\_释放再取锁\_vs\_原子降级
