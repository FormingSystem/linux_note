...
# 第14章\_READWRITE\_ONCE\_与\_smp\_(内存可见性与顺序原语)

## 14.1\_READ/WRITE\_ONCE\_与\_smp\_*(内存可见性与顺序原语)
...
#### (4)\_为何会引入\_READ\_ONCE()/WRITE\_ONCE()\_机制
...
##### 3)\_READ\_ONCE\_/\_WRITE\_ONCE\_的设计动机
...
| **层3：同步协议** | spinlock/mutex/RCU | 按具体原语提供互斥、发布—取得、生命周期或其他复合契约 |
...
这些层次会组合使用，但不是简单的强弱递进。例如锁提供互斥，RCU 通常不提供互斥；RCU 的指针发布、读侧保护和宽限期回收也不能由一个通用 `smp_mb()` 代替。
...
##### 2)\_READ/WRITE\_ONCE\_解决的是\_可见性\_不是\_顺序性
...
##### 4)\_smp\_*\_系列的动机\_建立\_跨CPU可观测顺序
...
##### 5)\_举例\_为什么不能靠\_READ\_ONCE\_/\_WRITE\_ONCE\_解决
...
##### 6)\_Linux\_为什么用\_smp\_*()\_而不是直接用\_CPU\_指令
...
- 对锁已经覆盖的普通共享数据访问，不应无依据地重复添加 `smp_*`；但锁外协议仍可能有额外顺序要求。
- 在无锁路径或设备交互路径中，应优先使用能表达协议的高层 API（如 RCU 发布—取得接口、原子 acquire/release、DMA/MMIO accessor），而不是把裸屏障当作唯一工具。
...
| 3️⃣    | 锁 / RCU / 信号量 | 提供各自不同的同步协议             | 取决于原语  | ✅          |

> ✅ **`ONCE` 是“可见性”语义，
> ✅ `smp_\*` 是“顺序”语义，
> ✅ 锁是“互斥”语义。**

三者职责不同，应按具体协议组合；不能把 RCU 当作“更强的一层屏障”。
...
| 锁、RCU、信号量保护的数据 | 使用对应 API 契约；不要凭感觉补屏障 | 不同原语提供的顺序保证不同，RCU 读侧尤其不能笼统等价为 `smp_mb()` |
...
#### (2)\_模式①\_发布-获取(Publish-Acquire)模型
...
### 14.1.9\_内存屏障的可视化与验证(Store\_Buffer\_/\_Load\_Queue\_执行模型)
...
#### (4)\_加上\_smp\_wmb()/smp\_rmb()\_强制顺序建立
