---
id: knowledge.linux.synchronization.concurrency_and_competition.p01_并发脉络与概念缓冲.p07_生命周期与有序停机
title: "生命周期与有序停机"
kind: mechanism
status: evolving
domains:
...
### 7.2.4\_表\_7-1\_devres\_与手动资源管理对比
...
- 提供统一的对象生命周期管理接口；
- 可与 `RCU`、`devres` 组合，但职责必须分开：RCU 负责 GP 后回收条件，devres 只按设备解绑顺序释放已登记资源，不会自动等待 RCU 读者或回调。
