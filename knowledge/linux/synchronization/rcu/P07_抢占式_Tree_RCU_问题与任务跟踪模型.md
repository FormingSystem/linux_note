...
仍使用第 5 章的配置对象，但这次内核启用了 `CONFIG_PREEMPT_RCU=y`。CPU1 上的普通优先级任务 `R-old` 已经取得 `old_obj`，还没执行 `rcu_read_unlock()`；此时一个更高优先级任务被唤醒，调度器把 `R-old` 换出。`R-old` 甚至可能稍后迁移到 CPU2 才继续运行。
