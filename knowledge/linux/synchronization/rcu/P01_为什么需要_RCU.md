...
第一，RCU 相对读写锁优化的不是“锁函数写法”，而是 **高频读者之间的同步方式**。传统读写锁的读者通常需要原子修改同一份共享锁状态，由此产生缓存行所有权转移；Tree RCU 则让普通读侧主要操作任务或 CPU 本地状态，把跨 CPU 汇聚放到每轮 GP 的 QS 报告、被抢占读者登记和必要的 force-QS 路径。更新者因此不再等待“所有读者先退出才能发布”，而是在发布新版本后等待发布前的旧读者越过安全边界。
...
更完整的角色视角、数据结构拓扑、调度通知链和静止状态汇聚将在[Linux Tree RCU 状态与通知机制](P04_Linux_Tree_RCU_状态与通知机制.md)中实现级展开；下一章先从本章的问题约束推演出不绑定 Linux 字段的抽象机制。
...
## 1.11\_先区分原子计数\_refcount\_与\_kref

到这里，RCU 自身的最小模型已经闭合：读者在临界区内取得原始对象指针，更新者先取消发布旧版本，RCU 再用 GP 证明旧的临时访问已经结束。现在才适合回到另一个自然问题：既然危险来自对象过早释放，引用计数能否解决它，又为什么还要与 RCU 组合？

“引用计数”是一类对象生命期方案，不应在视觉和概念上直接等同于 kref。Linux 中至少要区分下面三个层次：

| 层次 | 提供什么 | 不自动提供什么 |
| --- | --- | --- |
| 用 `atomic_t` 手写引用计数 | 并发原子加减和数值判断 | 引用计数专用的溢出、下溢和零值误用防护，也没有统一释放协议 |
| `refcount_t` | 引用计数专用操作、饱和语义和误用检查；最后一次 `refcount_dec_and_test()` 可供调用者判断 | 对象类型封装、统一的析构回调和共享入口的查找保护 |
| `kref` | 在对象中封装一个 `refcount_t`，以 `kref_init()`、`kref_get()`、`kref_put(release)` 约定引用取得、归还和最后一次归还后的析构 | 查找表本身的并发同步，也不会自动保证尚未取得引用的裸指针有效 |

因此，kref 不是与引用计数无关的另一种机制，而是建立在 `refcount_t` 之上的对象生命期接口。它把“计数降到零”与对象的 `release` 回调连接起来，使持有者只要成对遵守 get/put 协议，最后一个持有者就能触发释放；相比直接操作普通原子变量，这套接口还继承了 `refcount_t` 针对引用计数误用提供的防护。

这里还要校准一个容易受示例代码影响的细节：`struct kref` 只保存计数，`kref_init()` 也只把计数初始化为 1，并不会在对象里登记一个 release。调用者在 **每次** `kref_put(&obj->ref, obj_release)` 时传入回调，最后一次 put 才调用本次传入的函数。某份源码中的 `obj_release()` 是空函数，只表示该对象所属模块选择在归零事件上不做清理；它不是 kref 的“默认空 release”。回调可以直接释放、登记 RCU 回收、归还下层资源，甚至有意不做动作，但同一种对象的所有 put 必须遵守一致的归零协议。不能先用 `kref_read()` 观察到零，再在外部另开一条手工释放路径，因为观察与释放之间没有原子的所有权交接。

本章只保留理解 RCU 所需的最小边界。若要继续了解这些层次的源码关系、适用对象和接口差异，可以转到 kref 专题的 [kref、refcount_t 与 kobject 的边界](../../object_lifetime/kref/P11_kref_refcount_t_kobject_的边界.md)；完整学习顺序见 [kref 引用计数机制专题大纲](../../object_lifetime/kref/大纲.md)。

如果共享入口和 kref 遵守完整协议，kref 可以正确完成对象生命期同步与最终释放。不能先从一个可并发删除的入口读取裸指针，再在没有任何保护的情况下调用 `kref_get()`；这不是 kref 的正常用法，而是违反了“取引用时对象仍然有效”的前提。`refcount_inc()` 对零计数发出警告也不能使已经释放的对象重新有效。

### 1.11.1\_kref\_单独怎样完成生命期同步

沿用前文的读写锁方案，可以让共享入口持有 A 的一份基础引用，并把“查找对象与取得引用”放在同一个读侧临界区内：

```text
CPU0 读者                              CPU1 删除者
down_read(&table_lock)
从 global_ptr 取得 A
kref_get(&A->ref)                      down_write(&table_lock) 并等待
up_read(&table_lock)
                                       将 global_ptr 从 A 改为 B
                                       up_write(&table_lock)
                                       kref_put(&A->ref, release)
使用 A
kref_put(&A->ref, release)
```

这里不会出现计数为零后再次自增：删除者必须等读者完成 `kref_get()` 才能摘除入口并释放入口持有的基础引用；只要 CPU0 的独立引用尚未归还，A 的计数就不会降到零。若查找表还有其他并发访问规则，也可以使用 mutex、spinlock 或适合该数据结构的同步机制实现同一保证。锁怎样保护首次取引用、脱链和最后一次 put，可继续阅读 [kref 与锁的组合](../../object_lifetime/kref/P09_kref_与锁的组合.md)。

RCU 在这里不是为了修补 kref 的正确性，也不是因为 kref 不能同步释放。单独使用“读写锁 + kref”已经可以建立完整保证：读写锁保护共享入口及首次取引用，kref 统计取引用成功后的长期持有者，并在最后一次 `kref_put()` 时调用释放函数。

### 1.11.2\_kref\_与\_RCU\_为什么会组合

“RCU 旧照”不能默认理解为一块连续内存。它可能是单个对象，也可能是一个很小的版本根节点加上多个分散数据块。讨论 RCU 与 kref 的组合之前，必须先画清 **哪个分配块被 RCU 入口直接发布、哪个对象持有哪个 kref、reader 是否要把某个块带出读侧区间**。所有权拓扑不同，宽限期与引用归零的先后顺序也会不同。

| 模型 | RCU 直接发布什么 | kref 统计什么 | 常见回收顺序 |
| --- | --- | --- | --- |
| 单个可查找对象 | 对象 A 本身 | 离开 RCU 后仍持有 A 的用户 | A 的 kref 归零 → 把 A 交给 RCU → GP 后释放 A |
| 单个对象但发布引用跨越 GP | 对象 A 本身 | 查找入口和长期用户对 A 的持有 | 取消发布 A → GP 后归还入口引用 → A 的 kref 归零后释放 |
| 复合旧照 | 版本根 A，根节点指向多个 block | 各版本根、逃逸 reader 对每个 block 的持有 | 取消发布根 A → GP 后归还 A 对各 block 的引用 → 各 block 分别在引用归零时释放 |

前两行操作的是 **同一块对象内存**，第三行操作的是 **一个所有权图**，不能用一条“最后 put 后 `kfree_rcu(obj)`”概括全部场景。

#### (1)\_单个对象从\_RCU\_读区带出去

若共享入口直接指向一个内嵌 `struct kref` 的对象，reader 在 RCU 临界区内取得的是该对象的原始指针，不是对象副本。reader 若要在退出 RCU 后继续使用它，必须在临界区内把 RCU 提供的临时可访问资格转换成独立引用：

```text
RCU 保护 lookup 和尝试取引用的窗口
    ↓
kref_get_unless_zero() 成功
    ↓
reader 退出 RCU 后凭 kref 继续使用同一块对象内存
```

这类对象可以选择“最后 kref 归零后由 `release()` 调用 `kfree_rcu()`”，也可以让查找入口持有一份引用，取消发布后先跨过 GP，再在 RCU 回调中归还这份入口引用。二者都是有效协议，但同一对象必须选定一个唯一销毁出口，不能一条路径直接 `kfree()`，另一条路径又独立登记 RCU 回调。

#### (2)\_一张旧照由多个分散块组成

设旧版本不是一块 A，而是一个版本根 `root_A` 指向三个独立分配的数据块：

```mermaid
flowchart LR
    Entry["RCU 共享入口"] --> RootA["root_A<br/>版本与块目录"]
    RootA -->|"持有一份 kref"| B0["block_0"]
    RootA -->|"持有一份 kref"| B1["block_1"]
    RootA -->|"持有一份 kref"| B2["block_2"]
    RootB["新版本 root_B"] -->|"复用并持有另一份 kref"| B1
    RootB -->|"持有一份 kref"| B3["新 block_3"]
```

这里的关键不是给每个 RCU reader 都增加三个 kref，而是让 **每个版本根为自己引用的每个数据块持有一份所有权引用**：

- 旧 reader 在 RCU 临界区内从 `root_A` 访问 `block_0～2`，不必为这次短读取逐块执行 get／put。
- 新版本 `root_B` 若复用 `block_1`，构造 B 时为 `block_1` 再取得一份引用；A、B 各自拥有它。
- 发布 B 并取消发布 A 后，A 仍必须保留它对所有 block 的引用，直到旧 reader 全部退出。
- A 的 RCU 回调在 GP 结束后逐个 `kref_put()`；只属于 A 的块可能立即归零释放，被 B 复用的块因 B 仍持有引用而继续存在。

所以复合旧照的自然回收链是：

```text
RCU 先证明没有 reader 再沿 root_A 访问子块
    ↓
root_A 才归还它对各 block 的所有权引用
    ↓
每个 block 的 kref 独立决定该分配块是否归零
    ↓
各 block 的 release 分别释放自己的内存
```

这与“单个对象先等 kref 归零，再 `kfree_rcu()`”的顺序正好相反。原因不是 API 相互冲突，而是 kref 在两个模型中代表的所有权不同：前者统计对 RCU 对象本身的长期持有，后者统计各版本根和逃逸用户对分散子块的持有。

#### (3)\_某个子块还要被\_reader\_带出去

若 reader 只在 `rcu_read_lock()` 与 `rcu_read_unlock()` 之间使用子块，版本根持有的那份 kref 已经足够。若 reader 要把某个 block 交给工作队列、跨越睡眠或保存为长期句柄，则必须在仍受 root 的 RCU 保护时额外取得该 block 的 kref：
...
reader 进入 RCU
    ...
    -> 取得 root_A->block_1
    -> 为 block_1 增加一份逃逸引用
reader 退出 RCU 并长期使用 block_1

root_A 的 GP 结束
    -> A 归还对 block_1 的版本引用
    -> reader 的逃逸引用仍使 block_1 存活

reader 最后 put
    -> block_1 归零并执行自己的 release
```

只要协议保证 `root_A` 对 block 的引用一定保持到 A 的 GP 结束，reader 在该 RCU 临界区内就已经知道 block 计数非零；若 block 还允许从版本内部独立取消发布，则需要额外的子块状态机、`kref_get_unless_zero()` 或子块自己的 RCU 协议，不能继续套用这个简化前提。

#### (4)\_RCU\_和\_kref\_仍然彼此无感

普通 RCU 不接收对象地址，不知道某个旧版本包含几个块；kref 也不知道自己的对象属于哪个 RCU 版本。真正执行组合的是对象所属模块：

```text
版本根的 RCU 回调
    -> 表示旧 reader 已经不能再沿该根访问子块
    -> 模块在回调中归还根对各子块的 kref
...
这里出现多个 `release()` 并不表示同一块内存有多个释放出口。`root_A`、`block_0`、`block_1` 是不同分配，每个分配都必须有且只有一条最终销毁路径。

#### (5)\_读多时怎样取舍

复合模型恰好说明“reader 多”不等于“每次读取都要逐块修改 kref”。高频短 reader 可以只使用 RCU；kref 的原子成本主要发生在构造新版本时共享 block，以及少量确实要逃出读区的 reader 上。真正需要评估的是：

- 每次更新新建和复用多少 block，构造新版本需要多少 get。
- 一次旧版本退休要归还多少 block 引用，RCU 回调能否保持短小。
- 是否存在大量 reader 把同一个热点 block 带出读区，重新制造引用计数缓存行争用。
- 长期引用和连续更新是否使多代 root 与 block 同时滞留，形成内存峰值。

本章只建立“先看所有权拓扑，再决定 GP 与 kref 的顺序”这个问题框架。抽象状态转换放在下一章的[RCU 抽象机制推演](P02_RCU_抽象机制推演.md)，单对象查找交接可继续阅读[kref 与 RCU](../../object_lifetime/kref/P10_kref_与_RCU.md)，单块、复合快照和逃逸子块的完整代码模板统一放在[RCU 数据结构模板与选型](P21_RCU_数据结构模板与选型.md)。

所以这一节得到的结论不是“RCU 一定先等 kref”或“kref 一定先等 RCU”，而是：**先确定哪一个对象拥有哪一块内存，再让取消发布、宽限期和引用归零按所有权图逐层收敛。** 这一组合结论建立在前文已经完成的 RCU 时间轴之上；GP 的定义不依赖 kref，kref 只是对象所属模块在 RCU 临时访问之外增加的所有权层。

## 1.12\_接下来怎样把直觉变成机制

下一章不急着继续增加术语，而是先验证本章关于成本转移的结论：

- 多个读者 CPU 与单个写者 CPU 分别做什么。
- `task_struct`、`rcu_data`、context tracking、`rcu_node` 和 `rcu_state` 分别保存什么状态。
- 正常 QS 怎样主动上报，被抢占读者怎样从任务私有状态转为共享登记。
- GP kthread 怎样读取远端状态并在慢路径催促未报告 CPU。

下一章不立即进入 Linux 字段，而是从本章已经建立的问题约束自行推出发布、取消发布、宽限期和回收；等朴素机制经过真实运行条件检验后，第四章才进入 Linux 的具体状态与通信实现。
