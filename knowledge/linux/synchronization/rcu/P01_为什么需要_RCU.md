...
flowchart TD
    ...
    T --> O{离开读侧后继续使用吗}
    O -->|否| X[RCU 临时生命期足够]
    O -->|是| Y["在 RCU 内转换成独立所有权，<br/>见 1.11"]
```
...
flowchart TD
    ...
    U --> X{是否存在特殊慢路径状态?}
    X -->|否| F[快速退出]
    X -->|是| S[处理被抢占读者／<br/>deferred QS 等状态]
```
...
1. 非抢占读侧通过禁止抢占，保证临界区不会跨过调度静止点；CPU 后续发生上下文切换、进入用户态、进入 idle/EQS 或下线时，可以证明它已经离开此前的非抢占读侧区间。这条证明可以拆成四步：
   - 更新者先取消发布旧指针，再启动或等待一个 GP；因此该 GP 只需覆盖 **取消发布之前可能已经取得旧指针** 的读者。
   - 非抢占读者进入临界区后不能被调度切走。某个 CPU 在 GP 期间经过上下文切换、用户态、idle/EQS 或下线边界，就证明该 CPU 上取消发布前开始的非抢占读侧临界区已经结束。
   - 在取消发布后才开始的新读者无法再从受保护的发布点取得旧指针，所以它们不属于本轮 GP 必须等待的旧读者集合；这就是读者不必逐次向写者登记的前提，而不是写者按固定次数轮询后猜测安全。
   - 每 CPU 路径把 QS 证据记录到 `rcu_data`，再由 `rcu_node` 位图逐层清除和汇聚。根节点只有在本轮旧读者证据全部闭合后才宣布 GP 完成，等待者或回调随后才能回收旧对象。
...
更完整的角色视角、数据结构拓扑、调度通知链和静止状态汇聚将在[Linux Tree RCU 状态与通知机制](P10_Tree_RCU_统一状态与通知总图.md)中实现级展开；下一章先从本章的问题约束推演出不绑定 Linux 字段的抽象机制。
...
## 1.11\_先完成RCU主线再讨论kref

引用计数确实也是对象生命期工具，但现在插入 `atomic_t`、`refcount_t`、`kref` 和多块所有权图，会迫使读者在尚未理解 GP 的情况下同时处理第二套状态机。本章只留下必要边界：**RCU 保护协议内的临时访问区间，不自动提供跨读区的长期所有权。**

先在下一章独立推演发布边界、旧读者集合和 GP，再由第 3 章给出 RCU 的最小 API 闭环。到第 4 章时，读者已经知道 RCU 自己解决什么，才能准确比较：
...
单对象、入口发布引用以及“版本根 + 多个独立 kref 数据块”的完整实现统一放在[RCU、kref 与复合对象生命周期](P04_RCU_kref与复合对象生命周期.md)，不再由问题引入章提前展开。
...
下一章不进入 Linux 字段，而是从本章已经建立的问题约束自行推出发布、取消发布、宽限期和回收。随后先通过最小 API 和对象生命期组合建立使用边界，再分别进入非抢占式与抢占式 Tree RCU 的证明和源码状态机。
