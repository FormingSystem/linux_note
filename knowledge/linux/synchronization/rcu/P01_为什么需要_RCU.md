...
看到这里，一个很自然的反应是：既然危险来自对象过早释放，给对象加引用计数不就可以了吗？这个方向并没有错。引用计数确实能够建立完整的对象生命期保证，只是它与前文的读写锁一样，也有自己的使用前提和路径成本。先把几个容易混在一起的名称分清楚，后面的比较才不会变成“kref 能不能替代 RCU”的二选一。

## 1.6\_先区分原子计数\_refcount\_与\_kref
...
### 1.6.2\_kref\_与\_RCU\_为什么会组合

既然“锁 + kref”已经能够正确管理生命期，为什么还会出现“RCU + kref”？原因不是前一种方案不正确，而是读多写少的查找路径可能不愿让每个读者都修改共享读锁状态。此时可以把首次取引用之前的临时保护从锁换成 RCU，而长期持有仍然交给 kref。两者不是重复维护两份相同的引用计数，而是分别覆盖同一操作周期中的不同阶段：

```text
RCU：保证查找期间对象所在内存仍可访问
                 ↓
kref_get_unless_zero()：确认对象尚未进入零引用析构状态，并取得长期引用
                 ↓
kref：退出 RCU 后继续保证对象存活，直到最后一个持有者归还引用
```

需要比较的是 **取得稳定对象的路径成本与使用方式**：

| 方案 | 查找阶段怎样保证对象有效 | 离开查找阶段后怎样使用 | 主要成本 |
| --- | --- | --- | --- |
| 读写锁 + kref | 在读锁内查找并执行 `kref_get()` | 凭独立引用长期使用 | 每次查找修改共享读锁状态，并执行引用计数原子操作 |
| RCU + kref | 在 RCU 读侧区间内查找，并用 `kref_get_unless_zero()` 尝试取得引用 | 取得成功后凭独立引用长期使用 | 查找避免共享读锁，但仍有引用计数操作；删除还要遵守 RCU 延迟回收协议 |
| 仅用 RCU 临时访问 | 在 RCU 读侧区间内查找和使用 | 不把裸指针带出读侧区间 | 不为每次读取取得长期引用，但对象必须延迟到宽限期后回收 |

Linux 6.12.20 的 kref 文档明确要求：从查找结构中取得此前并未持有引用的对象时，必须让查找、对象存活和取引用形成同一个受保护过程。若采用 RCU 查找，`kref_get_unless_zero()` 只负责拒绝把零计数复活；包含 `struct kref` 的内存本身仍须在 RCU 读者可能访问期间保持有效，删除路径通常用 `kfree_rcu()` 或等价的宽限期安排满足这一点。反过来，RCU 读侧区间结束后不再保护这个读者，后续长期使用依靠刚刚取得的 kref。完整代码、删除顺序和宽限期关系集中放在 [kref 与 RCU](../../object_lifetime/kref/P10_kref_与_RCU.md)，这里不提前展开。

选择边界最终还是由实际负载决定。如果读者取得对象后需要跨越睡眠、I/O 或较长处理过程，锁内取得 kref 后再解锁通常已经完整解决问题；如果读者只需短暂查看对象，而且高频查找对共享读锁和每次引用计数操作的成本已经不可接受，才有理由让 RCU 单独承担这段临时访问。对象字段如何形成一致状态、多个字段怎样原子更新，则是另一项并发一致性问题，不能由 kref 或 RCU 的生命期保证自动推出。

所以这一节得到的结论不是“引用计数做不到”，而是：**锁加 kref 可以用集中同步换取直接而完整的生命期保证；RCU 则尝试在满足特定读取条件时，拿掉高频读路径上的集中登记。** 接下来要追问的，正是 RCU 在不阻止旧读者继续运行的情况下，怎样把立即释放改造成一个可以等待并证明的过程。

## 1.7\_关键转折\_不赶走旧读者

前面的读写锁与 kref 方案都在“使用对象之前先取得一份明确许可”：锁提供临界区许可，kref 提供长期持有许可。RCU 没有否定这些方案，而是针对短小、高频的读取换了一个方向：既然读操作远多于写操作，能否先不赶走已经取得 A 的旧读者，让写者承担复制与等待成本？
...
前面的方案比较可以压缩成三组便于工程判断、但不能丢失适用边界的关系。
...
第三，RCU 与 kref 不是同一层次上的互斥替代方案。RCU 保护的是共享入口的并发发布、取得以及旧版本延迟回收窗口；kref 为已经安全取得的具体对象建立可跨越原读侧临界区的独立持有关系。查找路径可以先在 RCU 保护下取得仍然有效的对象指针，再按照对象协议增加引用；离开 RCU 读侧以后由 kref 继续保证对象生命期。反过来，不能先从可并发删除的入口取得一个未受保护的裸指针，再试图用 `kref_get()` 挽救它。
