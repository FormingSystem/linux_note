...
# 第3章\_RCU\_机制完善\_硬件与运行约束

上一章推导出了最小 RCU 时间线，但那个模型默认读者会顺利结束、CPU 会及时参与、所有状态会按源码顺序被观察。真实系统并不满足这些假设。本章用抢占、迁移、idle、无 tick、迟延 CPU 和内存乱序逐项检验朴素模型，说明一套可实现的 RCU 还必须增加什么状态和通信能力；下一章再看 Linux 如何兑现这些要求。

> **阅读方法：** 每一节都按“朴素假设为什么失败 → 必须补充什么能力”阅读。不要先背 Linux 字段；先理解为什么目标系统不得不拥有这些状态。

## 3.1\_六个反例怎样逼出完整机制

| 朴素模型的缺口 | 真实运行反例 | 被迫增加的机制能力 | Linux 实现将在下一章映射到 |
| --- | --- | --- | --- |
| 不知道读者状态放在哪里 | 每次修改全局计数会重新产生共享缓存行争用 | 任务或 CPU 本地状态，低频转成共享证明 | `task_struct`、per-CPU `rcu_data` |
| 默认读者始终留在原 CPU | 读者可能在临界区内被抢占，随后迁移 | 能跟随任务存在的状态，以及本地状态转共享登记的路径 | nesting、`blkd_tasks/gp_tasks` |
| 默认 CPU 会及时报告 | CPU 可能进入 user、idle、NO_HZ 或长时间不调度 | watching/EQS 状态与跨时间快照 | context tracking/dynticks |
| 默认所有 CPU 可写一个完成字 | 大量 CPU 汇聚到同一缓存行会形成新瓶颈 | 每 CPU 记录、叶节点聚合、逐层向根汇聚 | `rcu_data → rcu_node` 树 |
| 默认参与者不会迟延 | CPU 可能很久不经过常规 RCU 检查点 | 远端观察、urgent 标志、重调度请求或 IPI 催促 | force-QS 路径 |
| 默认源码顺序就是观察顺序 | 编译器和 CPU 可以重排不同地址的访问 | 发布—取得契约与 GP 内部顺序保证 | RCU 指针原语、屏障与锁链 |

这张表形成下一章的需求清单。**Linux 字段不是凭空设计出来的；每一组状态都在修补朴素模型的一个具体漏洞。**

## 3.2\_CPU\_中没有\_RCU\_单元
...
下一篇：[Linux Tree RCU 状态与通知机制](P04_Linux_Tree_RCU_状态与通知机制.md)。
