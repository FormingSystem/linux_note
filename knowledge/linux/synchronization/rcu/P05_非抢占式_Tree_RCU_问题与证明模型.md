...
先固定一个能把争议暴露出来的场景：CPU0 上的管理线程替换对象；CPU1 上已有一个 reader 正在使用旧对象；CPU2 的运行队列里早已存在另一个 reader 任务，但它还没有执行读侧代码。写者不知道任何 reader 的任务 ID，也不知道谁读到了 `old_obj`。
...
只有最后一种事实与 `old_obj` 的回收直接相关。完整可构建实验见[晚到读者与抢占读者的对象回收实验](../../../../labs/kernel/rcu/P01_晚到读者与抢占读者/README.md)。
...
这种保守性允许 RCU 不在每次读操作上维护一个全局读者登记表。一个实际上没有旧 reader 的 CPU 也可能被等待一次；这是把高频读侧写共享状态的成本转移到按 GP 发生的证明收集。

### 5.4.3\_T2\_全部证明汇聚后回收

只有所有相关 CPU 的 QS 证明都汇聚完成，GP 才结束。于是 T0 前可能取得旧对象的合法 reader 已不可能继续使用它，写者才能 `kfree(old_obj)`。
...
若实现要允许 reader 被抢占并在另一 CPU 恢复，仅等待 CPU QS 就会发生假证明：原 CPU 已切换并报告，但旧任务仍持有指针。第七章将由这个反例推出 PREEMPT_RCU 的任务跟踪。
