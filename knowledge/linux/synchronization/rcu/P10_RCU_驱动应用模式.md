...
本章只保留驱动场景中的组合方式，不重复维护通用接口说明。接口契约统一查阅[RCU API 速查](P08_RCU_API_速查.md)，可直接复用的最小调用链统一查阅[RCU 模板、选型与核对](P09_RCU_模板_选型与核对.md)。

审查驱动代码时，应把注意力放在场景新增的责任上：写者由谁串行化、对象能否逃出读侧区间、回调是否引用模块代码，以及卸载路径是否阻止了新回调产生。

上一篇：[RCU 模板、选型与核对](P09_RCU_模板_选型与核对.md)。

下一篇：[RCU 类型语义与 Sparse 检查](P11_RCU_类型语义与_Sparse_检查.md)。
