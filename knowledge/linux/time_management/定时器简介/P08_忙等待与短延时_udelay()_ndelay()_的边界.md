...
#### (5)_面向_demo_led_key_int@0_的落地示例
...
### 8.1.7_SMP_/_PREEMPT_/_PREEMPT_RT_下的注意事项
...
#### (4)_可视化_RT_内核下的_短忙等_to_线程化处理_路径
...
#### (4)_什么时候选_hrtimer_而不是_udelay()_/_usleep_range()
...
### 8.1.9_用户视角_接口速览与使用边界(一页速查)

#### (1)_结论先行(能睡就不忙_不可睡只短忙)
...
#### (2)_端到端事务(以_demo_led_key_int@0_为例)
...
#### (1)_策略化轮询_Helper(统一_sleep/atomic_两路)
...
#### (2)_devres_化收尾_hrtimer_与_delayed_work
...
#### (3)_端到端示例_demo_led_key_int@0(整合_DT_参数与两路轮询)
...
### 8.1.12_调试与验证(可复制命令清单)

> 目标：证明**没有**长忙等与系统迟滞；若存在，定位到具体调用栈与参数，并给出修正建议。

#### (1)_ftrace_确认忙等窗口与中断延迟
