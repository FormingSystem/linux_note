...
## 3.2_常用宏_msecs_to_jiffies()_usecs_to_jiffies()_jiffies_to_msecs()
...
## 3.3_纳秒系与_ktime_t_ktime_set()_ns_to_ktime()
...
### 3.3.5_用户视角(从外部/DT_来的值怎么进_ktime)
...
#### (1)_示例_A_周期性_hrtimer(HRTIMER_MODE_REL_+_forward_now)
...
#### (2)_示例_B_设备树_nxp,debounce-ms_to_ktime(高精度去抖)+_工作队列分层
...
#### (3)_示例_C_绝对超时点(deadline)计算与判断(ktime_get_+_ktime_add)
...
## 3.5_与设备树时间属性的映射(以_demo_led_key_int@0_/_nxp,debounce-ms_为例)
...
#### (1)_模板_A_边沿去抖(one-shot_IRQ_中启动)_使用成员_debounce_kt
...
#### (2)_模板_B_高精度周期轮询(periodic_hrtimer_forward_now())_使用成员_period_kt
