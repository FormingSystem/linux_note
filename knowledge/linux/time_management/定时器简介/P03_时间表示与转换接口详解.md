---
id: knowledge.linux.time_management.定时器简介.p03_时间表示与转换接口详解
title: "时间表示与转换接口详解"
kind: mechanism
status: evolving
domains:
...
## 3.2\_常用宏\_msecs\_to\_jiffies()\_usecs\_to\_jiffies()\_jiffies\_to\_msecs()
...
## 3.3\_纳秒系与\_ktime\_t\_ktime\_set()\_ns\_to\_ktime()
...
### 3.3.5\_用户视角(从外部/DT\_来的值怎么进\_ktime)
...
#### (1)\_示例\_A\_周期性\_hrtimer(HRTIMER\_MODE\_REL\_+\_forward\_now)
...
#### (2)\_示例\_B\_设备树\_nxp,debounce-ms\_to\_ktime(高精度去抖)+\_工作队列分层
...
#### (3)\_示例\_C\_绝对超时点(deadline)计算与判断(ktime\_get\_+\_ktime\_add)
...
## 3.5\_与设备树时间属性的映射(以\_demo\_led\_key\_int@0\_/\_nxp,debounce-ms\_为例)
...
### 3.5.5\_代码模板(delayed\_work\_版\_可睡)
...
### 3.5.6\_代码模板(hrtimer\_版\_高精度\_+\_分层)
...
#### (1)\_模板\_A\_边沿去抖(one-shot\_IRQ\_中启动)\_使用成员\_debounce\_kt
...
#### (2)\_模板\_B\_高精度周期轮询(periodic\_hrtimer\_forward\_now())\_使用成员\_period\_kt
