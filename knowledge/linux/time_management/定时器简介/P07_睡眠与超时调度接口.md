---
id: knowledge.linux.time_management.定时器简介.p07_睡眠与超时调度接口
title: "睡眠与超时调度接口"
kind: mechanism
status: evolving
domains:
...
## 7.1\_主动延时接口\_msleep()\_usleep\_range()\_ssleep()

### 7.1.1\_引入(是什么\_/\_为什么)
...
## 7.2\_调度型超时\_schedule\_timeout()\_系列(本章核心)
...
### 7.2.3\_自然到期\_vs\_被动唤醒\_把\_谁来叫醒我\_说清楚
...
## 7.3\_等待队列带超时\_wait\_event\_timeout()\_/\_wait\_event\_interruptible\_timeout()

### 7.3.1\_引入(为什么还需要等待队列)
...
## 7.4\_completion\_超时\_wait\_for\_completion*\_timeout()

### 7.4.1\_导语(为什么还有\_completion)
...
### 7.6.2\_模板二\_完整一单\_+\_超时\_+\_回滚/重试(completion\_版)
...
### 7.6.3\_模板三\_按键/传感器去抖确认(与第\_6\_章协同)
