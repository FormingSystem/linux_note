...
下面把这种交错写成确定的 C11 模型；它不依赖宿主调度碰巧发生，也不模拟 CPU 内存乱序。
...
保存为 `interleave_model.c`，用 `cc -std=c11 -Wall -Wextra -Werror -pedantic interleave_model.c -o interleave_model` 编译，再执行 `./interleave_model`。代码在单线程中重放已指定的交错，不让两个真实线程无锁修改普通 C 对象；后者会引入 C 数据竞争，不能当作可预测的示范。
...
1. 给上面的 C 模型每次赋值加一个“原子”标签，会阻止丢失 A 吗？
