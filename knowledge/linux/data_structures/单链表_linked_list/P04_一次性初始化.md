...
若确实要懒初始化，先让所有使用者通过同一把可睡眠锁取得结果。下面完整 C++17 模型用 std::mutex 表示这种互斥。std::lock_guard 在离开作用域时归还锁，异常退出也不漏锁；std::shared_ptr 保存共享所有权，const std::vector<int> 使发布后的序列只读。构建者先持有可写的局部 candidate，完成后才把它交给共享 state。std::async 启动两个调用者，其 future 的 get 等待完成并取得结果。首次故意失败，随后两个调用者得到同一个完整对象：
...
保存为 `once_retry.cpp`。编译器需要支持 C++17 的线程标准库：

```bash
c++ -std=c++17 -Wall -Wextra -Werror -pedantic -pthread once_retry.cpp -o once_retry
./once_retry
```

预期尝试次数为 2，结果为 10、20、30。第一次只把 10 放进私有候选就抛出异常；局部智能指针释放候选，共享 state 仍为空。后两个调用者谁先获得锁不重要，成功构建者发布后，另一个复用同一对象。两个结果各自持有引用，因此解锁后不会因局部 candidate 销毁而失效。

本例使用真实宿主互斥和线程，但只演示共同锁、失败重试与 C++ 共享所有权，不验证 Linux 内核屏障、DO_ONCE 或内核对象回收。断言在两个 get 完成后读取 attempts，没有越过同步直接读取构建中的共享计数。
...
1. 如果模型在 candidate->push_back(10) 后立即设置 state，第一次失败会留下什么？
