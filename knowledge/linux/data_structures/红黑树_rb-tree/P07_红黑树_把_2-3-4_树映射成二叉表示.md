...
至此可以从性质、分组、边界槽和高度证明共同解释一棵稳定红黑树。接着用 [P35 完整插入](P35_红黑插入与红红冲突上推.md#35.3_运行完整插入程序)和 [P36 完整删除](P36_红黑删除与缺黑位置传播.md#36.3_用完整程序删除图中的对象)检验更新过程；已经完成两者的读者进入 [P33 同阶段对照](P33_从多路删除到红黑缺口.md#33.2_按同一删除阶段比较局部动作)，再沿 [P08 工程结构](P08_Linux_6.12_内核_rbtree_基础结构与工程模型.md)进入 Linux。返回[大纲](大纲.md#1.1_沿问题进入现有章节)。
