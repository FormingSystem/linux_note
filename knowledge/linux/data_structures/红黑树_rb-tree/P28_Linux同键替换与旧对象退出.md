...
入口交接完成不等于旧对象寿命结束；缓存和增广字段也不随嵌入节点赋值自动得到正确协议。下一章进入[P12 工程扩展](P12_Linux_6.12_内核_rbtree_工程扩展_并发与验证.md#12.1_章节内容说明)，继续讲 cached rbtree、augmented rbtree、并发控制、示例代码、调试验证和内核使用场景。
