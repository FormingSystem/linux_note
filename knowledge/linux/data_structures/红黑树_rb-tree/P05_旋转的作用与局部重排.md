...
以下历史片段继续按本节声明的旧版本阅读；当前 NXP 固定提交的完整两侧实现已归入[缺黑修复](../../../../research/source_reading/rbtree/source_explanations/lib/rbtree.c.md#1.5_缺黑修复的四种转换)。先沿[P11 取消周期](P11_Linux_6.12_内核_rbtree_删除与缺黑修复.md#11.1.3_一轮取消经过哪些状态)建立缺口位置，再比较改边；不要把抽象先染色的图直接当作 Case 3 回调时的字段值。
