...
### 5.5.5_rb_rotate_set_parents()_内核抽出来的公共收尾逻辑
...
#### (2)_Case_3_再在_gparent_上做局部旋转并收尾
...
### 5.5.7_删除修复中的旋转_直接写在_rb_erase_color()_的_case_里

删除修复同样不是通过一个“公开右旋 / 左旋 API”来组织，而是直接在 `____rb_erase_color()` 的四类 case 中展开。

#### (1)_Case_1_兄弟为红_先围绕_parent_旋转
...
#### (2)_Case_3_先在_sibling_上旋转_把近侄红转换成远侄红
...
#### (3)_Case_4_最后围绕_parent_旋转并做颜色翻转
...
### 5.5.9_增强树视角_为什么还要看_include/linux/rbtree_augmented.h
...
### 5.5.10_内核实现还有一个额外约束_WRITE_ONCE()
...
### 5.7.4_诊断的第一步_从变化点_z_向上找第一个失衡祖先_g
...
### 5.7.10_为什么这套诊断天然兼容_整棵树根_和_内部子树根
