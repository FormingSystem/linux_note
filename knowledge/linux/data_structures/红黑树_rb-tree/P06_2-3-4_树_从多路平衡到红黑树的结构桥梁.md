...
容量下界、借位、合并、根收缩和四组完整推演已组织为[P31 预修复删除](P31_2-3-4树预修复删除.md#31.1_从装不下走到不能删空)。先运行其中的完整 C++ 程序，再回来比较下面自底向上的传播与红黑表示；两种组织方式不能混成一条控制流。

### 6.6.1\_2-3-4\_树的自底向上删除
...
### 6.6.2\_红黑树的删除操作\_以及红黑树和\_2-3-4\_树删除操作的对比
