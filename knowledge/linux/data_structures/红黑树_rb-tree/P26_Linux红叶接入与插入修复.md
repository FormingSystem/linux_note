...
抽象分支与可运行的宿主树先见[P35 完整插入](P35_红黑插入与红红冲突上推.md#35.3_运行完整插入程序)。本章继续核对 Linux 的嵌入式节点、父槽写入与固定版本实现；两套对象布局不能混用。
...
下一篇沿默认路线进入[P11 取消周期](P11_Linux_6.12_内核_rbtree_删除与缺黑修复.md#11.1.3_一轮取消经过哪些状态)。追加编号保留既有章节身份，阅读次序以专题大纲为准。
