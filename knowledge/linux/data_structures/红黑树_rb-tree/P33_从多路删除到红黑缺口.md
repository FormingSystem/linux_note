...
先完成 [P31 预修复](P31_2-3-4树预修复删除.md#31.3_运行完整预修复删除)、[P32 回溯](P32_2-3-4树下溢回溯与根收缩.md#32.3_内部零键节点怎样继续传播)，再读 P07 的[颜色与黑高性质](P07_红黑树_把_2-3-4_树映射成二叉表示.md#7.3_红黑树的五条性质)。随后完成[P36 删除周期](P36_红黑删除与缺黑位置传播.md#36.1_移走对象为何不一定马上产生缺口)。本章是两套模型都成立后的比较，不在定义之前要求读者先背删除 case。
