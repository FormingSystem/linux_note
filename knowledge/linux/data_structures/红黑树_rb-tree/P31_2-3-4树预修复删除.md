...
现在可以解释本算法为什么无需向上回溯修复容量：非根节点进入时的余量足够它捐出一个父键。它付出的代价是可能在真正删除前改变路径，且为“未命中保持形状”多查一次。下一步进入[P32 下溢回溯](P32_2-3-4树下溢回溯与根收缩.md#32.1_把余量准备改成下溢报告)，比较先删再处理下溢怎样改变传播状态。
