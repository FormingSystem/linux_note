...
### 6.1.1\_进程管理\_PID\_哈希表与\_task\_struct\_的快速映射

#### (1)\_问题背景\_从\_$O(n)$\_到\_$O(1)$\_的必然选择
...
#### (3)\_深度源码\_无锁\_RCU\_检索逻辑

PID 的查找是典型的“读多写少”场景。内核利用 **RCU (Read-Copy-Update)** 机制，使读侧不必争抢传统共享读锁。读侧仍有配置相关的生命周期标记、指针取得和状态追踪开销，不能笼统描述成“完全没有状态”或“零开销”。

```c
/* 机制示意：调用者必须持有 RCU 读侧保护；具体版本以 kernel/pid.c 为准 */
struct pid *find_pid_ns(int nr, struct pid_namespace *ns)
{
    ...
    // pid_hashfn 结合 nr 和 ns 指针，减少不同 namespace 间的碰撞
...
调用者必须在 `rcu_read_lock()` 与 `rcu_read_unlock()` 之间调用并使用返回值；若要离开读侧区间继续持有 `struct pid`，应在区间内通过相应引用接口取得长期引用。不能由 lookup 自己解锁后把没有独立引用的裸指针交给调用者。
