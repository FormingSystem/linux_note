...
## 5.1_动态伸缩的_rhashtable_解决_弹性扩展_的死穴
...
### 5.1.2_核心句柄_struct_rhashtable_的实战意图
...
### 5.1.3_策略引擎_rhashtable_params_是如何防止崩溃的
...
#### (3)_深度辩证_为什么用_offset_而非_container_of

##### 1)_彻底解耦_库(Library)与用户(User)的隔离
...
#### (3)_指针标记(Pointer_Tagging)_搬迁状态的隐形信号
...
### 5.1.8_算法安全_为什么_hashfn_必须要带_seed
...
### 5.1.11_扩容期间的并发控制_rhashtable_insert_slow
...
#### (6)_读者遇到_封存桶_无锁重定向(Non-blocking_Redirect)
...
#### (7)_动态搬迁的一致性保障_表与桶的原子协作

##### 1)_物理层级_表(bucket_table)与桶(bucket)的关系
