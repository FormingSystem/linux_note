...
#### (2)\_禁止编译器\_作弊\_READ\_ONCE\_与\_rcu\_dereference
...
### 4.1.5\_RCU\_链表删除\_为什么\_next\_不能清空

这是 5.10 内核中 `hlist_del_rcu` 最反直觉的地方。

#### (1)\_示例\_删除一个\_僵尸\_进程节点
...
### 4.1.6\_宽限期(Grace\_Period)\_不记账的\_侧向观察

这是你最关心的：不计数，怎么知道没人读了？

#### (1)\_核心原理\_大门的\_开关\_实验
...
### 4.1.8\_RCU\_链表操作\_hlist\_的\_结构连续性\_保障

在 RCU 环境下，`hlist` 的删除操作不再是简单的指针置空，它必须在逻辑删除的同时，维持物理路径的连通。

#### (1)\_hlist\_del\_rcu\_为何不斩断退路
...
### 4.1.9\_宽限期(Grace\_Period)\_无计数的分布式共识算法

这是你最关心的部分：**写者不看读者的记账本，它是如何判定“清场”的？**

#### (1)\_静止态(Quiescent\_State)\_判定\_安全\_的物理指标
...
### 4.1.10\_RCU\_的局限性与工程约束

#### (1)\_读者\_禁眠令\_OOM\_的头号杀手
