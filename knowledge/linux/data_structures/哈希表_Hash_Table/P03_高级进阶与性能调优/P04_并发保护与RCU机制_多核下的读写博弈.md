...
RCU 的问题动机、硬件基础、读侧通知、宽限期和通用 API 已统一收拢到[RCU 专题](../../../synchronization/rcu/大纲.md)。如果尚不能解释“取消发布为什么不等于可以立即释放”，应先阅读[为什么需要 RCU](../../../synchronization/rcu/P01_为什么需要_RCU.md)到[Tree RCU 宽限期与回调机制](../../../synchronization/rcu/P12_Tree_RCU_GP请求与全局生命周期.md)。
...
这里使用 `kfree_rcu()` 的理由和内部 GP/回调推进过程属于通用机制，分别参见[宽限期与回调机制](../../../synchronization/rcu/P12_Tree_RCU_GP请求与全局生命周期.md)和[RCU API 速查](../../../synchronization/rcu/P03_RCU_通用API与最小使用闭环.md)。哈希表章节只需确认：节点已经取消发布，而且写侧锁已经释放，最终回收才进入 RCU 回调路径。
...
- [RCU、kref 与复合对象生命周期](../../../synchronization/rcu/P04_RCU_kref与复合对象生命周期.md)：跨读区持有对象和复合版本根的所有权模板。
