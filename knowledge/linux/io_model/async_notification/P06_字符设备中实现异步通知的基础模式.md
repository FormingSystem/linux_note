...
### 6.1.3\_驱动路径\_open\_/\_release\_/\_fasync\_/\_read\_/\_write(开发者视角)
...
### 6.1.4\_用户态\_配置\_F\_SETOWN\_/\_F\_SETSIG\_/\_F\_SETFL(用户视角)
...
### 6.1.5\_时序图\_从\_write()\_到\_SIGIO\_的完整链路(可视化)
...
## 6.2\_在\_open()\_/\_release()\_中维护\_fasync\_状态
...
### 6.2.3\_同一进程多次\_open()\_同一设备\_多\_struct\_file\_节点
...
### 6.2.4\_dup()\_/\_fork()\_的影响\_共享\_struct\_file\_vs\_复制\_struct\_file

这块稍微绕一点，但理解之后会对“fasync 行为为什么有时显得怪怪的”更有底。

#### (1)\_dup()\_多个\_fd\_共享同一个\_struct\_file
...
### 6.2.5\_open()\_/\_release()\_里\_该做什么\_/\_不该做什么
...
### 6.2.6\_一个稍微\_增强版\_的\_open/release\_模板
...
### 6.2.7\_小结\_open/release\_中对\_fasync\_的\_唯一刚性要求
...
### 6.3.1\_设计目标.fasync\_要\_薄\_只做一件事
...
### 6.3.2\_调用规律再复习\_on\_的取值与多次\_F\_SETFL
...
### 6.3.4\_fasync\_helper()\_的返回值与错误场景
...
#### (1)\_错误\_1\_在.fasync\_里手动维护链表\_不用\_fasync\_helper()
...
#### (2)\_错误\_2\_忽略\_on\_参数\_总是当做\_添加订阅
...
#### (3)\_错误\_3\_在.release()\_中忘记调用.fasync(...,\_0)
...
#### (4)\_错误\_4\_在.fasync\_中做复杂逻辑(加锁\_睡眠\_IO)
...
### 6.3.6\_多队列场景\_读写分队列\_/\_多通道\_fasync
...
## 6.4\_在中断处理函数/下半部中调用\_kill\_fasync()\_的流程
...
### 6.4.2\_从中断到\_SIGIO\_的完整数据流(可视化)
...
### 6.4.3\_在硬中断中调用\_kill\_fasync()\_可行但要严格控制

#### (1)\_kill\_fasync()\_是否可以在硬中断上下文调用
...
### 6.4.4\_在下半部调用\_kill\_fasync()\_tasklet\_/\_workqueue\_/\_threaded\_IRQ
...
### 6.4.5\_示例\_基于中断的\_demo\_async\_irq\_驱动骨架
...
### 6.4.6\_调试与验证\_从\_中断触发\_到\_用户\_SIGIO
...
### 6.4.7\_小结\_中断场景下\_kill\_fasync()\_的基本规范
...
### 6.5.2\_一个简化\_状态机\_data\_ready\_及其生命周期
...
### 6.5.4\_与\_waitqueue\_组合\_阻塞\_read\_+\_fasync\_的标准写法
...
#### (2)\_中断/事件路径\_统一唤醒\_waitqueue\_与\_fasync
...
#### (3)\_阻塞\_read\_wait\_event\_interruptible()
...
#### (4)\_.poll\_与\_data\_ready\_保持同一条件
...
### 6.5.5\_fasync\_与\_waitqueue\_的组合注意点
...
### 6.5.6\_与\_poll()\_的一致性\_POLLIN/POLLERR\_等语义对齐
...
## 6.6\_devm\_风格与非\_devm\_风格下资源/状态管理思路
...
### 6.6.1\_资源\_vs\_状态\_先把\_职责边界\_分清楚
...
### 6.6.2\_非\_devm\_风格\_手动\_request/free\_的典型模型
...
### 6.6.3\_devm\_风格\_让\_资源释放顺序\_与设备生命周期绑定
...
### 6.6.4\_fasync\_场景下使用\_devm\_的关键点

结合上面的层次划分，**devm 与 fasync 的互动点主要在“设备生命周期与文件生命周期的交叉”**。
 几个关键原则：

#### (1)\_保证\_设备还活着\_时\_file\_operations\_才可用
...
#### (2)\_不要把\_struct\_demo\_dev\_本身交给\_devm\_然后又在其他地方自己\_kfree
...
#### (3)\_IRQ\_使用\_devm\_request\_irq()\_时\_仍需注意\_IRQ\_回调访问\_dev
...
### 6.6.5\_对比\_非\_devm\_版\_vs\_devm\_版\_demo\_驱动骨架

为了对比直观，简单写两段“初始化/退出”伪代码（省略错误处理）：

#### (1)\_非\_devm\_版(手动资源管理)
...
### 6.6.6\_fasync\_+\_devm\_的推荐实践小结
...
## 6.7\_本章小结\_一个\_最小可用\_字符设备异步通知驱动长什么样
...
### 6.7.2\_一个\_最小可用\_fasync\_驱动\_的结构轮廓
