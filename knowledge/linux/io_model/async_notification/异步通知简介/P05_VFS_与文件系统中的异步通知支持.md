...
### 5.1.1_open()_f_flags_的初始化_但通常不触发_fasync
...
### 5.1.2_close()/release()_VFS_调用.release()_驱动负责清理_fasync
...
### 5.1.3_VFS_不关心_设备是什么_只关心_file_与_f_op_的约定
...
### 5.1.4_与_fcntl(F_SETFL)_的协作_FASYNC_从_VFS_到驱动
...
### 5.1.5_小结_VFS_在_open/close_中对_fasync_的_有限参与
...
## 5.2_fcntl(F_SETFL)_与_O_ASYNC/FASYNC_的交互
...
### 5.2.2_内核视角_fcntl()_to_setfl()_to.fasync
...
### 5.2.4_开发者视角.fasync_如何感知_打开/关闭_FASYNC
...
### 5.2.5_调试视角_如何验证.fasync_是不是因为_F_SETFL_被调用
...
### 5.2.6_不同文件类型上的_F_SETFL(O_ASYNC)_行为差异(预告)
...
### 5.2.7_小结_F_SETFL_是_fasync_的_真正开关
...
## 5.3_各类文件(常规文件_管道_socket_字符设备)的行为差异
...
### 5.4.3_.fasync_的调用时机_只在_FASYNC_位变化时
...
### 5.4.5_.fasync_与.poll_/.read_的时序关系(预热)
...
### 5.5.2_网络_socket_sock_fasync()_/_sock_wake_async()
...
### 5.5.3_管道/FIFO_读写两端各一队列_配合_buffer_状态
...
### 5.5.4_专用字符设备_input_evdev_misc_设备等
...
## 5.6_本章小结_从_VFS_视角看_fasync_是如何_插_进去的
...
### 5.6.3_从_VFS_视角看_fasync_是如何插进去的
