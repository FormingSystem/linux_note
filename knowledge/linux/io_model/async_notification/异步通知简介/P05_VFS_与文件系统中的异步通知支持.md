...
## 5.1\_VFS\_打开/关闭路径中\_FASYNC\_的传播机制
...
### 5.1.1\_open()\_f\_flags\_的初始化\_但通常不触发\_fasync
...
### 5.1.2\_close()/release()\_VFS\_调用.release()\_驱动负责清理\_fasync
...
### 5.1.3\_VFS\_不关心\_设备是什么\_只关心\_file\_与\_f\_op\_的约定
...
### 5.1.4\_与\_fcntl(F\_SETFL)\_的协作\_FASYNC\_从\_VFS\_到驱动
...
### 5.1.5\_小结\_VFS\_在\_open/close\_中对\_fasync\_的\_有限参与
...
## 5.2\_fcntl(F\_SETFL)\_与\_O\_ASYNC/FASYNC\_的交互
...
### 5.2.2\_内核视角\_fcntl()\_to\_setfl()\_to.fasync
...
### 5.2.3\_O\_ASYNC\_/\_FASYNC\_的转换关系
...
### 5.2.4\_开发者视角.fasync\_如何感知\_打开/关闭\_FASYNC
...
### 5.2.5\_调试视角\_如何验证.fasync\_是不是因为\_F\_SETFL\_被调用
...
### 5.2.6\_不同文件类型上的\_F\_SETFL(O\_ASYNC)\_行为差异(预告)
...
### 5.2.7\_小结\_F\_SETFL\_是\_fasync\_的\_真正开关
...
## 5.3\_各类文件(常规文件\_管道\_socket\_字符设备)的行为差异
...
### 5.3.2\_常规文件(普通文件)的\_fasync\_行为
...
### 5.3.3\_管道\_/\_FIFO\_读写两侧的\_fasync
...
### 5.3.4\_Socket\_网络栈中大量使用\_fasync
...
### 5.3.7\_小结\_fasync\_在不同文件类型上的\_存在感
...
### 5.4.1\_引入\_从\_cdev\_注册到.fasync\_生效
...
### 5.4.3\_.fasync\_的调用时机\_只在\_FASYNC\_位变化时
...
### 5.4.4\_字符设备最小骨架\_含.fasync\_的完整路径
...
### 5.4.5\_.fasync\_与.poll\_/.read\_的时序关系(预热)
...
### 5.5.1\_TTY/串口子系统\_典型的\_复合语义\_扩展
...
### 5.5.2\_网络\_socket\_sock\_fasync()\_/\_sock\_wake\_async()
...
### 5.5.3\_管道/FIFO\_读写两端各一队列\_配合\_buffer\_状态
...
### 5.5.4\_专用字符设备\_input\_evdev\_misc\_设备等
...
## 5.6\_本章小结\_从\_VFS\_视角看\_fasync\_是如何\_插\_进去的
...
### 5.6.1\_全路径回顾\_从\_open\_到\_close
...
### 5.6.3\_从\_VFS\_视角看\_fasync\_是如何插进去的
