...
### 9.1.2_fcntl(F_SETOWN)_/_F_SETSIG_/_F_SETFL(FASYNC)_的调用顺序

> 本小节按照你要求的固定结构：引入 → 数据结构视角 → 开发者视角 → 用户/平台视角 → 可视化 → 示例代码 → 调试与验证 → 小结。

#### (1)_引入_这三个调用_是什么_干什么_为什么要讲顺序
...
#### (2)_数据结构视角_owner_/_signum_/_f_flags_是怎么落到内核里的
...
#### (4)_用户/平台视角_在_i.MX6ULL_demo_异步驱动上的配置建议
...
#### (5)_可视化_从_open_到_FASYNC_生效的调用时序
...
### 9.1.3_使用_SIGIO_的经典写法(signal_handler_+_全局状态)

#### (1)_引入_最常见_也是最_教材式_的用法
...
### 9.1.4_使用_sigaction_的可靠信号处理模式

#### (1)_引入_为什么要从_signal()_升级到_sigaction()
...
#### (2)_数据结构视角_struct_sigaction_与_siginfo_t

先从数据结构看 `sigaction()` 到底在改什么。

##### 1)_struct_sigaction_的关键字段
...
#### (3)_开发者视角_基于_sigaction_的可靠_SIGIO_模式
...
#### (6)_示例代码_基于_sigaction_的改进版_SIGIO_处理
...
#### (8)_小结_sigaction_在_fasync_用户态链路中的位置
...
### 9.1.5_使用_signalfd_接收_SIGIO_并接入_epoll_事件循环

#### (1)_引入_为什么要把信号_转化_为_fd_事件
...
#### (2)_数据结构视角_signalfd_与_signalfd_siginfo
...
##### 2)_struct_signalfd_siginfo_的字段
...
#### (3)_开发者视角_从_SIGIO_到_signalfd_+_epoll_的步骤拆解
...
#### (5)_可视化_fasync_+_SIGIO_+_signalfd_+_epoll_的整体链路
...
#### (6)_示例代码_signalfd_+_epoll_版_demo
...
#### (7)_调试与验证_观察_signalfd_和_epoll_的行为
...
#### (8)_小结_signalfd_+_epoll_在本章中的定位
...
### 9.1.6_多线程/多进程下的所有权_fd_共享和信号路由问题

#### (1)_引入_单进程没问题_一上多线程/多进程就乱了
...
#### (2)_数据结构视角_谁是_owner_谁持有_fd_谁收到信号
...
##### 2)_struct_fown_struct_owner_是谁
...
#### (3)_开发者视角_在多线程/多进程中设计可靠的所有权与路由方案

下面从工程设计角度给出几条“硬规则”，你在写异步通知相关的用户态代码时可以直接采用。

##### 1)_规则一_多线程时_用_专门信号线程_简化路由
...
##### 2)_规则二_如确实需要_信号直接送到某个线程_才用_F_SETOWN_EX(F_OWNER_TID)
...
##### 3)_规则三_多进程时_不要让多个进程同时对同一_struct_file_开启_FASYNC
...
##### 4)_规则四_fork_+_exec_时重设_F_SETOWN_/_FASYNC
...
#### (6)_示例代码_多线程下_专门信号线程_+_signalfd_的骨架
...
### 9.1.7_用户态典型误用_顺序错误_信号未解掩_处理函数过重等

#### (1)_引入_机制本身不复杂_踩坑多在_用法细节
...
#### (3)_开发者视角_几种高频_坑_的具体形式与后果

下面从开发者角度，直接按问题形式列出几个最常见、最影响调试效率的错误模式。

##### 1)_典型误用一_FASYNC_配置顺序错误或被覆盖
...
##### 2)_典型误用二_信号未解掩/误掩_或与_signalfd_并存
...
##### 4)_典型误用四_与_poll/waitqueue_组合使用时未统一_事件语义
...
### 9.1.8_现代风格_的用户态事件循环(epoll_+_signalfd_+_fasync)的组合示例

#### (1)_引入_把前面所有内容收束成一个_可直接复用_的模板
