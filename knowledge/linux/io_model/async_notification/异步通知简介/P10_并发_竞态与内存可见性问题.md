...
## 10.1_fasync_相关状态(链表_标志位)的并发访问问题

### 10.1.1_引入_在_fasync_场景里_到底_谁在和谁抢东西
...
### 10.1.2_数据结构视角_哪些状态在_被抢

从内核数据结构角度看，一个典型的“简单按键 + fasync”字符设备，大致会牵涉以下几类共享状态。

#### (1)_struct_file_上的_FASYNC_标志
...
### 10.1.3_开发者视角_把并发访问问题拆成几个_必看场景

对于编驱动的人来说，与其死记各种“抽象术语”，不如明确记下几个**必须显式考虑并发的场景**。
 围绕 fasync，最常见可以归纳为四类：

#### (1)_场景_1.fasync_与_kill_fasync()_并发操作链表
...
#### (2)_场景_2_事件标志位在中断与.read()_/.poll()_之间的同步
...
#### (3)_场景_3_设备关闭_/_模块卸载_时与_kill_fasync()_的竞态
...
#### (4)_场景_4_多_open_实例_多个_struct_file_对同一设备的并发
...
### 10.1.4_用户/平台视角_这些并发问题对_上层程序_的影响
...
### 10.1.5_可视化_fasync_状态在不同上下文中的访问关系
...
### 10.1.6_示例代码_一个_带锁的最小_fasync_模式骨架_(并发友好版)
...
### 10.1.8_小结_把_状态并发访问_当作后续章节的基础图层
...
## 10.2_中断上下文_vs_进程上下文_锁的选择(spinlock/mutex)

### 10.2.1_引入_同一块状态_在_两个世界_里被同时访问
...
### 10.2.2_语义与约束_spinlock_与_mutex_在_fasync_场景里的_硬边界
...
### 10.2.3_开发者视角_在_fasync_驱动中制定完整的锁策略
...
#### (1)_方案_A_spinlock_统一保护_async_queue_+_event_pending(中断内直接_kill_fasync)
...
#### (2)_方案_B_ISR_只置位标志_+_调度_workqueue_fasync_在_workqueue_中完成(允许使用_mutex)
...
### 10.2.5_可视化_两种锁方案的结构与时序对比

先用结构图对比方案 A 与方案 B。

#### (1)_方案_A_spinlock_+_中断中直接_kill_fasync
...
#### (2)_方案_B_ISR_只置位_+_调度_workqueue_在_workqueue_中_kill_fasync
...
### 10.2.6_示例代码_两个锁方案的对照实现骨架

下面给出两个简化版本骨架代码，对比同一设备在两种锁方案下的写法差异。

#### (1)_方案_A_自旋锁_+_中断中直接_kill_fasync
...
#### (2)_方案_B_workqueue_+_mutex_fasync_在_work_里执行
...
### 10.2.8_小结_把_锁的选择_固化成_fasync_驱动设计规则
...
## 10.3_kill_fasync()_与进程信号队列的并发行为

### 10.3.1_引入_从_链表并发_走到_信号队列并发
...
### 10.3.2_数据结构视角_kill_fasync()_穿过了哪些层
...
### 10.3.3_开发者视角_kill_fasync()_在并发下的行为特征

站在驱动开发者视角，有几个问题特别关键。

#### (1)_多_CPU_同时调用_kill_fasync()_会不会_把队列搞坏
...
#### (2)_多次连续_kill_fasync()_事件是_合并_还是_排队
...
### 10.3.4_用户/平台视角_kill_fasync()_并发行为在上层表现出来的_症状
...
### 10.3.5_可视化_kill_fasync()_与信号队列的并发调用场景

用两个图来帮助建立直观认识。

#### (1)_多_CPU_并发_kill_fasync()_的结构示意
...
#### (2)_多次_kill_fasync()_合并为一个_SIGIO_的时序示意
...
### 10.3.6_示例代码_从伪代码角度理解_kill_fasync()_的工作流程
...
### 10.3.7_调试与验证_如何从_信号队列视角_检查_kill_fasync()_的并发行为
...
### 10.3.8_小结_把_kill_fasync()_看成_对信号队列的一次写操作_而不是_发出_N_个信号
...
## 10.4_fasync_与_waitqueue_/_poll_的竞态_谁先感知_如何保持顺序语义

### 10.4.1_引入_同一事件被_多个通知机制_消费时的顺序问题
...
### 10.4.2_数据结构视角_三个通知相关元素的_单一事实来源
...
#### (2)_.read()_/.poll()_与_ISR_的竞态_如何用状态机避免_睡死/重复
...
#### (3)_fasync_与_waitqueue/poll_的顺序_不能依赖_谁先唤醒
...
### 10.4.4_用户/平台视角_怎样在用户态_容忍并利用_这些竞态
...
### 10.4.5_可视化_在时间轴上看_3_条路径的竞态关系

#### (1)_时间线_一次_事件_+_两次通知_+_多种消费_的可能顺序
...
### 10.4.6_示例代码_一个_统一状态_+_限制通知频率_的组合模板
...
### 10.4.8_小结_把_fasync_与_waitqueue/poll_视为_多个观察者_共享一个状态机
...
## 10.5_fasync_相关状态的释放与_悬垂_fasync_struct_问题

### 10.5.1_引入_为什么_关闭/卸载_阶段最容易把_fasync_搞崩
...
#### (2)_设备/驱动释放路径_remove()_/_module_exit()_的正确顺序
...
#### (3)_devres_vs_非_devres_谁帮你释放_谁帮不了你管并发
...
### 10.5.4_用户/平台视角_悬垂_fasync_struct_的典型症状
...
### 10.5.5_可视化_安全释放顺序与悬垂场景示意

#### (1)_安全释放顺序(非_devres_版本)
...
### 10.5.6_示例代码_带关闭序列的最小_fasync_驱动骨架(非_devres_+_devres_对照)
...
### 10.5.7_调试与验证_专门针对_释放阶段_BUG_的检查方法
...
### 10.5.8_小结_把_fasync_关闭序列_当成驱动设计的一等公民
...
## 10.6_内存屏障_原子操作在_fasync_实现中的角色

### 10.6.1_引入_锁解决_互斥_屏障解决_顺序与可见性
...
### 10.6.2_数据结构视角_哪些字段需要考虑_跨_CPU_可见性
...
#### (3)_模式_C_混合模式(读无锁_写加锁)与_READ_ONCE_/_WRITE_ONCE
...
### 10.6.5_可视化_在时间轴上理解_写-通知-读_的内存顺序
...
### 10.6.6_示例代码_两种模式的对比实现

#### (1)_示例一_纯锁模式(推荐实践_内存屏障由锁隐含)
...
### 10.6.8_小结_把_内存模型_落回到_fasync_的三条设计原则
...
## 10.7_实战中值得显式考虑的竞态场景清单

### 10.7.1_引入_从_理解机制_到_可执行的核对表
...
### 10.7.3_场景_1_中断_vs.fasync()_/_kill_fasync()_链表修改竞态
...
### 10.7.4_场景_2_中断_vs_read_/_poll_vs_事件状态变量竞态
...
### 10.7.5_场景_3_多线程/多进程对_F_SETOWN_/_F_SETSIG_/_F_SETFL_的竞态
...
### 10.7.8_场景_6_错误路径与异常恢复中的_fasync_状态
...
### 10.7.9_统一_Checklist_表_驱动并发自检表(fasync_维度)
