...
## 11.1_已经设置_FASYNC_却收不到_SIGIO_的排查步骤

### 11.1.1_引入_一个最常见_也最容易_甩锅给内核_的问题
...
### 11.1.2_数据结构视角_SIGIO_要_走通_必须满足的条件集合
...
### 11.1.3_开发者视角_驱动侧必须先自查的几个点(静态_+_动态)
...
### 11.1.5_可视化_一张_收不到_SIGIO_时_的排查流程图
...
### 11.1.7_调试与验证_按层使用_strace_/_ftrace_/_/proc_的组合拳
...
### 11.1.8_小结_把_收不到_SIGIO_拆回可验证的几个前提
...
## 11.2_查看进程状态_/proc/PID/status_/proc/PID/fd_等

### 11.2.1_引入_为什么先看_/proc_再怀疑代码
...
### 11.2.2_数据结构视角_从_/proc_行文本到内核结构的映射

#### (1)_/proc/PID/status_task_struct_+_signal_struct_的_摘要
...
#### (2)_/proc/PID/fd_与_/proc/PID/fdinfo/FD_files_struct_to_file_to_inode
...
### 11.2.3_开发者视角_驱动调试时看_/proc_的_标准动作
...
#### (2)_确认该_fd_上_O_ASYNC/FASYNC_是否打开
...
### 11.2.4_用户/平台视角_给应用工程师的一套_自检工具箱
...
### 11.2.5_可视化_从_PID_到_fd_再到_file/fasync_struct_的路径
...
### 11.2.6_示例代码_一个小工具_自动解析_O_ASYNC_/_信号状态
...
### 11.2.7_调试与验证_结合_11.1_的排查步骤使用_/proc
...
### 11.2.8_小结_把_/proc_当成_fasync_调试的_状态仪表盘
...
## 11.3_使用_strace_观察_fcntl/信号行为

### 11.3.1_引入_先看_系统调用层面_再去怀疑驱动
...
### 11.3.3_开发者视角_fasync/SIGIO_时_strace_应该看什么
...
#### (2)_重点观察点_1_fcntl(F_SETOWN/F_SETFL/F_SETSIG)
...
#### (3)_重点观察点_2_rt_sigaction_/_sigprocmask
...
### 11.3.4_用户/平台视角_几类_典型错误模式_的_strace_特征
...
### 11.3.5_可视化_strace_在_fasync_调试中的位置
...
### 11.3.6_示例_用_strace_走一遍完整的_正确工作路径
...
### 11.3.7_调试与验证_strace_与_/proc_ftrace_的组合使用建议
...
### 11.3.8_小结_把_strace_当作_fasync_协议层_的抓包工具
...
## 11.4_使用_ftrace/trace_printk_定位.fasync_/_kill_fasync()_调用

### 11.4.1_引入_当_strace_和_/proc_都说_正常_但还是出问题时
...
### 11.4.2_数据结构与机制_ftrace_对函数调用的_钩子_视角

#### (1)_ftrace_的核心_函数入口/退出上的_trace_钩子
...
#### (2)_trace_printk_在_trace_缓冲区里插入你自己的日志
...
### 11.4.3_开发者视角_为_fasync_路径配置_ftrace_的基本步骤

以下以典型的 debugfs 路径 `/sys/kernel/debug/tracing` 为例（需要 root）：

#### (1)_开启_function_tracer_并限制到关注的函数
...
### 11.4.5_典型使用场景_ftrace_在_fasync_调试中的_必查三问

结合实战，ftrace/trace_printk 最常被用来回答下面三类问题：

#### (1)_问题_1.fasync_有被调用吗_被谁调用
...
#### (2)_问题_2_每次事件发生时_kill_fasync_有没有被调用
...
### 11.4.6_示例_一步步_trace_一个_demo_async_驱动
...
### 11.4.7_调试与验证_使用_ftrace/trace_printk_时的注意事项
...
### 11.4.8_小结_ftrace_=_让_fasync_变成_可观测系统_的最后一环
...
## 11.5_信号风暴_丢信号_重复通知的典型症状与定位方法

### 11.5.1_引入_三个现象_三个维度的问题
...
### 11.5.2_数据结构视角_信号队列_+_事件计数的天然_语义差异
...
### 11.5.3_开发者视角_每种现象在驱动中的典型成因

#### (1)_信号风暴_电平式_kill_fasync()_+_高频事件
...
### 11.5.4_用户/平台视角_从_期望协议_角度调整用户态设计
...
### 11.5.6_示例代码_限流/聚合后的_健康版_fasync_驱动事件路径
...
### 11.5.7_调试与验证_如何判断_风暴/丢/重复_的具体归属
...
### 11.5.8_小结_把_奇怪的信号行为_还原为_明确的语义_+_状态机设计
...
## 11.6_延迟测量_从设备事件到用户态处理的时间链路分析

### 11.6.1_引入_只看_能不能收到信号_是不够的
...
### 11.6.4_用户/平台视角_信号_handler_/_signalfd_内的时间戳
...
### 11.6.6_示例_一个简单的_ioctl_接口导出内核时间戳
...
## 11.7_性能优化方向_事件聚合_节流_分级通知策略

### 11.7.1_引入_在正确性之外追求_便宜而稳定_的通知
...
### 11.7.2_数据结构视角_从_单一计数_到_分维度的事件状态
...
### 11.7.8_小结_fasync_性能优化_=_通知策略设计问题
