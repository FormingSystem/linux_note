...
## 7.1\_适合使用\_fasync\_的中断型设备特征

### 7.1.1\_引入\_不是所有中断驱动设备都适合\_fasync
...
### 7.1.2\_数据结构视角\_什么样的中断事件模型适合挂在\_async\_queue\_上
...
### 7.1.3\_开发者视角\_什么时候你应该给中断型设备加上\_fasync
...
### 7.1.4\_用户/平台视角\_以\_i.MX6ULL\_GPIO\_按键为例
...
### 7.1.5\_可视化\_中断驱动设备接入\_fasync\_的结构图
...
### 7.1.6\_示例代码骨架\_典型\_适合用\_fasync\_的中断型设备\_抽象
...
### 7.1.7\_调试与验证\_如何确认\_这个中断型设备真的适合\_fasync
...
### 7.1.8\_小结\_中断型设备适配\_fasync\_的\_特征清单
...
## 7.2\_GPIO\_按键\_/\_外部事件输入的异步通知设计
...
### 7.2.1\_引入\_从抽象中断事件到\_GPIO\_按键/外部事件输入
...
### 7.2.2\_数据结构视角\_从设备树到\_struct\_demo\_dev
...
### 7.2.4\_用户/平台视角\_对用户态暴露怎样的接口才合理
...
### 7.2.5\_可视化\_以\_i.MX6ULL\_GPIO\_为例的完整路径图
...
### 7.2.6\_示例代码片段\_设计层面需要的关键函数形态

这里不写完整驱动，只给出 **设计层面关键函数原型与核心逻辑片段**，后面第 7.7 会整合为完整可编译版本。

#### (1)\_probe\_解析\_DTS\_并初始化设备
...
#### (2)\_中断\_handler\_从\_GPIO\_到事件状态再到通知
...
### 7.2.8\_小结\_从\_抽象中断事件\_到\_i.MX6ULL\_GPIO\_+\_fasync\_设计蓝图
...
## 7.3\_中断上下文\_tasklet\_workqueue\_与\_kill\_fasync()\_的时序选择
...
### 7.3.1\_引入\_同一个\_GPIO\_事件\_可能有多种\_通知时机
...
### 7.3.2\_数据结构视角\_哪些状态必须在\_同一个上下文\_里完成
...
### 7.3.3\_方案一\_在硬中断上下文中直接调用\_kill\_fasync()
...
### 7.3.4\_方案二\_使用\_threaded\_IRQ\_在线程\_handler\_中\_kill\_fasync()
...
### 7.3.5\_方案三\_硬中断中调度\_workqueue\_在工作函数中\_kill\_fasync()
...
#### (2)\_threaded\_IRQ\_/\_workqueue\_模式
...
### 7.3.8\_小结\_以\_GPIO\_场景为例的\_时序选择\_结论
...
### 7.4.1\_引入\_电平抖动\_+\_信号风暴\_+\_事件丢失
...
### 7.4.4\_用户/平台视角\_对应用暴露怎样的行为是合理的
...
### 7.4.5\_可视化\_两种去抖策略的时间线

#### (1)\_简单时间窗去抖(基于\_jiffies)
...
### 7.4.6\_示例代码\_两种去抖方案的具体实现

下面给出两个可直接使用的内核代码片段。
 注意：仅展示与去抖、`kill_fasync()` 时机相关的部分，完整驱动会在 7.7 整合。

#### (1)\_方案一\_中断内基于时间窗的简单去抖/节流
...
#### (2)\_方案二\_基于\_delayed\_work\_的严格去抖与事件聚合
...
### 7.4.8\_小结\_去抖/节流/聚合与\_fasync\_的关系
...
## 7.5\_与\_waitqueue\_/\_completion\_/\_poll\_的组合使用模式
...
### 7.5.1\_引入\_多个\_等待原语\_必须围绕同一套状态工作
...
### 7.5.3\_模式一\_仅\_waitqueue\_+\_阻塞\_read()\_的基线模型
...
### 7.5.4\_模式二\_在基线上加入\_poll(waitqueue\_+\_poll)
...
### 7.5.5\_模式三\_waitqueue\_+\_poll\_+\_fasync\_三合一(推荐组合)
...
### 7.5.6\_模式四\_completion\_在\_命令完成\_场景中的辅助角色
...
#### (3)\_模式\_C\_SIGIO\_+\_signalfd\_+\_epoll
...
### 7.5.8\_小结\_统一状态\_多种等待原语的\_安全组合
...
## 7.6\_典型错误模式\_在错误上下文调用\_kill\_fasync()\_重复通知\_丢事件
...
### 7.6.2\_数据结构视角\_错误模式的本质都是\_状态与通知脱节
...
### 7.6.3\_错误模式一\_在错误上下文调用\_kill\_fasync()
...
#### (2)\_错误写法示例\_remove\_之后还在中断中\_kill\_fasync()
...
#### (2)\_错误写法示例\_对同一个状态反复\_kill\_fasync()
...
#### (3)\_正确写法要点\_只在\_状态从\_0\_to\_1\_时发通知
...
#### (2)\_错误写法示例\_1\_先\_kill\_fasync()\_再更新状态
...
#### (3)\_错误写法示例\_2\_错误清理\_data\_ready\_导致覆盖或丢失
...
### 7.6.6\_错误模式四\_fasync\_订阅关系维护不当
...
### 7.6.7\_小结\_fasync\_相关错误模式\_checklist
...
## 7.7\_案例\_基于\_i.MX6ULL\_GPIO\_中断的\_demo\_异步通知驱动结构
...
### 7.7.2\_数据结构与\_DTS\_视角\_以\_demo\_led\_key\_int@0\_节点为基础
...
### 7.7.6\_示例代码\_devm\_版本\_demo\_驱动(可直接编译)
...
### 7.7.7\_示例代码\_用户态\_SIGIO\_+\_poll\_测试程序
...
### 7.7.9\_小结\_将\_fasync\_理论转化为可运行\_demo
