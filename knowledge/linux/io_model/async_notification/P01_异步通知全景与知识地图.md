---
id: knowledge.linux.io_model.async_notification.异步通知简介.p01_异步通知全景与知识地图
title: "异步通知全景与知识地图"
kind: mechanism
status: evolving
domains:
...
### 1.1.4\_用户\_/\_平台视角\_假定读者基础与目标平台
...
### 1.1.5\_可视化\_本书知识地图(Mermaid\_结构图)
...
### 1.1.6\_示例代码\_全书统一采用的最小\_fasync\_模板骨架
...
## 1.2\_Linux\_中的多种\_通知机制\_概览

### 1.2.1\_引入\_通知机制在\_I/O\_子系统中的位置
...
### 1.2.4\_用户\_/\_平台视角\_API\_使用方式与适用场景
...
#### (3)\_fasync\_+\_SIGIO\_模式(只展示最小骨架)
...
### 1.3.2\_数据结构视角\_fasync\_的位置\_fd\_与信号之间的一条支路
...
### 1.3.3\_开发者视角\_哪类驱动\_值得\_做\_fasync\_哪类不必要
...
### 1.3.4\_用户\_/\_平台视角\_fasync\_的限制条件
...
### 1.3.5\_可视化\_fasync\_在通知通路中的分支位置
...
### 1.3.6\_示例代码\_同一驱动\_有/无\_fasync\_改造的对比点

下面给出一个极简对比片段，强调“加 fasync 不改变原有 `read/poll` 路径”。

#### (1)\_原始版本\_只支持\_read\_+\_poll
...
### 1.3.7\_调试与验证视角\_如何确认问题在\_fasync\_边界内
...
### 1.3.8\_小结\_fasync\_的\_定位与边界\_规则
...
## 1.4\_本书的假定前提(内核版本\_平台\_读者基础)

### 1.4.1\_引入\_为什么必须先约束前提
...
### 1.4.4\_用户\_/\_平台视角\_系统环境与部署方式假定
...
### 1.5.2\_数据结构视角\_知识依赖的\_图结构\_而非线性列表
...
### 1.5.7\_调试与验证视角\_如何评估自己当前适合哪条路径
