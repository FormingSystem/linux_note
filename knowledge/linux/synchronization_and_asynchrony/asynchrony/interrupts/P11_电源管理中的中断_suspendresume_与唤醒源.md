...
## 11.1\_电源管理中的中断\_suspend/resume\_与唤醒源
...
本章的 Linux 唤醒策略与控制器硬件能否接收、保持和递送请求需要分别核对。GIC（Generic Interrupt Controller，通用中断控制器）自身的初始化、每个处理器的接收条件和电源交接由[接收侧电源交接章节](../../../../../platforms/arm/architecture/gic/P07_目标选择_核间通知与电源交接.md#7.5_目标准备下线时_要交接的不只是路由)解释，RK3588 集成另见[电源域记录](../../../../../platforms/arm/rockchip/rk3588/P01_RK3588的GIC-600集成与手册核对.md#1.4_电源域解释了为什么普通中断与消息路径可能不同)，实际固件配置仍需目标环境核对；本章继续承担驱动侧的休眠与唤醒使用边界，不能只凭一个 Linux 标志位推断板上唤醒路径成立。



------

### 11.1.2\_suspend/resume\_下的中断生存期问题
...
### 11.1.3\_唤醒源(wakeup\_source)与中断的绑定方式
...
#### (4)\_能唤醒\_与\_IRQF\_NO\_SUSPEND\_的差别
...
### 11.1.4\_GPIO\_中断唤醒与\_pinctrl-sleep\_的协同

#### (1)\_为何\_GPIO\_中断能用\_但不能唤醒
...
#### (2)\_active\_/\_sleep\_两套\_pin\_配置的作用
...
#### (3)\_唤醒用的\_GPIO\_必须在\_sleep\_状态下仍然保持为\_能触发中断的输入
...
### 11.1.5\_runtime\_PM\_场景下的中断可用性

#### (1)\_runtime\_PM\_与系统\_suspend\_的区别
...
#### (4)\_runtime\_suspend\_正常\_system\_suspend\_不正常
