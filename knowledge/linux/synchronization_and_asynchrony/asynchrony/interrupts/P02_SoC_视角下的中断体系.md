...
下面出现的 GIC（Generic Interrupt Controller，通用中断控制器）是组织硬件请求并向处理器递送中断的控制器架构名称。本章只建立它在芯片内的位置，为 Linux 软件抽象铺垫；若要从零学习该控制器本身，另从 [GICv3 独立专题](../../../../../platforms/arm/architecture/gic/大纲.md#1.2_因果阅读地图)进入。该专题先学习 RK3588 的 GICv3.0，不要求先理解平台或代际差异。
...
## 2.3\_多源中断问题\_GPIO\_也能发中断之后\_一切都变复杂了
...
上一篇：[P01 中断的定位与演化](P01_中断的定位与演化.md)。
下一篇：[P03 Linux 的中断抽象 irq chip irq domain 与 irq desc](P03_Linux_的中断抽象_irq_chip_irq_domain_与_irq_desc.md)。
