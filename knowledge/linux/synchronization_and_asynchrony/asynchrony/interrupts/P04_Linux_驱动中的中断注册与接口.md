...
下面用一次设备完成事件把这些对象串成完整时序。设备寄存器保存事件原因，中断控制器保存 pending/active 状态，`irq_desc` 与 `irqaction` 保存 Linux 分发状态；上半部确认设备事件后，才把耗时工作交给线程化处理或工作队列。若设备状态尚未清除就过早解除屏蔽，电平中断会再次进入，而不是凭空“多来一次”。

```mermaid
sequenceDiagram
    participant Device as 设备状态寄存器
    participant Controller as irq_chip/中断控制器
    participant Entry as CPU异常入口
    participant Desc as irq_desc/irqaction
    participant Handler as 驱动上半部
    participant Deferred as IRQ线程或工作队列

    Device->>Device: 完成事件并置位状态位
    Device->>Controller: 拉高电平或发送边沿/MSI
    Controller->>Controller: 记录pending并选择目标CPU
    Controller->>Entry: 投递IRQ
    Entry->>Controller: 读取并确认硬件IRQ
    Entry->>Desc: 映射Linux IRQ并进入通用分发
    Desc->>Handler: 调用已登记的irqaction
    Handler->>Device: 读取原因并清除/确认设备状态
    alt 只需短处理
        Handler-->>Desc: 返回IRQ_HANDLED
    else 需要可睡或耗时处理
        Handler->>Deferred: 唤醒IRQ线程或投递work
        Handler-->>Desc: 返回IRQ_WAKE_THREAD或IRQ_HANDLED
        Deferred->>Device: 在可调度上下文完成后续处理
    end
    Desc->>Controller: EOI并按策略解除屏蔽
    Controller-->>Entry: pending清除后允许返回原上下文
```
...
上一篇：[P03 Linux 的中断抽象 irq chip irq domain 与 irq desc](P03_Linux_的中断抽象_irq_chip_irq_domain_与_irq_desc.md)。
下一篇：[P05 GPIO 与触发语义 电平 边沿 DTS 与 只来一次一直来 为何出现](P05_GPIO_与触发语义_电平_边沿_DTS_与_只来一次一直来_为何出现.md)。
