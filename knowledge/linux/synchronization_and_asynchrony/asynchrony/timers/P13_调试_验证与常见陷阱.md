...
这一节把“回调执行晚了”与“时间基准本身发生跳变或失稳”分开，先依据到期点采用的时间语义缩小问题范围。
...
- 对 `timer_list`：区分 jiffies 量化、timer slack/deferrable 属性、到期 IRQ 与 softirq 执行延迟；NO_HZ 会省略中间的无用周期 tick，但普通非 deferrable timer 仍应被纳入下一个 clockevent，不能只凭“空闲时停 tick”解释迟到。
