...
```mermaid
flowchart LR
    "DEV"["DEV: demo_async_dev<br/>(事件状态: event_count 等)"]
    "WQ"["WQ: wait_queue_head_t<br/>(dev->wait)"]
    "FAS"["FAS: fasync_struct 链表<br/>(dev->async_queue)"]

    "FOPS"["file_operations<br/>(.read/.poll/.fasync)"]

    "SYS_SELECT"["内核: sys_select/sys_pselect"]
    "SYS_POLL"["内核: sys_poll/ppoll"]
    "SYS_EPOLL"["内核: epoll_wait / eventpoll"]
    "SYS_SIG"["内核: 信号子系统<br/>(send_sigio, pending 队列)"]
...
flowchart LR
    ...
    subgraph "路径 A: 裸 fasync 字符设备"
        "DRV_A"["驱动A: demo_async_key<br/>(字符设备 + fasync)"]
        "CDEV"["/dev/demo_async_key"]
        "APP_A"["用户应用A<br/>(自定义协议 + SIGIO)"]
    end

    subgraph "路径 B: input/evdev"
        "DRV_B"["驱动B: demo_input_key<br/>(struct input_dev)"]
        "EVDEV"["通用驱动: evdev<br/>(/dev/input/eventX)"]
        "APP_B1"["用户应用B1<br/>(evdev 专用工具/库)"]
        "APP_B2"["系统组件B2<br/>(libinput/桌面系统)"]
    end
...
flowchart LR
    ...
    subgraph "数据面: 数据收发通知"
        "DRV_DATA"["DRV_DATA: 驱动数据路径<br/>(NAPI / 驱动 RX/TX)"]
        "CHAR"["/dev/my_nic_data<br/>(假设有一个字符接口)"]
        "APP_DATA"["APP_DATA: 用户态数据消费者<br/>(业务进程)"]
    end

    subgraph "控制面: 链路/状态变化通知"
        "DRV_CTRL"["DRV_CTRL: 驱动控制路径<br/>(link up/down, carrier)"]
        "NETLINK"["NETLINK_ROUTE<br/>(内核 netlink socket)"]
        "UEVENT"["kobject_uevent<br/>(设备 ADD/REMOVE/CHANGE)"]
        "SYSFS"["/sys/class/net/eth0/*<br/>(状态/属性文件)"]
...
```mermaid
flowchart LR
    "DEV"["DEV: demo_async_dev<br/>(.read/.poll/.fasync)"]
    "FD_DEV"["FD_DEV: /dev/demo_async"]
    "FD_SIG"["FD_SIG: signalfd(SIGIO)"]
    "URING"["io_uring ctx<br/>(SQ/CQ)"]

    "APP"["APP: 用户程序<br/>(统一事件循环)"]
...
flowchart TD
    ...
    "USE_INPUT"["选择 input 子系统 + evdev<br/>(驱动: input_report_*, 上层: /dev/input/eventX)"]
    "GEN_CHAR"["通用字符设备 + .read/.poll<br/>(事件=自定义结构/流)"]

    "Q3"["Q3: 事件消费者主要是系统守护进程/管理进程?"]
    "USE_NETLINK"["控制面通知: netlink/kobject_uevent<br/>+ sysfs 属性 (低频)"]
    "NO_NETLINK"["无需系统级广播, 可留在字符设备/私有协议中"]

    "Q4"["Q4: fd 数量少, 事件稀疏, 需要被\"推一把\"?"]
    "ADD_FASYNC"["在 .poll 基础上增加 fasync + SIGIO/signalfd<br/>(少量 fd, 便于维护)"]
    "ONLY_POLL"["只实现 .poll, 用 select/poll/epoll/io_uring<br/>(不引入信号复杂度)"]

    "Q5"["Q5: 是否需要大规模 fd + 高频 I/O?"]
    "USE_IOURING"["应用层使用 io_uring<br/>(驱动只需保证 .poll/.read 行为规范)"]
