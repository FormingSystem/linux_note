---
id: knowledge.linux.io_model.async_notification.异步通知简介.p10_并发_竞态与内存可见性问题
title: "并发 竞态与内存可见性问题"
kind: mechanism
status: evolving
domains:
...
## 10.1\_fasync\_相关状态(链表\_标志位)的并发访问问题

### 10.1.1\_引入\_在\_fasync\_场景里\_到底\_谁在和谁抢东西
...
### 10.1.2\_数据结构视角\_哪些状态在\_被抢

从内核数据结构角度看，一个典型的“简单按键 + fasync”字符设备，大致会牵涉以下几类共享状态。

#### (1)\_struct\_file\_上的\_FASYNC\_标志
...
### 10.1.3\_开发者视角\_把并发访问问题拆成几个\_必看场景

对于编驱动的人来说，与其死记各种“抽象术语”，不如明确记下几个**必须显式考虑并发的场景**。
 围绕 fasync，最常见可以归纳为四类：

#### (1)\_场景\_1.fasync\_与\_kill\_fasync()\_并发操作链表
...
#### (2)\_场景\_2\_事件标志位在中断与.read()\_/.poll()\_之间的同步
...
#### (3)\_场景\_3\_设备关闭\_/\_模块卸载\_时与\_kill\_fasync()\_的竞态
...
#### (4)\_场景\_4\_多\_open\_实例\_多个\_struct\_file\_对同一设备的并发
...
### 10.1.4\_用户/平台视角\_这些并发问题对\_上层程序\_的影响
...
### 10.1.5\_可视化\_fasync\_状态在不同上下文中的访问关系
...
flowchart TB
    ...
    "OPEN"["OPEN: open()/fcntl()<br/>调用 .fasync"]
    "REL"["REL: close()/release()<br/>调用 .fasync 清理"]
    "READ"["READ: .read()"]
    "POLL"["POLL: .poll()"]

    "ASQ"["状态1: dev->async_queue<br/>(struct fasync_struct 链表)"]
    "EVP"["状态2: dev->event_pending<br/>(事件标志)"]
    "FFL"["状态3: file->f_flags<br/>(FASYNC 位)"]

    "OPEN" -->|"新增/修改 fasync 节点"| "ASQ"
    "REL" -->|"删除 fasync 节点<br/>清理 async_queue"| "ASQ"
...
### 10.1.6\_示例代码\_一个\_带锁的最小\_fasync\_模式骨架\_(并发友好版)
...
### 10.1.7\_调试与验证\_如何判断\_锁和状态\_设计是否正确
...
### 10.1.8\_小结\_把\_状态并发访问\_当作后续章节的基础图层
...
## 10.2\_中断上下文\_vs\_进程上下文\_锁的选择(spinlock/mutex)

### 10.2.1\_引入\_同一块状态\_在\_两个世界\_里被同时访问
...
### 10.2.2\_语义与约束\_spinlock\_与\_mutex\_在\_fasync\_场景里的\_硬边界
...
### 10.2.3\_开发者视角\_在\_fasync\_驱动中制定完整的锁策略
...
#### (1)\_方案\_A\_spinlock\_统一保护\_async\_queue\_+\_event\_pending(中断内直接\_kill\_fasync)
...
#### (2)\_方案\_B\_ISR\_只置位标志\_+\_调度\_workqueue\_fasync\_在\_workqueue\_中完成(允许使用\_mutex)
...
### 10.2.4\_用户/平台视角\_锁选错了会对系统造成什么影响
...
### 10.2.5\_可视化\_两种锁方案的结构与时序对比

先用结构图对比方案 A 与方案 B。

#### (1)\_方案\_A\_spinlock\_+\_中断中直接\_kill\_fasync

```mermaid
flowchart LR
    "HW"["HW: 设备/按键"]
    "ISR"["ISR: 中断处理函数<br/>(持 dev->lock 自旋锁)"]
    "DEV"["DEV: demo_async_dev<br/>{event_pending, async_queue}"]
    "WQ"["WAITQ: dev->wait"]
    "FAS"["FAS: dev->async_queue<br/>(fasync 链表)"]
    "SIG"["SIG: 信号子系统"]
    "UP"["UP: 用户态进程"]

    "HW" -->|"中断"| "ISR"
    "ISR" -->|"dev->event_pending=1<br/>wake_up_interruptible()"| "WQ"
    "ISR" -->|"kill_fasync(..., SIGIO)"| "FAS"
    "FAS" --> "SIG"
    "SIG" -->|"SIGIO"| "UP"
```

#### (2)\_方案\_B\_ISR\_只置位\_+\_调度\_workqueue\_在\_workqueue\_中\_kill\_fasync

```mermaid
flowchart LR
    "HW"["HW: 设备/按键"]
    "ISR"["ISR: 中断处理函数<br/>(仅置位 event_pending)"]
    "WQK"["WQK: schedule_work()<br/>调度 event_work"]
    "WORK"["WORK: event_work<br/>(持 dev->lock mutex)"]
    "DEV"["DEV: demo_async_dev<br/>{event_pending, async_queue}"]
    ...
    "WQK" --> "WORK"
    "WORK" -->|"检查 event_pending<br/>wake_up_interruptible()"| "WAITQ"
...
### 10.2.6\_示例代码\_两个锁方案的对照实现骨架

下面给出两个简化版本骨架代码，对比同一设备在两种锁方案下的写法差异。

#### (1)\_方案\_A\_自旋锁\_+\_中断中直接\_kill\_fasync
...
#### (2)\_方案\_B\_workqueue\_+\_mutex\_fasync\_在\_work\_里执行
...
### 10.2.7\_调试与验证\_如何确认锁选择与上下文使用正确
...
### 10.2.8\_小结\_把\_锁的选择\_固化成\_fasync\_驱动设计规则
...
## 10.3\_kill\_fasync()\_与进程信号队列的并发行为

### 10.3.1\_引入\_从\_链表并发\_走到\_信号队列并发
...
### 10.3.2\_数据结构视角\_kill\_fasync()\_穿过了哪些层
...
### 10.3.3\_开发者视角\_kill\_fasync()\_在并发下的行为特征

站在驱动开发者视角，有几个问题特别关键。

#### (1)\_多\_CPU\_同时调用\_kill\_fasync()\_会不会\_把队列搞坏
...
#### (2)\_多次连续\_kill\_fasync()\_事件是\_合并\_还是\_排队
...
### 10.3.4\_用户/平台视角\_kill\_fasync()\_并发行为在上层表现出来的\_症状
...
### 10.3.5\_可视化\_kill\_fasync()\_与信号队列的并发调用场景

用两个图来帮助建立直观认识。

#### (1)\_多\_CPU\_并发\_kill\_fasync()\_的结构示意
...
#### (2)\_多次\_kill\_fasync()\_合并为一个\_SIGIO\_的时序示意
...
sequenceDiagram
    ...
    "HW"->>"DRV": "事件2: IRQ"
    "DRV"->>"SIGQ": "kill_fasync(SIGIO)"
    "SIGQ"->>"SIGQ": "pending(SIGIO)仍为1<br/>不追加第二个副本"
    ...
    "TASK"->>"LOOP": "handler/signalfd 处理<br/>一次 SIGIO"
    "LOOP"->>"DRV": "read()/recv() 循环读出<br/>硬件事件队列中的多条记录"
```
...
### 10.3.6\_示例代码\_从伪代码角度理解\_kill\_fasync()\_的工作流程
...
### 10.3.7\_调试与验证\_如何从\_信号队列视角\_检查\_kill\_fasync()\_的并发行为
...
### 10.3.8\_小结\_把\_kill\_fasync()\_看成\_对信号队列的一次写操作\_而不是\_发出\_N\_个信号
...
## 10.4\_fasync\_与\_waitqueue\_/\_poll\_的竞态\_谁先感知\_如何保持顺序语义

### 10.4.1\_引入\_同一事件被\_多个通知机制\_消费时的顺序问题
...
### 10.4.2\_数据结构视角\_三个通知相关元素的\_单一事实来源
...
#### (2)\_.read()\_/.poll()\_与\_ISR\_的竞态\_如何用状态机避免\_睡死/重复
...
#### (3)\_fasync\_与\_waitqueue/poll\_的顺序\_不能依赖\_谁先唤醒
...
### 10.4.4\_用户/平台视角\_怎样在用户态\_容忍并利用\_这些竞态
...
### 10.4.5\_可视化\_在时间轴上看\_3\_条路径的竞态关系

#### (1)\_时间线\_一次\_事件\_+\_两次通知\_+\_多种消费\_的可能顺序
...
#### (2)\_状态机图\_基于\_event\_count\_的简单语义

```mermaid
stateDiagram-v2
    state "IDLE(无未处理事件)<br/>(event_count == 0)" as IDLE
    state "PENDING(有未处理事件)<br/>(event_count > 0)" as PENDING

    IDLE --> PENDING: "ISR: event_count++<br/>(首次从 0->1)<br/>唤醒 wait + kill_fasync"
    PENDING --> PENDING: "ISR: event_count++<br/>(累积更多事件)<br/>可选: 再次通知"
    PENDING --> IDLE: "read()/handle_device_events()<br/>消耗完队列<br/>(event_count 变为 0)"
```
...
### 10.4.6\_示例代码\_一个\_统一状态\_+\_限制通知频率\_的组合模板
...
### 10.4.7\_调试与验证\_针对\_多通知机制\_的竞态检测方法
...
### 10.4.8\_小结\_把\_fasync\_与\_waitqueue/poll\_视为\_多个观察者\_共享一个状态机
...
## 10.5\_fasync\_相关状态的释放与\_悬垂\_fasync\_struct\_问题

### 10.5.1\_引入\_为什么\_关闭/卸载\_阶段最容易把\_fasync\_搞崩
...
#### (2)\_设备/驱动释放路径\_remove()\_/\_module\_exit()\_的正确顺序
...
#### (3)\_devres\_vs\_非\_devres\_谁帮你释放\_谁帮不了你管并发
...
### 10.5.4\_用户/平台视角\_悬垂\_fasync\_struct\_的典型症状
...
### 10.5.5\_可视化\_安全释放顺序与悬垂场景示意

#### (1)\_安全释放顺序(非\_devres\_版本)
...
flowchart TD
    ...
    "REL" -->|"清理 fasync 节点<br/>(dev->async_queue 不再包含本 file)"| "RM"

    "RM" -->|"禁止新 open/fasync<br/>(cdev_del, device_destroy)"| "CDEV"
    "CDEV" -->|"逻辑停机<br/>不再允许新事件"| "DIS"
    "DIS" -->|"等待所有中断/工作队列<br/>执行完成"| "SYNCIRQ"
...
### 10.5.6\_示例代码\_带关闭序列的最小\_fasync\_驱动骨架(非\_devres\_+\_devres\_对照)
...
### 10.5.7\_调试与验证\_专门针对\_释放阶段\_BUG\_的检查方法
...
### 10.5.8\_小结\_把\_fasync\_关闭序列\_当成驱动设计的一等公民
...
## 10.6\_内存屏障\_原子操作在\_fasync\_实现中的角色

### 10.6.1\_引入\_锁解决\_互斥\_屏障解决\_顺序与可见性
...
### 10.6.2\_数据结构视角\_哪些字段需要考虑\_跨\_CPU\_可见性
...
#### (1)\_模式\_A\_锁\_+\_状态机(大部分驱动只用这一种即可)
...
#### (3)\_模式\_C\_混合模式(读无锁\_写加锁)与\_READ\_ONCE\_/\_WRITE\_ONCE
...
### 10.6.4\_用户/平台视角\_内存可见性问题在上层的\_表象
...
### 10.6.5\_可视化\_在时间轴上理解\_写-通知-读\_的内存顺序
...
### 10.6.6\_示例代码\_两种模式的对比实现

#### (1)\_示例一\_纯锁模式(推荐实践\_内存屏障由锁隐含)
...
### 10.6.7\_调试与验证\_如何发现\_内存可见性类\_问题
...
### 10.6.8\_小结\_把\_内存模型\_落回到\_fasync\_的三条设计原则
...
## 10.7\_实战中值得显式考虑的竞态场景清单

### 10.7.1\_引入\_从\_理解机制\_到\_可执行的核对表
...
### 10.7.2\_场景分类总览\_从\_什么时候会出问题\_开始
...
### 10.7.3\_场景\_1\_中断\_vs.fasync()\_/\_kill\_fasync()\_链表修改竞态
...
### 10.7.4\_场景\_2\_中断\_vs\_read\_/\_poll\_vs\_事件状态变量竞态
...
### 10.7.5\_场景\_3\_多线程/多进程对\_F\_SETOWN\_/\_F\_SETSIG\_/\_F\_SETFL\_的竞态
...
### 10.7.7\_场景\_5\_高频事件与\_信号风暴\_/限流策略
...
### 10.7.8\_场景\_6\_错误路径与异常恢复中的\_fasync\_状态
...
### 10.7.9\_统一\_Checklist\_表\_驱动并发自检表(fasync\_维度)
