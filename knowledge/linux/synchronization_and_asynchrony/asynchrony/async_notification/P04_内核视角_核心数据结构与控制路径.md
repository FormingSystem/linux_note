...
## 4.1\_file->f\_flags\_与\_FASYNC\_标志位
...
### 4.1.1\_引入\_为什么要从\_file->f\_flags\_看起
...
### 4.1.2\_数据结构视角\_struct\_file\_f\_flags\_与\_FASYNC

从内核结构体上看，关键点很少，但非常核心。

#### (1)\_struct\_file\_里的标志位
...
### 4.1.3\_开发者视角\_内核中谁来修改\_f\_flags\_和\_FASYNC
...
#### (2)\_fcntl(F\_SETFL)\_路径中的\_f\_flags\_修改
...
### 4.1.4\_用户/平台视角\_如何通过用户态控制\_FASYNC
...
### 4.1.5\_可视化\_从\_O\_ASYNC\_到\_file->f\_flags.FASYNC\_的路径
...
### 4.1.6\_示例代码\_用户态\_+\_内核态如何围绕\_FASYNC\_协作

这一小节给出一组“能跑通”的最小示例片段，用于把 `file->f_flags` 与 `FASYNC` 的角色串起来。

#### (1)\_用户态\_设置\_FASYNC\_并注册\_SIGIO\_处理函数
...
### 4.1.7\_调试与验证\_如何确认\_FASYNC\_确实被设置
...
### 4.1.8\_小结\_file->f\_flags/FASYNC\_在\_fasync\_机制中的角色
...
## 4.2\_struct\_fasync\_struct\_数据结构与链表组织
...
### 4.2.1\_为什么需要单独的\_struct\_fasync\_struct
...
### 4.2.2\_struct\_fasync\_struct\_字段拆解
...
### 4.2.4\_开发者视角\_在驱动中如何持有\_struct\_fasync\_struct\_*
...
### 4.2.6\_小结\_struct\_fasync\_struct\_的角色
...
## 4.3\_struct\_file\_operations\_中的.fasync\_回调
...
### 4.3.1\_.fasync\_在\_file\_operations\_里的位置与作用
...
### 4.3.2\_调用路径概览\_谁在什么时机调用.fasync
...
#### (1)\_使用全局\_struct\_fasync\_struct\_*\_管理多个设备实例
...
#### (2)\_在.release()\_中忘记清理\_fasync\_状态
...
#### (3)\_在.fasync\_中自行改\_file->f\_flags
...
#### (4)\_在中断上下文直接调用.fasync\_/\_fasync\_helper()
...
## 4.4\_fasync\_helper()\_的工作流程与使用约定
...
## 4.5\_kill\_fasync()\_与信号子系统的衔接逻辑
...
### 4.5.2\_内部逻辑\_遍历链表并调用\_send\_sigio()
...
### 4.5.6\_小结\_kill\_fasync()\_的位置与约束
...
## 4.6\_进程/进程组所有权\_F\_SETOWN\_F\_SETSIG\_与任务结构
...
### 4.6.1\_引入\_为什么\_fasync\_还需要\_owner\_这一层
...
### 4.6.2\_数据结构视角\_struct\_fown\_struct\_的字段
...
### 4.6.3\_内核开发者视角\_f\_setown()\_/\_f\_setown()\_调用链
...
### 4.6.4\_用户态视角\_F\_SETOWN\_/\_F\_SETSIG\_的行为规则
...
### 4.6.5\_可视化\_从\_F\_SETOWN\_到\_send\_sigio()\_的路径
...
### 4.6.7\_调试与验证\_owner\_相关问题的排查思路
...
### 4.6.8\_小结\_fasync\_与\_owner\_的关系
...
### 4.7.1\_顶层\_fcntl()\_与\_F\_SETFL/F\_SETOWN/F\_SETSIG
...
### 4.7.2\_中层\_fasync\_helper()\_与\_struct\_fasync\_struct\_链表
...
### 4.7.3\_底层\_send\_sigio()\_与任务/进程组
...
上一篇：[P03 fasync 机制的历史与设计动机](P03_fasync_机制的历史与设计动机.md)。
下一篇：[P05 VFS 与文件系统中的异步通知支持](P05_VFS_与文件系统中的异步通知支持.md)。
