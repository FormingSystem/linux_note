...
## 8.1\_串口\_/\_TTY\_等流式设备中的异步通知习惯用法
...
### 8.1.2\_数据结构视角\_从单一\_ev\_到\_环形缓冲区\_+\_统计字段
...
### 8.1.3\_开发者视角\_典型的流式设备\_fasync\_模式
...
### 8.1.4\_用户视角\_串口场景下\_SIGIO\_的典型用法
...
### 8.1.5\_设计模式\_把\_缓冲区从空到非空\_视为\_事件
...
### 8.1.6\_与\_GPIO\_按键场景的对比\_何时用\_单槽事件\_何时用\_流式缓冲
...
### 8.1.7\_实战中的习惯\_fasync\_往往是\_兼容接口\_epoll\_才是主力
...
### 8.1.8\_小结\_流式设备中的\_fasync\_使用原则
...
## 8.2\_高速采集设备中使用\_fasync\_的限制与折中方案

### 8.2.1\_引入\_从\_几次按键/秒\_到\_上万次采样/秒
...
### 8.2.2\_数据结构视角\_从\_事件\_为单位到\_块/帧\_为单位
...
### 8.2.3\_开发者视角\_fasync\_的限制与典型折中策略
...
### 8.2.4\_用户/平台视角\_高速场景下用户真正在意的是什么
...
### 8.2.6\_示例代码\_阈值型\_fasync\_通知\_的核心逻辑
...
#### (2)\_在\_DMA\_完成中断里入队\_block\_并触发阈值型通知
...
### 8.2.7\_调试与验证\_如何确认\_数据流正常\_通知频率合理
...
### 8.2.8\_小结\_高速采集场景中的\_fasync\_角色定位
...
## 8.3\_大量事件场景下的\_信号风暴\_问题

### 8.3.1\_引入\_什么是\_信号风暴
...
### 8.3.3\_开发者视角\_导致信号风暴的典型模式与\_稀疏化\_策略
...
### 8.3.4\_用户\_/\_平台视角\_如何从外部识别与缓解信号风暴
...
### 8.3.5\_可视化\_信号风暴\_vs\_稀疏化的对比

#### (1)\_错误模式\_每事件都\_kill\_fasync()\_的信号风暴
...
#### (2)\_改进模式\_基于\_0\_to\_1\_+\_时间间隔\_的稀疏化通知
...
### 8.3.6\_示例代码\_通用的\_信号限频\_+\_状态变化\_辅助函数
...
## 8.4\_与缓冲区管理\_丢帧策略\_SYN\_DROPPED\_类语义的对应关系

### 8.4.1\_引入\_通知机制必须和\_丢数据语义\_一起设计
...
### 8.4.2\_数据结构视角\_缓冲区\_+\_丢帧标志\_+\_语义事件
...
### 8.4.3\_开发者视角\_缓冲区策略\_+\_丢帧语义要一起设计
...
### 8.4.4\_用户\_/\_平台视角\_SYN\_DROPPED\_类语义如何使用
...
### 8.4.5\_可视化\_事件缓冲区\_+\_丢帧\_+\_DROP\_语义的流程图
...
flowchart LR
    ...
    "ISR"["ISR/下半部: 生产事件"]
    "QUEUE"["QUEUE: 环形缓冲区"]
    "POLICY"["POLICY: 丢弃策略<br/>覆盖旧/丢新/阻塞"]
    "DROP"["DROP: 插入 DEMO_EVENT_TYPE_DROP"]
    "FAS"["FAS: fasync + waitqueue 通知"]
    "APP"["APP: 用户态<br/>read()/poll()/SIGIO"]
    ...
    "DROP" --> "QUEUE"
    "FAS" --> "APP"
    "APP" -->|"read() 读取事件,<br/>遇到 DROP 触发重同步"| "APP"
```
...
### 8.4.6\_示例代码\_完整的\_带\_DROP\_语义的事件读取\_片段
...
### 8.4.7\_调试与验证\_如何确认丢帧语义与缓冲区策略匹配
...
## 8.5\_与\_input\_子系统通知机制\_netlink\_通知的对比

### 8.5.1\_引入\_三种\_通知通路\_的定位
...
### 8.5.4\_用户\_/\_平台视角\_API\_与可观测性对比
...
flowchart LR
    ...
    "APP_SIG"["APP_SIG: 用户进程<br/>signal/signalfd/read()"]
    "APP_INPUT"["APP_INPUT: 用户进程<br/>read(/dev/input/eventX)"]
    "APP_NL"["APP_NL: 用户进程<br/>netlink socket"]
...
### 8.5.6\_示例代码\_fasync\_vs\_input\_vs\_netlink\_的最小片段对比
...
#### (3)\_netlink\_通知骨架(generic\_netlink\_简化样式)
...
### 8.5.8\_小结\_fasync\_input\_netlink\_在工程中的角色划分
...
## 8.6\_适合\_/\_不适合使用\_fasync\_的高级场景分类

### 8.6.1\_引入\_从\_能用\_到\_应该用\_的差别
...
### 8.6.2\_分类维度\_速率\_语义\_订阅者\_历史包袱
...
### 8.6.3\_典型\_适合使用\_fasync\_的场景

#### (1)\_场景\_A\_低频离散事件\_+\_单订阅者\_+\_传统\_char\_驱动
...
#### (2)\_场景\_B\_中等频率的\_告警/状态类事件\_+\_少量订阅者
...
### 8.6.4\_明确\_不适合使用\_fasync\_的场景

#### (1)\_场景\_C\_高频流式数据(高速串口\_高速\_ADC\_逻辑分析器)
...
### 8.6.5\_灰色地带\_可以用\_但\_更推荐其它机制\_的中间场景

#### (1)\_场景\_E\_中等频率流式设备\_+\_单订阅者
...
#### (2)\_场景\_F\_需要同时向\_GUI/CLI\_多个进程提供\_轻量事件通知
...
### 8.6.8\_小结\_在高级场景中给\_fasync\_划清使用边界
