---
id: knowledge.linux.io_model.async_notification.异步通知简介.p13_工程实践案例与编码核对表
title: "工程实践案例与编码核对表"
kind: mechanism
status: evolving
domains:
...
## 13.1\_案例一\_GPIO\_中断\_+\_fasync\_+\_用户态\_SIGIO

### 13.1.1\_引入\_从\_按键中断\_到\_用户态\_SIGIO\_回调
...
### 13.1.2\_数据结构视角\_struct\_demo\_async\_key\_dev\_如何承载完整状态
...
### 13.1.3\_开发者视角\_驱动实现的关键步骤(devm\_版为主)

以 devm 版本为主线，从驱动作者角度，把实现拆成有明确顺序的步骤。

#### (1)\_步骤\_1\_解析设备树\_获取\_GPIO\_/\_中断号\_/\_去抖参数
...
#### (4)\_步骤\_4\_实现.read\_/.poll\_/.fasync
...
#### (5)\_步骤\_5.open\_/.release\_中维护私有数据与\_fasync\_状态
...
#### (6)\_步骤\_6\_devm\_/\_非\_devm\_版本的差异
...
### 13.1.4\_用户/平台视角\_如何从\_SIGIO\_看到真实按键事件
...
### 13.1.5\_可视化\_从中断到\_SIGIO\_的路径图
...
flowchart LR
    ...
    "DEV"["demo_async_key_dev<br/>(wait/event_count/async_queue)"]
    "GPIO"["GPIO 控制器: gpio1"]
...
### 13.1.6\_示例代码\_devm\_版完整驱动\_+\_用户态\_SIGIO\_Demo
...
#### (1)\_内核驱动\_demo\_async\_key.c(devm\_版)
...
#### (2)\_用户态\_SIGIO\_示例\_demo\_sigio\_key.c
...
### 13.1.7\_调试与验证\_从\_设备不收信号\_到\_完整闭环\_的排查步骤
...
### 13.1.8\_小结\_一个完整的\_GPIO\_中断\_to\_fasync\_to\_SIGIO\_标准样例
...
## 13.2\_案例二\_多路设备\_+\_signalfd\_+\_epoll\_统一事件循环

### 13.2.1\_引入\_从\_单个\_SIGIO\_到\_多设备\_+\_socket\_的统一事件循环
...
### 13.2.2\_数据结构视角\_从\_每个\_fd\_一个\_SIGIO\_到\_统一的\_signalfd\_事件源
...
### 13.2.3\_开发者视角\_驱动侧几乎不用改\_重点在用户态事件循环设计
...
### 13.2.4\_用户/平台视角\_signalfd\_+\_epoll\_的统一处理流程

从用户/平台视角，完整流程可以概括为以下几步。

#### (1)\_步骤\_1\_创建\_signalfd\_并屏蔽普通信号
...
#### (2)\_步骤\_2\_打开各个\_fasync\_设备\_并配置\_F\_SETOWN\_/\_O\_ASYNC
...
#### (3)\_步骤\_3\_把\_signalfd\_与所有\_fd\_加入\_epoll
...
### 13.2.5\_可视化\_多设备\_+\_signalfd\_+\_epoll\_的整体拓扑
...
flowchart LR
    subgraph "内核: 驱动与信号"
        "DEV0"["驱动0: /dev/demo_async_key0<br/>(.read/.poll/.fasync)"]
        "DEV1"["驱动1: /dev/demo_async_key1<br/>(.read/.poll/.fasync)"]
        "SOCK"["内核socket<br/>(网络栈)"]
        "SIGCORE"["内核信号子系统<br/>(SIGIO队列)"]
    ...
    subgraph "用户态: FD 与事件循环"
        ...
        "EPOLL"["epoll 实例"]
        "LOOP"["主事件循环<br/>(epoll_wait + dispatch)"]
    end
...
### 13.2.6\_示例代码\_多\_fasync\_设备\_+\_signalfd\_+\_epoll\_的用户态\_demo
...
### 13.2.7\_调试与验证\_如何确认\_signalfd\_与\_epoll\_行为正确
...
### 13.2.8\_小结\_signalfd\_是\_fasync\_to\_统一事件循环\_的关键适配器
...
## 13.3\_案例三\_从轮询版驱动改造为\_fasync\_版驱动的全过程

### 13.3.1\_引入\_从\_用户态傻等\_到\_驱动主动通知\_的演进路径
...
### 13.3.2\_数据结构视角\_从\_裸全局变量\_到\_统一事件状态机
...
#### (2)\_阶段\_1\_加入\_waitqueue\_+.poll(但暂不引入\_fasync)
...
#### (3)\_阶段\_2\_在.poll\_基础上引入\_fasync\_+\_SIGIO
...
### 13.3.4\_用户/平台视角\_逐步迁移用户态程序的行为

改造驱动只是第一步，更关键的是 **用户态如何逐步迁移**，避免一次性大改出 bug。

一个合理的迁移顺序是：

#### (1)\_阶段\_A\_驱动已支持.poll\_用户态先从\_死轮询\_切换到\_poll
...
#### (2)\_阶段\_B\_驱动加入\_fasync\_后\_用户态增加\_SIGIO\_支持
...
### 13.3.5\_可视化\_轮询版\_to.poll\_版\_to\_fasync\_版的演进图

用一个简单流程图表示三个阶段的关系。

```mermaid
flowchart TD
    "STAGE0"["阶段0: 轮询版驱动<br/>- 全局变量 counter<br/>- 仅实现 .read<br/>- 用户态 sleep+read 轮询"]
    "STAGE1"["阶段1: 加入 .poll + waitqueue<br/>- 引入 demo_counter_dev<br/>- counter/event_count<br/>- .read 阻塞等待事件<br/>- .poll 支持 select/poll/epoll"]
    "STAGE2"["阶段2: 加入 fasync + SIGIO<br/>- 在 dev 中增加 async_queue<br/>- .fasync + .release<br/>- 中断/定时器中 kill_fasync<br/>- 用户态可用 SIGIO/signalfd"]
...
### 13.3.6\_示例代码\_最小\_轮询版\_to\_poll\_版\_to\_fasync\_版\_对比模板

这里给出一份压缩版代码，展示三个阶段的差异点。为了避免冗长，只保留核心逻辑，不再重复字符设备注册/平台匹配等框架部分，这些可直接复用 13.1 的做法。

#### (1)\_阶段\_0\_核心差异(仅.read\_全局计数)
...
#### (2)\_阶段\_1\_核心差异(.poll\_+\_waitqueue)
...
#### (3)\_阶段\_2\_核心差异(在阶段\_1\_基础上加\_fasync)
...
## 13.4\_devm\_与非\_devm\_版本的差异与迁移注意点

### 13.4.1\_引入\_同一个\_fasync\_驱动\_为何要同时准备\_devm\_/\_非\_devm\_两套写法
...
### 13.4.2\_数据结构视角\_devm\_管的是\_device\_资源\_不是\_file/fasync\_资源
...
### 13.4.3\_开发者视角\_同一个\_fasync\_驱动的\_devm\_/\_非\_devm\_差异点
...
#### (1)\_设备私有结构\_devm\_kzalloc\_vs\_kzalloc
...
#### (2)\_GPIO\_/\_IRQ\_等资源\_devm\_gpiod\_get\_/\_devm\_request\_irq\_vs\_手动释放
...
#### (3)\_字符设备\_/\_class\_/\_device\_节点\_devm\_不会帮你管
...
#### (4)\_fasync\_状态\_devm\_完全不介入\_靠.release\_保证清理
...
### 13.4.4\_用户/平台视角\_devm\_与非\_devm\_对用户态行为的影响(几乎为\_0)
...
### 13.4.5\_可视化\_devm\_与非\_devm\_在生命周期上的差异
...
### 13.4.6\_示例代码\_同一驱动的\_devm\_/\_非\_devm\_关键对比
...
### 13.4.7\_调试与验证\_从非\_devm\_迁移到\_devm\_时要重点检查什么
...
### 13.4.8\_小结\_在\_fasync\_驱动里使用\_devm\_的原则与迁移建议
...
## 13.5\_驱动侧开发核对表(必写项/可选项/禁用项)

### 13.5.1\_引入\_把所有\_应该做\_变成可以逐条勾选的清单
...
### 13.5.2\_数据结构视角\_事件状态机\_+\_fasync\_链路的最小组成
...
### 13.5.3\_开发者视角\_驱动实现过程中的\_必写/可选/禁用\_条目

这一节按主题拆分 checklist，你可以在写驱动时逐块对照。

#### (1)\_file\_operations\_与基本框架
...
### 13.5.4\_用户/平台视角\_驱动\_checklist\_如何映射到\_可观测行为
...
flowchart TD
    "ROOT"["驱动侧开发核对表"]
    "FOps"["file_operations 层<br/>.open/.release/.read/.poll/.fasync"]
    "EVT"["事件状态机层<br/>(wait/event_count/async_queue)"]
    "IRQ"["中断/下半部层<br/>事件更新 + kill_fasync"]
    "RES"["资源管理层<br/>(devm vs 非devm)"]
    "USR"["用户可观测行为<br/>(SIGIO/poll/read 语义)"]
...
### 13.5.6\_示例代码\_内联注释版\_最小合格\_fasync\_驱动骨架
...
### 13.5.7\_调试与验证\_如何用\_checklist\_做代码评审与回归测试
...
### 13.5.8\_小结\_把\_经验\_变成\_可操作的核对表
...
## 13.6\_用户态应用核对表(配置步骤/信号处理/退出流程)

### 13.6.1\_引入\_驱动已经\_会叫人\_应用该怎么\_听得懂\_收得稳\_退得干净
...
### 13.6.3\_开发者视角\_配置步骤与调用顺序的必选项\_/\_推荐项\_/\_禁用项

这一节按两种典型场景分别给出 checklist。

#### (1)\_场景一\_传统\_SIGIO\_+\_signal\_handler\_模式
...
#### (2)\_场景二\_signalfd\_+\_epoll\_统一事件循环模式
...
### 13.6.4\_用户/平台视角\_多线程\_多进程\_退出路径的策略核对
...
flowchart LR
    subgraph "内核"
        "DRV"["驱动: demo_async_xxx<br/>(.read/.poll/.fasync)"]
        "SIGCORE"["信号子系统<br/>(SIGIO队列)"]
    end
...
### 13.6.6\_示例代码\_两个\_最小合格\_用户态\_demo
...
#### (1)\_示例一\_SIGIO\_+\_handler\_最小模板
...
#### (2)\_示例二\_signalfd\_+\_epoll\_最小模板
...
### 13.6.7\_调试与验证\_如何用\_checklist\_排查用户态典型问题
...
### 13.6.8\_小结\_把\_能收信号\_变成\_能稳定工作\_能优雅退出\_的规范
