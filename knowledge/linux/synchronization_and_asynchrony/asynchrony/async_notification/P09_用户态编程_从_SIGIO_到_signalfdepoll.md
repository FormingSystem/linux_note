...
## 9.1\_用户态编程\_从\_SIGIO\_到\_signalfd/epoll
...
### 9.1.2\_fcntl(F\_SETOWN)\_/\_F\_SETSIG\_/\_F\_SETFL(FASYNC)\_的调用顺序

> 本小节按照你要求的固定结构：引入 → 数据结构视角 → 开发者视角 → 用户/平台视角 → 可视化 → 示例代码 → 调试与验证 → 小结。

#### (1)\_引入\_这三个调用\_是什么\_干什么\_为什么要讲顺序
...
#### (2)\_数据结构视角\_owner\_/\_signum\_/\_f\_flags\_是怎么落到内核里的
...
#### (4)\_用户/平台视角\_在\_i.MX6ULL\_demo\_异步驱动上的配置建议
...
#### (5)\_可视化\_从\_open\_到\_FASYNC\_生效的调用时序
...
#### (7)\_调试与验证\_如何确认顺序正确且\_FASYNC\_生效
...
### 9.1.3\_使用\_SIGIO\_的经典写法(signal\_handler\_+\_全局状态)

#### (1)\_引入\_最常见\_也是最\_教材式\_的用法
...
#### (2)\_数据结构视角\_信号处理函数和全局标志在进程内部的位置
...
#### (4)\_用户/平台视角\_在嵌入式单进程/守护进程中的使用场景
...
sequenceDiagram
    ...
    "K_SIG"->>"SIGH": "投递 SIGIO 到 U_APP<br/>调用 demo_sigio_handler()"
...
### 9.1.4\_使用\_sigaction\_的可靠信号处理模式

#### (1)\_引入\_为什么要从\_signal()\_升级到\_sigaction()
...
#### (2)\_数据结构视角\_struct\_sigaction\_与\_siginfo\_t

先从数据结构看 `sigaction()` 到底在改什么。

##### 1)\_struct\_sigaction\_的关键字段
...
##### 2)\_siginfo\_t\_中与异步\_I/O\_相关的字段
...
#### (3)\_开发者视角\_基于\_sigaction\_的可靠\_SIGIO\_模式
...
#### (4)\_用户/平台视角\_多\_fd\_/\_多线程时的注意事项
...
#### (5)\_可视化\_sigaction\_模式下的事件路径与状态机
...
#### (6)\_示例代码\_基于\_sigaction\_的改进版\_SIGIO\_处理
...
#### (7)\_调试与验证\_如何确认\_sigaction\_配置正确
...
#### (8)\_小结\_sigaction\_在\_fasync\_用户态链路中的位置
...
### 9.1.5\_使用\_signalfd\_接收\_SIGIO\_并接入\_epoll\_事件循环

#### (1)\_引入\_为什么要把信号\_转化\_为\_fd\_事件
...
#### (2)\_数据结构视角\_signalfd\_与\_signalfd\_siginfo
...
##### 2)\_struct\_signalfd\_siginfo\_的字段
...
#### (3)\_开发者视角\_从\_SIGIO\_到\_signalfd\_+\_epoll\_的步骤拆解
...
#### (5)\_可视化\_fasync\_+\_SIGIO\_+\_signalfd\_+\_epoll\_的整体链路
...
flowchart LR
    "HW"["HW: 按键/外部信号"]
    "DRV"["DRV: demo_async_key 驱动<br/>(fasync + kill_fasync)"]
    "K_SIG"["K_SIG: 内核信号子系统"]
    "SFD"["SFD: signalfd fd"]
    "EPOLL"["EPOLL: 主事件循环"]
    "APP"["APP: 用户态业务逻辑<br/>(read / 状态机 / 日志)"]
    ...
    "APP" -->|"根据 siginfo 解析事件<br/>对 /dev/demo_async_key 执行 read()"| "DRV"
```
...
#### (6)\_示例代码\_signalfd\_+\_epoll\_版\_demo
...
#### (7)\_调试与验证\_观察\_signalfd\_和\_epoll\_的行为
...
#### (8)\_小结\_signalfd\_+\_epoll\_在本章中的定位
...
### 9.1.6\_多线程/多进程下的所有权\_fd\_共享和信号路由问题

#### (1)\_引入\_单进程没问题\_一上多线程/多进程就乱了
...
#### (2)\_数据结构视角\_谁是\_owner\_谁持有\_fd\_谁收到信号
...
##### 2)\_struct\_fown\_struct\_owner\_是谁
...
#### (3)\_开发者视角\_在多线程/多进程中设计可靠的所有权与路由方案

下面从工程设计角度给出几条“硬规则”，你在写异步通知相关的用户态代码时可以直接采用。

##### 1)\_规则一\_多线程时\_用\_专门信号线程\_简化路由
...
##### 2)\_规则二\_如确实需要\_信号直接送到某个线程\_才用\_F\_SETOWN\_EX(F\_OWNER\_TID)
...
##### 3)\_规则三\_多进程时\_不要让多个进程同时对同一\_struct\_file\_开启\_FASYNC
...
##### 4)\_规则四\_fork\_+\_exec\_时重设\_F\_SETOWN\_/\_FASYNC
...
flowchart TB
    ...
    "T_GPIO"["T_GPIO: gpio_thread<br/>(持有 signalfd + epoll)"]
    "T_IO"["T_IO: io_thread"]
    "T_WORK"["T_WORK: worker_thread"]

    "DEVFD"["DEVFD: /dev/demo_async_key<br/>(struct file + fasync)"]
    "SFD"["SFD: signalfd_fd"]
    ...
    "T_GPIO" -->|"封装为高层事件<br/>(按键 A/B/...)"| "T_MAIN"
    "T_GPIO" -->|"推入队列"| "T_WORK"
...
flowchart LR
    "DRV"["DRV: demo_async_key 驱动<br/>(fasync + kill_fasync)"]
    "AGENT"["AGENT: demo_daemon<br/>(事件代理进程)"]
    "APP1"["APP1: 上层应用 1"]
    "APP2"["APP2: 上层应用 2"]

    "DRV" -->|"SIGIO → signalfd"| "AGENT"
    "AGENT" -->|"本地 IPC: UNIX 域 socket<br/>广播按键事件"| "APP1"
    "AGENT" -->|"本地 IPC: UNIX 域 socket<br/>广播按键事件"| "APP2"
```
...
#### (6)\_示例代码\_多线程下\_专门信号线程\_+\_signalfd\_的骨架
...
#### (8)\_小结\_多线程/多进程下的\_fasync\_使用原则
...
### 9.1.7\_用户态典型误用\_顺序错误\_信号未解掩\_处理函数过重等

#### (1)\_引入\_机制本身不复杂\_踩坑多在\_用法细节
...
#### (2)\_机制层视角\_错误分类按\_层级\_而不是按\_API
...
#### (3)\_开发者视角\_几种高频\_坑\_的具体形式与后果

下面从开发者角度，直接按问题形式列出几个最常见、最影响调试效率的错误模式。

##### 1)\_典型误用一\_FASYNC\_配置顺序错误或被覆盖
...
##### 2)\_典型误用二\_信号未解掩/误掩\_或与\_signalfd\_并存
...
##### 4)\_典型误用四\_与\_poll/waitqueue\_组合使用时未统一\_事件语义
...
#### (4)\_用户/平台视角\_这些误用在嵌入式场景中的\_表面症状
...
### 9.1.8\_现代风格\_的用户态事件循环(epoll\_+\_signalfd\_+\_fasync)的组合示例

#### (1)\_引入\_把前面所有内容收束成一个\_可直接复用\_的模板
...
#### (5)\_可视化\_现代事件循环的结构与时序

先用结构图描述整体组件关系：

```mermaid
flowchart LR
    "DEV"["DEV: /dev/demo_async_key<br/>(fasync + kill_fasync)"]
    "K_SIG"["K_SIG: 内核信号子系统"]
    "SFD"["SFD: signalfd_fd<br/>(接收 SIGIO)"]
    "TFD"["TFD: timerfd_fd<br/>(周期任务)"]
    "EP"["EP: epoll_fd<br/>(统一事件循环)"]
    "APP"["APP: 用户态业务逻辑<br/>(状态机/处理函数)"]
    ...
    "APP" -->|"根据事件类型<br/>read(dev_fd)/read(tfd_fd)"| "DEV"
```
