...
这一步改变了验证目标：不再要求测试命中完整故障交错，而是要求每条会参与故障的简单组件链至少执行一次。随后由验证器把它们组合起来，搜索尚未真实发生、但已经被这些组件链允许的复杂交错。

可是，“保存组件链”还不是一个可实现方案。第一条路径释放以后，锁对象的瞬时状态会消失；第二条路径出现时，验证器既要知道 **current 此刻仍持有什么**，又要记得 **过去观察过什么顺序**。这就把问题自然推进到状态应该存在哪里。

## 1.3\_为了组合证据还需要当前账本与历史账本
...
现在，两条 mutex 路径已经逼出了两本不能互相替代的账：一本保存本次尚未结束的等待前缀，一本保存跨任务、跨时间仍有效的顺序。若内核里只有进程上下文中的 mutex，这个朴素模型似乎已经够用；但中断可以在同一 CPU 上改变“谁还能继续运行”，因此下一步必须检验这两本账是否还需要记录执行上下文。

## 1.4\_IRQ抢占把执行上下文加入同一条证明链
...
这个 IRQ 场景不是突然插入的第二个知识点。它是在检验上一节得到的“当前账本＋历史账本”是否足以表达真实内核：若账本只记锁名和先后顺序，却不记取得时的 IRQ 上下文与本地 IRQ 状态，就仍然无法推出同 CPU 的不可恢复抢占关系。因此，Lockdep 需要积累的不只是图边，还包括这些边在什么执行条件下成立。

## 1.5\_两类现场在Lockdep检查链中汇合
...
进程 ABBA 提供“跨时间组合锁序”的压力，IRQ 场景提供“组合时还要携带上下文”的压力。二者在同一条检查链中汇合：同步原语上报本次身份、取得类型和上下文，current 账本给出本次前缀，全局历史给出过去的路径，规则引擎判断它们能否组成不可前进的等待闭包。
...
本章不是依次回答几个互不相干的疑问，而是完成了一次连续收缩：从两条局部正确路径得到全局锁序矛盾；从难以稳定命中的真实卡死转向积累组件证据；再由证据的时间尺度推出 current 账本与全局历史，最后用 IRQ 抢占证明这些记录还必须携带执行上下文。
