...
先限定为普通独占锁，准备提交 `A → B` 时，这条边还不能污染历史。要判断它是否闭环，只需问既有图中能否从 B 到达 A。后文加入读写类型后，还必须检查路径是否真的能逐段传递阻塞，不能仅按几何形状判错：
...
本节沿用 queue_lock 的独占自旋取得；共享读场景还须结合下一节之后的阻塞类型规则，不能只凭 safe/unsafe 两个标签省掉读写语义。
...
若中间还有 K，不要求某一个中断处理函数真的依次调用 H、K、U。可以由多条已观察组件链组合：本 CPU 进程持 U 时被中断，IRQ 持 H 等 K；另一个执行流持 K 等 U。进程被中断无法释放 U，远端无法取得 U 因而不释放 K，IRQ 无法取得 K 因而不退出。这才把间接路径还原成端到端等待闭包。告警所需证据可以分时收集，但推导出的故障交错必须能逐段解释阻塞。
...
这里的“读取得”是按阻塞契约分类，不是对 Linux 名为 read_lock() 的每个实现做同一断言。使用具体原语前，必须核对其读侧递归和写者等待规则；尤其不能把允许递归读的原语与 rwsem 的非递归读要求混为一谈。Linux 6.12.20 的版本依据是固定提交中的 Documentation/locking/lockdep-design.rst 的递归读与强路径说明；具体接口选择仍回到对应锁专题。

反过来看一条“有圆却没有这一死锁”的时间线：X、Y 都提供递归共享读契约，A 已读持有 X，再读取得 Y；B 已读持有 Y，再读取得 X。假定没有写持有者，A 的 Y 请求不会被 B 的读持有挡住，B 的 X 请求也不会被 A 的读持有挡住。图上可以画出 X→Y→X，但闭包在共享读接点断开，不能据此报告普通独占 ABBA。若把其中的持有换成独占写，必须重新逐接点判断，不能沿用“都是读锁名称所以安全”的结论。
...
用三次预测复核规则。历史有 B→C→A，本次请求 A→B，应从哪里开始搜，哪条边是尚未提交的候选？若 H→K→U 的每条边来自不同测试，为什么仍能形成上文的中断等待闭包？如果只有两个递归读请求形成图上的环，又缺少写持有者，阻塞链在哪里断开？

对照答案：从 B 沿历史搜到 A，候选仍是 A→B；多个测试留下的是可组合的组件证据，不要求测试现场已经卡死；最后一种在共享读持有者与递归读请求的接点不形成阻塞。三题分别检查搜索方向、跨时间组合和边的阻塞类型。没有告警不能替代这些前提，也不能证明从未执行的路径安全。
