...
`lockdep_cycle_demo.c` 使用 `DEFINE_MUTEX` 静态初始化两把 mutex；加载入口依次建立两条链，卸载入口没有额外资源需要回收。`MODULE_LICENSE` 声明模块许可证，`MODULE_DESCRIPTION` 提供模块描述，均不是检查事件。代码如下：
...
{
	/* 配置不满足时拒绝实验，不把静默退化误当作验证通过。 */
	if (!IS_ENABLED(CONFIG_PROVE_LOCKING))
		return -EOPNOTSUPP;

	pr_info("lockdep_textbook: S0 first -> second\n");
	...
	pr_info("lockdep_textbook: S1 second -> first\n");
	...
	pr_info("lockdep_textbook: S2 functional path completed\n");
	return 0;
}
...
这里 `-C` 让 make 进入运行内核的构建目录；`M="$PWD"` 把当前目录交给 Kbuild，作为外部模块的源码和输出目录；`modules` 请求构建模块目标。上面的 Makefile 用 `obj-m` 指定要形成的模块对象。构建成功只说明这份代码能交给该构建环境处理，加载是否允许、目标路径是否执行仍需下一步证据。
...
grep -E 'lockdep_textbook|possible circular locking dependency|existing dependency chain|locks held' \
...
本章日志以 `lockdep_textbook` 标识，配套实验使用 `lockdep_cycle_lab` 和 `config_lock/state_lock` 名称；两者分别对应相同的第一把/第二把锁，不要拿一个版本的日志筛选词去验证另一个版本。预期先出现 S0，再出现 S1，循环报告位于 S1 与 S2 之间，最后 S2 表明这次功能路径确实走到了末尾。以上是 **预期顺序**，不是本仓库已经采集的运行报告。配置拒绝只验证构建能力；运行前仍须确认 `debug_locks` 有效。

执行前先预测三个结果：为什么 S0 释放以后历史边仍在？为什么 S1 只有一个线程也能报环？为什么 S2 出现仍不能证明锁序正确？运行后分别用历史路径、当前取得栈和 S2 标记回答。若任一标记缺失，先查加载返回值与完整日志，不要立刻推断规则没有工作。
...
| 组件链1内 | 增加 `[first,second]`，随后撤销这两条记录 | 保存 `first → second` | 不应有循环报告 |
...
sequenceDiagram
    ...
    participant G as 全局锁类图
    participant L as 日志与实验者
    M->>C: S0 上报first再上报second
    C->>G: 验证并保存first到second
    M->>C: 撤销两条当前记录
    Note over G: 历史边继续保留
    M->>C: S1 持有second，候选取得first
    C->>G: 查询first到second的历史路径
    G-->>L: 发现闭环，停检并报告
    M->>M: 单线程完成空闲mutex取得与释放
    M-->>L: S2 功能路径结束标记
```
...
本例可以做一次可比较的修复：仅把第二段改成先取得 `first_lock`、再取得 `second_lock`，仍按相反顺序释放，同时把 S1 日志改为修复后的顺序。在干净启动中重新构建和加载，保留 S0/S1/S2 三个标记、配置及前后状态。预期不再产生本例的反向边，且检查器保持有效。若只删掉第二段，告警也可能消失，但它回答的是“未执行反向路径”，并没有测试改正后的第二条业务路径。比较实验必须保留要验证的操作。
