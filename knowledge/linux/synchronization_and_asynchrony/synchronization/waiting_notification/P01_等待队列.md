...
正常离开等待循环时需要恢复任务状态、清理等待项。标准宏还会处理信号分支，具体源码不一定每条退出路径都再调用一次finish_wait：prepare阶段的信号分支已执行相应摘链，宏可以直接返回。手写循环则必须明确每个出口的清理责任，不能套用“某分支没写finish_wait所以都能省略”。精确分支见[prepare实现](../../../../../research/source_reading/waiting_notification/source_explanations/P01_Linux_6.12_wait_c入队与唤醒源码实现.md#1.3_prepare_to_wait_event登记与信号分支)，阶段证明继续在[统一状态机](P03_条件等待的统一状态机.md)。
