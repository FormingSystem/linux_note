...
| Tasks RCU | 无显式经典读锁 | `synchronize_rcu_tasks()` / `call_rcu_tasks()` | 函数前导、trampoline 等任务执行轨迹 |
...
| 把某个对象或 block 带出读侧交给异步任务 | 在读侧内 `get_unless_zero()`，长期用户最终 put；支付原子计数和可能更长的对象生命期 |
