...
要解释的不是三个函数各自“有什么作用”，而是解释如下动作：

1. `synchronize_rcu()` 怎样把一个栈上等待者连接到 GP；
2. GP 怎样把 CPU 债务写进树；
3. CPU 怎样形成并上报 QS；
4. 完成又怎样回到这个等待者；

这里的 `GP` 是 Grace Period（宽限期）这一逻辑证明区间，`GP 请求` 是目标需求，`物理 GP` 是实际执行的一轮，`GP kthread` 则是长期推进许多物理 GP 的内核任务。四者的定义、状态所有权和完整周期统一见 [Tree RCU GP 请求与全局生命周期](P12_Tree_RCU_GP请求与全局生命周期.md#12.2_六个必须分开的专有名词)；Linux 6.12.20 的模块协作见 [GP 模块源码概念导读](../../../../../research/source_reading/rcu/navigation/P06_Linux_6.12_Tree_RCU_GP全局生命周期模块源码概念导读.md#6.2_先把四个对象摆到源码现场)，从启动创建、请求漏斗、主循环到 init/FQS/cleanup 的逐函数链见 [GP 全局生命周期源码实现](../../../../../research/source_reading/rcu/source_explanations/P05_Linux_6.12_Tree_RCU_GP全局生命周期源码实现.md#5.5.1_先从内核启动链定位early_initcall)。本章不再默认读者已经知道这些专有名词，也不把公共 GP 控制误归为非抢占配置私有实现。
