...
先给本类型一张“身份证”，避免只记住“SRCU 可以睡眠”这一条表象：

| 层次 | SRCU 在本专题中的答案 | 本章展开位置 |
| --- | --- | --- |
| 分类位置 | 独立的 SRCU 保护域；本章源码基线讨论 Tree SRCU 后端，不与普通 Tree RCU 共用 GP | 18.1～18.2 与 18.9 |
| 问题背景 | reader 必须跨 mutex、I/O 或迁移继续借用对象，普通 RCU 执行边界不再适用 | 18.1 |
| reader 与完成证据 | 指定 `srcu_struct` 域内、持有 lock 返回 index 的 reader；全 CPU 进入/退出累计计数配平 | 18.2～18.4 |
| 运行原理 | 两轮扫描与一次 index 翻转隔离旧 reader，S0～S5 交付同步等待或 callback | 18.4～18.6 |
| 初始化与实现 | `init_srcu_struct()` 建立域级、每 CPU、汇聚和工作状态；模块导读后接唯一函数实现 | 18.1、18.9 与本卷相邻源码章节 |
| 选择边界 | 适合显式私有域和可阻塞 reader；不替代普通对象 GP、Tasks 轨迹 GP 或长引用 | 18.7～18.8 |
