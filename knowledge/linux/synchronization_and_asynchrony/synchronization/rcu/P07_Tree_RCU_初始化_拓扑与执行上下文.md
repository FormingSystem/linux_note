...
[P04 提炼的七项实现抽象](P04_RCU_分类坐标与内核配置.md#4.1.4_先提炼跨类型实现都必须回答的七个问题)描述运行期必须存在的职责；本节 S0～S6 描述 Tree RCU 怎样把这些职责变成可寻址状态和可调度执行者。Linux 6.12.20 的逐函数源码则使用 I0～I13 记录更细的启动事务。三套编号不能互相替代，下面只建立交叉关系：

| 本节初始化阶段 | Linux 6.12.20 源码阶段 | 建立的实现抽象 | 第一次重要运行期消费 |
| --- | --- | --- | --- |
| S0 计算几何 | `rcu_init()` 的 I5 | 汇聚与全局结论所需的节点容量和层级 | [P05 S4](P05_Tree_RCU_公共骨架与完整周期.md#5.5.2_S3到S7_GP请求证明与根完成)开始一轮 GP 时遍历实际树形 |
| S1 初始化节点 | I6 的节点初始化部分 | GP 代际、局部债务汇聚与父子传播的共享地址 | P05 S4 建债、S6 逐层汇聚与 S7 发布完成 |
| S2 绑定每 CPU | I6 的 per-CPU 绑定部分 | reader 状态、局部证据与 callback 队列到叶节点的直接寻址关系 | P05 S5 记录证据、S6 清叶节点位以及 S8 推进 callback |
| S3 注册 core 执行入口 | I7；线程分支还要等待后继 initcall | 局部证据消费者和结果交付执行者 | P05 S5 的本地检查与 S8 的 callback 推进/执行 |
| S4 准备 boot CPU | I9 的 `prepare` 部分 | 当前 CPU 的本地状态、代际快照和回调容器 | 下一轮 P05 S4 建债以及本 CPU 后续 S5 证据记录 |
| S5 CPU online | I9 的 `starting/online` 部分；次级 CPU 以后复用 hotplug 协议 | 保护域参与集合及下一轮债务位 | 后续 GP 在 P05 S4 封闭本轮参与集合 |
| S6 工作线程就绪 | I10 先建立 workqueue；GP/core/NOCB 等线程由后继 initcall 创建 | GP 请求协调者、慢路径执行者和结果交付者 | P05 S3 接收需求、S4～S7 推进 GP、S8 交付结果 |

这张表也解释了为什么不能只读 `rcu_init()` 函数体：I5～I10 主要建立地址、入口和排队能力，实际 GP kthread、完整 workqueue worker 与次级 CPU 都在函数返回以后继续接力。完整源码交叉账本见 [`rcu_init()` 的 I 阶段到运行期职责映射](../../../../../research/source_reading/rcu/source_explanations/P12_Linux_6.12_Tree_RCU_rcu_init启动初始化源码实现.md#12.6.1_I阶段怎样回接公共实现抽象与运行周期)。
