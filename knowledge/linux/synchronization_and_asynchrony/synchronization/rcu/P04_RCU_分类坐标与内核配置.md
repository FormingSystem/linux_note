...
`CONFIG_SMP` 是 Kconfig 配置符号，表示生成的内核是否具备对称多处理器构建能力；它不是“此刻 online CPU 数量”的运行时变量。
...
代码中的 `mutex` 是可能使任务睡眠的互斥锁，**I/O（Input/Output，输入 / 输出）** 表示设备或数据传输等待；**tracing** 指内核跟踪设施，**BPF** 名称源自 Berkeley Packet Filter（伯克利包过滤器），现代 Linux 中也用于受验证程序在内核事件上的执行。局部整型变量 `idx` 是 SRCU 读侧进入函数返回的 index/cookie（索引令牌），退出路径必须原样交还。这里列出它们，只为说明某些 reader 可能阻塞或执行旧代码轨迹，不要求先掌握这些子系统。
...
### 4.1.4\_先提炼跨类型实现都必须回答的七个问题

前面的公共接口已经告诉调用者“要做什么”，但它没有规定内核必须用树、计数器、任务扫描还是单 CPU 状态兑现。进入类型分类以前，先把不同实现都必须回答的问题提炼出来；后面的普通 RCU、SRCU、Tasks 与 Tiny 章节都复用这组问题，读者才不会把某个源码字段误认成 RCU 的定义。

| 实现抽象 | 必须回答的问题 | 共同输入与输出 | 不同类型主要改变什么 |
| --- | --- | --- | --- |
| 保护域与正式入口 | 哪些执行可能取得旧对象或进入旧代码轨迹，什么动作封闭未来参与者 | 输入是发布入口或受保护执行边界；输出是本轮旧 reader 集合的时间边界 | 普通对象 reader、SRCU 私有域 reader、Tasks 代码轨迹 reader 的定义不同 |
| reader 状态 | reader 进入、退出、抢占、迁移或阻塞时，旧访问责任保存在何处 | 输入是读侧事件；输出是“仍可能属于旧集合”或“已经越过边界”的局部事实 | 可以依赖 CPU 执行约束、每任务登记、显式域计数或单 CPU 前提 |
| GP 请求与代际 | 更新者或 callback 怎样提出“至少等到某个边界”的需求，多次需求怎样绑定或合并 | 输入是同步等待、异步 callback 或轮询请求；输出是目标代际与待完成请求 | 请求对象、代际字段和协调执行者由实现家族决定 |
| 局部证据 | 哪个事件足以排除一个 CPU、任务或域内 reader，它由谁记录 | 输入是调度、user/idle、读侧退出、计数配平或主动探测；输出是可共享的局部完成证据 | QS、双 index 计数、任务 holdout 和 trace 状态不能互换 |
| 汇聚与全局结论 | 分散证据怎样变成“本轮旧集合已经排空”的结论 | 输入是局部证据；输出是 GP 完成或继续等待 | Tree 使用分层节点，SRCU 扫描域计数，Tasks 扫描任务/CPU，Tiny 在本地闭合 |
| 结果交付 | GP 完成以后，谁推进 callback、唤醒同步等待者或触发最终释放 | 输入是完成代际；输出是 callback 执行、completion 唤醒或对象销毁资格 | callback 队列、工作线程、softirq 和直接等待桥的组织不同 |
| 初始化与拆除 | 上述状态地址、执行者和参与集合何时建立，何时停止接收请求并排空 | 输入是构建选择、启动/域初始化和 CPU 生命周期；输出是可运行状态与唯一关闭边界 | 全局启动、每域初始化、静态初始化和配置退化路径不同 |

这七项不是七个顺序阶段。前六项描述一轮运行期证明需要哪些职责，最后一项描述这些职责怎样获得真实地址和执行上下文。运行期完整时间线仍以 [RCU 抽象机制](P02_RCU_抽象机制推演.md#2.9_完整抽象时序) 和 [普通 Tree RCU 的 S0～S9 周期](P05_Tree_RCU_公共骨架与完整周期.md#5.5_S0到S9的一次完整周期) 为准；启动初始化不能为了形式整齐而冒充一次 GP。

后续每种可以独立学习的类型或实现家族都按同一闭环展开：

```text
新增问题与旧方案缺口
    → 本类型重新定义的reader和证明边界
    → 七项实现抽象中哪些保持不变、哪些发生变化
    → 初始化怎样建立状态地址和执行者
    → API入口怎样走到证据、全局结论和结果交付
    → 实验或观察怎样验证，代价和选择边界是什么
```

源码初始化章节还必须提供一张交叉表，逐项回答“初始化步骤建立哪项抽象职责、写入什么状态、谁在运行期第一次消费、服务哪段完成证明”。这样初始化源码与原理使用同一套角色和状态，但保留各自不同的时间轴。
...
**NOCB（no-callbacks-on-this-CPU，从指定 CPU 卸载回调）** 是 callback 执行策略的名称；它不删除回调，也不改变 reader 或 GP 的保护域。
...
`rcu_segcblist` 是保存分段 callback 代际的结构体类型；`gp_tasks` 是 `rcu_node` 中指向本轮仍阻塞 GP 的被抢占任务区段的字段。二者都属于普通 Tree RCU 的内部状态，不是公共 API 类型。
...
`CONFIG_RCU_NOCB_CPU_DEFAULT_ALL` 是 Kconfig 配置符号，用于指定在没有显式 CPU 列表时是否默认把所有 CPU 纳入 NOCB 回调卸载范围。
