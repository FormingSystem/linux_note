...
P08 已经说明 GP 怎样请求、开始并建立等待债务，但 `gp_seq` 前进本身不能证明旧 reader 已经离场。本章只追踪 **CPU 在什么执行边界产生 QS、持续 EQS 怎样成为可读取状态、context tracking 怎样避免把 IRQ 或内核重入误判成静默区间**；这些局部证据怎样汇聚到根节点留给 P10。
