...
# 第10章\_Tree\_RCU\_统一状态与通知总图

上一章已经用六个反例推出可实现 RCU 必须具备的能力。本章在第 5～8 章两种具体设计之上做一次统一复盘，并把抽象需求逐一映射到 Linux 6.12.20 Tree RCU：任务状态进入 `task_struct`，每 CPU 进度进入 `rcu_data`，分层等待集合进入 `rcu_node`，全局 GP 生命周期进入 `rcu_state`，CPU 是否处于 EQS 则由 context tracking 的 watching 状态表达。
...
> **GP 专有名词入口：** 本章中的 GP 是 Grace Period 这一逻辑证明区间；GP 请求、实际执行的一轮物理 GP、长期存在的 GP kthread 与 callback 完成交付不是同一对象。若这些词尚未建立，请先读 [GP 的六个基本名词](P12_Tree_RCU_GP请求与全局生命周期.md#12.2_六个必须分开的专有名词)，再回到总图；Linux 6.12.20 的任务指针、等待队列、请求漏斗和主循环见 [GP 模块源码概念导读](../../../../../research/source_reading/rcu/navigation/P06_Linux_6.12_Tree_RCU_GP全局生命周期模块源码概念导读.md#6.2_先把四个对象摆到源码现场)，从内核启动创建到每轮 cleanup/再次休眠的源码链见 [GP 全局生命周期唯一实现](../../../../../research/source_reading/rcu/source_explanations/P05_Linux_6.12_Tree_RCU_GP全局生命周期源码实现.md#5.5.1_先从内核启动链定位early_initcall)。这里允许前向跳读，是为了避免用尚未定义的专有名词要求读者自行拼接。
...
## 10.1\_固定角色视角\_多个读者\_CPU\_与单个写者\_CPU
...
后文所有状态变化都对应下面这段代码，而不是脱离对象更新凭空讨论 GP：
...
static struct route __rcu *active_route;

static int route_metric(void)
{
	struct route *p;
	int metric = -ENOENT;

	rcu_read_lock();
	p = rcu_dereference(active_route); /* p是对象地址，不是对象副本。 */
	if (p)
		metric = READ_ONCE(p->metric);
	rcu_read_unlock();
	return metric;
}

static void replace_route(struct route *new)
{
	struct route *old;

	old = rcu_replace_pointer(active_route, new, true);
	synchronize_rcu();
	kfree(old);
}
```

示例暂时假设单写者；如果多个更新者可能同时调用 `replace_route()`，还要用 mutex 或其他更新协议保护“取旧值并发布新值”这一写写过程。

### 10.1.1\_读者\_CPU\_负责什么
...
### 10.1.3\_为什么还要单列\_GP\_kthread

从业务视角看，CPU3 是需要同步结果的写者；从实现视角看，CPU3 通常不会亲自逐个管理其他 CPU 的报告。`synchronize_rcu()` 把等待动作接入普通 Tree RCU 的 GP/回调基础设施，随后 GP kthread代表一个或多个请求执行：
...
### 10.2.1\_完整状态拓扑\_状态放在哪里以及沿哪条路流动
...
### 10.2.2\_S0\_到\_S9\_一次完整通信到底怎样发生
...
| S2 请求 GP | 默认同步路径把栈上 completion 回调交给 `call_rcu_hurry()`；回调加速路径逐层提交 `gp_seq_needed`，根节点再设置 `gp_flags` 并唤醒 GP kthread；CPU3 等待 completion | 请求者写栈上等待项和本 CPU 回调队列；RCU 请求路径写 `gp_seq_needed/gp_flags` | 回调推进路径、GP kthread | 目标 GP 被已有轮次覆盖或新 GP 被接受 |
...
| S9 完成并唤醒 | 根节点没有欠报子树且没有阻塞本轮的 `gp_tasks` | 根报告路径唤醒 GP kthread；清理路径推进完成序号，随后唤醒回调调用 `complete()`；可选批量路径直接完成等待项 | CPU3 的同步等待路径、回调推进路径 | CPU3 返回并可安全释放 A |
...
sequenceDiagram
    ...
    W->>RS: 排队唤醒回调并提交所需GP代际
    W->>W: 睡眠等待完成量
    RS->>RS: 请求链把目标写入gp_seq_needed<br/>根节点设置gp_flags并唤醒GP线程
    RS->>RS: GP线程推进gp_seq，进入GP INIT
    ...
    RS->>W: 成熟唤醒回调调用complete()<br/>或批量等待路径直接完成
...
Linux 6.12.20 把主要 watching 序列放在每 CPU `context_tracking.state` 中，并用 `CT_RCU_WATCHING` 位表达 RCU 是否正在观察该 CPU。`rcu_watching_snap_in_eqs()` 判断快照是否位于 EQS，`rcu_watching_snap_stopped_since()` 比较前后快照，判断 CPU 自某个观察点以来是否曾经过 EQS。这里读取的是内核维护的软件计数和状态，不是在读取某个“CPU 的 RCU 寄存器”。

### 10.4.3\_硬件事件为什么不能直接等同于\_QS/EQS
...
## 10.5\_状态存在哪里\_Tree\_RCU\_的数据结构拓扑
...
### 10.5.2\_每任务区\_为什么\_nesting\_不等于已经通知远端
...
### 10.5.3\_每\_CPU\_区\_各自写自己的\_rcu\_data
...
调用 `synchronize_rcu()` 的写者并不是自己逐个访问所有 CPU 状态。默认普通路径经 `wait_rcu_gp(call_rcu_hurry)` 排队一个唤醒回调；回调加速路径把所需代际写入 `rcu_data/rcu_node.gp_seq_needed`，到根节点时才设置 `rcu_state.gp_flags` 并唤醒 GP kthread。GP kthread负责初始化 `qsmask`、等待节点报告、执行 force-QS 扫描和结束 GP，成熟回调最后调用 `wakeme_after_rcu()` 完成写者的 completion。

Linux 6.12 还提供由 `rcu_normal_wake_from_gp` 控制的批量等待路径：请求项进入 `rcu_state.srs_next`，`start_poll_synchronize_rcu()` 推动 GP，`rcu_sr_normal_gp_cleanup()` 在 GP 清理阶段直接完成一部分请求并把剩余清理交给 workqueue。两条实现路径共享同一个 GP 安全条件；不能把可选批量分支说成所有内核、所有配置上的唯一路径。
...
### 10.6.1\_先看完整通信图\_谁写状态\_谁读状态\_谁通知谁

Tree RCU 不是没有通信，而是把通信从“每次读侧进入/退出”移到“GP 开始、QS 发生、任务被抢占、GP 强制推进”这些事件上。下面这张关系图先固定每个角色负责的动作和共享载体：

```mermaid
flowchart LR
    W[写者任务] -->|排队等待项并提交GP目标| Q[本CPU回调队列<br/>或srs_next]
    Q -->|gp_seq_needed汇聚<br/>根设置gp_flags并唤醒| G[GP kthread]
...
| 交互 | 主动方 | 接收或被访问方 | 发生时机 |
| --- | --- | --- | --- |
| GP 请求 | 写者任务与回调加速路径 | 本 CPU 回调队列、`gp_seq_needed`，最终到 GP kthread | `synchronize_rcu()` 或回调需要新 GP |
...
sequenceDiagram
    ...
    W->>GP: synchronize_rcu经回调/代际请求链提交GP
...
### 10.6.3\_第二步\_nesting\_只在需要时转换成共享通知
...
### 10.6.4\_第三步\_CPU\_怎样主动完成本轮\_QS\_报告
...
CPU 长时间处于 idle 或用户态时，可能不运行内核 RCU 读侧。Tree RCU 通过 context tracking 的 watching 序列判断 CPU 是否：
...
- [GP 全局生命周期源码实现](../../../../../research/source_reading/rcu/source_explanations/P05_Linux_6.12_Tree_RCU_GP全局生命周期源码实现.md#5.5.1_先从内核启动链定位early_initcall)：从启动期创建长期任务，到请求、证明汇聚、cleanup 与再次休眠的控制主线。
...
- [`tree.c`](../../../../../research/source_reading/linux/kernel/rcu/tree.c)：GP 请求、`qsmask`、QS 上报、`rcu_sched_clock_irq()` 和 force-QS。
- 外部 Linux 6.12.20 `kernel/context_tracking.c` 与 `include/linux/context_tracking_state.h`：`context_tracking.state`、用户态/idle 的 watching 转换；本仓库未复制这两个大文件，取证路径记录在 [Linux 源码基线](../../../../../research/source_reading/linux/SOURCE_BASELINE.md)。

上一篇：[RCU 机制完善：硬件与运行约束](P09_RCU_机制完善_硬件与运行约束.md)。

下一篇：[Tree RCU 初始化、拓扑与执行上下文](P11_Tree_RCU_初始化_拓扑与执行上下文.md)。
