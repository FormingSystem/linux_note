...
实现家族、reader 语义与接口包装的总分类统一见 [P22 的三个正交维度](P22_RCU_实现家族与内核配置.md#22.2_三个正交维度)；若要比较 Tiny RCU 与 `!CONFIG_PREEMPT_RCU` 下的 Tree RCU，应回看 [P05 对标题限定和证明现场的说明](P05_非抢占式_Tree_RCU_问题与证明模型.md#5.1.1_标题里的两个限定不是同义关系)。本章只展开 Tasks 与 Tiny 各自改变了哪一个维度。
