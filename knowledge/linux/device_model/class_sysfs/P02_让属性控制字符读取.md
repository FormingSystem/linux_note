...
保存下面完整程序为 `control_probe.c`。set_enabled 用一次 write 提交两字节属性请求，out 路径再次写入 1 并检查恢复结果；本程序不把普通字符读取的 fd 交给属性接口。它使用 Linux 用户空间头文件和 C11 编译器，编译后在装有对应教学模块的目标运行：
...
```bash
cc -std=c11 -Wall -Wextra -Werror -pedantic control_probe.c -o control_probe
sudo ./control_probe
sudo rmmod note_control
```

预期先报告 first=hello，禁用时报告 EACCES，再启用后核对剩余的七字节 ` class\n`，最后报告 end=0。第二次成功读取不应再次出现 hello，因为失败的读取没有消费字节，也没有重置位置。这里描述的是实验预测，实际目标装卸和输出仍须在匹配内核上执行。
