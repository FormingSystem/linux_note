---
id: knowledge.linux.device_model.设备模型简介.p05_生命周期_依赖与属性体系
title: "生命周期 依赖与属性体系"
kind: mechanism
status: evolving
domains:
...
## 5.1\_设备模型中的引用计数与生命周期管理(get\_device\_/\_put\_device\_/\_kref\_机制)
...
### 5.1.3\_数据结构视角\_kref\_与\_device

#### (1)\_struct\_kref(核心计数结构)
...
### 5.1.4\_device\_initialize()\_初始化阶段
...
### 5.1.5\_get\_device()\_与\_put\_device()
...
### 5.1.6\_device\_release()\_生命周期终点
...
### 5.1.9\_引用计数在\_driver\_core\_各阶段的使用位置
...
## 5.2\_设备模型中的资源管理与\_devm\_*\_接口机制
...
### 5.2.4\_devm\_kzalloc()\_最常用的资源申请接口
...
### 5.2.6\_devres\_release\_all()\_自动释放机制
...
### 5.2.9\_实际示例\_带\_GPIO\_和\_IRQ\_的\_LED\_驱动
...
## 5.3\_device\_link\_与电源依赖关系(Power\_Domain\_Dependencies)
...
### 5.3.3\_核心数据结构\_struct\_device\_link
...
### 5.3.4\_device\_link\_add()\_建立设备依赖关系
...
### 5.3.7\_device\_link\_del()\_删除依赖关系
...
### 5.3.8\_dev\_pm\_domain\_attach\_by\_id()\_自动建立\_link\_示例
...
### 5.3.9\_电源域\_suspend/resume\_顺序控制
...
### 5.3.10\_可视化\_suspend/resume\_调用依赖顺序
...
### 5.3.13\_device\_link\_的通用用途(除电源外)
...
## 5.4\_bus\_type\_与\_class\_的深入实现机制(从\_bus\_register\_到\_sysfs\_呈现)
...
### 5.4.3\_struct\_bus\_type\_总线类型核心结构
...
### 5.4.5\_struct\_class\_设备功能分类核心结构
...
### 5.4.6\_class\_register()\_注册功能类入口
...
### 5.4.7\_subsys\_system\_register()\_核心封装函数
...
### 5.4.8\_kset\_create\_and\_add()\_创建集合容器
...
### 5.4.9\_可视化\_bus/class/kobject\_层次关系
...
### 5.4.11\_device\_create()\_class\_层生成节点
...
### 5.4.12\_示例\_Platform\_总线与\_LED\_类
...
### 5.4.13\_driver\_core\_中\_bus/class\_的数据关系
...
## 5.5\_driver\_core\_与模块系统的交互机制(THIS\_MODULE\_与引用保护)
...
### 5.5.8\_remove()\_调用链中的\_module\_put()
...
### 5.5.12\_probe/remove\_与模块引用关系表
...
## 5.6\_热插拔与设备动态创建机制(Hotplug\_&\_Dynamic\_Device)
...
### 5.6.8\_devtmpfs\_设备节点的内核自动挂载系统
...
### 5.6.9\_设备节点的自动创建流程(含\_devtmpfs)
...
## 5.7\_device\_create()\_与\_sysfs\_属性自动生成机制
...
### 5.7.3\_device\_create()\_原型与核心逻辑
...
### 5.7.4\_device\_add()\_与\_sysfs\_目录生成
...
### 5.7.5\_devtmpfs\_与\_/dev\_节点同步
...
### 5.7.6\_可视化\_device\_create()\_到节点生成流程
...
### 5.7.7\_DEVICE\_ATTR()\_宏\_创建\_sysfs\_属性文件
...
### 5.7.12\_驱动中\_device\_create()\_与\_cdev\_create()\_的区别
...
## 5.8\_sysfs\_属性机制的底层原理与实现(kobj\_attribute\_与属性访问路径)
...
### 5.8.4\_struct\_attribute(属性基础单元)
...
### 5.8.5\_struct\_kobj\_attribute(属性\_+\_操作)
...
### 5.8.6\_struct\_sysfs\_ops\_属性访问调度表
...
### 5.8.7\_device\_attr\_show()/store()\_调用路径
...
### 5.8.8\_kobj\_type\_与\_sysfs\_ops\_绑定
...
### 5.8.9\_sysfs\_文件创建函数\_sysfs\_create\_file()
...
### 5.8.10\_kobject\_create\_and\_add()\_对象层注册
...
### 5.8.11\_device\_create\_file()\_内部关系复盘
...
### 5.8.13\_sysfs\_emit()\_与\_sprintf()\_的区别
...
## 5.9\_class\_attribute\_与\_driver\_attribute\_的实现与差异
...
### 5.9.3\_struct\_class\_attribute
...
### 5.9.4\_struct\_driver\_attribute
...
### 5.9.5\_注册接口\_class\_create\_file()\_与\_driver\_create\_file()
...
## 5.10\_bus\_attribute\_与子系统属性机制(总线级属性注册)

### 5.10.1\_主题引入\_总线级属性的角色
...
### 5.10.2\_struct\_bus\_attribute\_定义与成员说明
...
### 5.10.3\_总线属性注册函数\_bus\_create\_file()
...
### 5.10.4\_删除接口\_bus\_remove\_file()
...
### 5.10.5\_bus\_attribute\_宏定义(简化声明)
...
### 5.10.7\_示例\_为\_platform\_总线添加属性文件
...
### 5.10.8\_bus\_属性的\_show()/store()\_调用流程
...
### 5.10.9\_subsys\_private\_与\_bus\_attribute\_的关系
...
### 5.10.10\_subsys\_system\_register()\_与\_bus\_register()\_的关系
...
### 5.10.13\_小结\_bus\_attribute\_的语义与应用场景
