...
## 4.1_设备模型与模块加载_从_insmod_到_probe_的全流程
...
### 4.1.4_驱动注册阶段_platform_driver_register()
...
### 4.1.5_driver_register()_驱动正式接入系统
...
### 4.1.8_设备匹配阶段_driver_match_device()
...
### 4.1.9_设备绑定阶段_driver_probe_device()
...
## 4.2_driver_core_设备匹配机制详解_from_of_match_device()_到_of_match_node()
...
### 4.2.3_第一层入口_driver_match_device()
...
### 4.2.5_第三层_of_driver_match_device()
...
#### (1)_关键调用_of_device_is_compatible()
...
## 4.3_设备树匹配的高级机制与多层继承(compatible_列表与_fallback_匹配)
...
### 4.3.6_使用_of_device_get_match_data()_提取匹配数据
...
## 4.4_从_device_node_到_platform_device_设备树节点在设备模型中的注册过程
...
### 4.4.4_of_platform_populate()_设备树扫描入口
...
### 4.4.6_of_device_add()_从节点到_platform_device
...
### 4.4.7_of_platform_device_create_pdata()
...
### 4.4.8_platform_device_register_full()
...
### 4.4.14_of_alias_get_id()_别名_ID_的生成
...
## 4.5_platform_device_与_platform_driver_的绑定过程(device_attach_与_probe_调用)
...
### 4.5.4_driver_attach()_驱动遍历设备的起点
...
### 4.5.5_driver_attach()_匹配与_probe_调用调度点
...
### 4.5.6_device_attach()_驱动与设备绑定入口
...
### 4.5.7_driver_probe_device()_执行真正的_probe_调用
...
### 4.5.8_really_probe()_绑定与错误回滚的核心
...
### 4.5.9_device_bind_driver()_建立_sysfs_双向链接
