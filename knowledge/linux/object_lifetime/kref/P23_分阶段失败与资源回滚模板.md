...
材料[rollback_ownership.c](../../../../labs/kernel/object_lifetime/materials/rollback_ownership.c)直接使用C11和标准库。两种policy各运行正常关闭和五个失败阶段；refs是顺序责任账本，不实现Linux kref、原子或并发内存序。registry也只模拟一个拥有型槽，不是Linux注册API。
...
下一篇：[返回删除与排空模板](P13_工程模板.md#13.6.1_模板十_remove_/_unlink_/_drain_/_release_模板)。
