...
用三个已经讲清的例子来判断下面的说法：命名对象主动撤下后仍有观察者，设备注销后仍有独立引用，会话最后退出才归还桥接设备份额。每个错误都不是因为少记了一个接口，而是把不同状态或存储合并推理了。

### 11.6.1\_误解一\_kobject\_是高级\_kref

kobject 在引用之外提供名称、类型清理与对象层次关系，可以连接 sysfs 和 kset。它并非每次初始化就自动出现在 sysfs，也不要求每个实例都加入 kset。本章的独立命名对象就是在初始化后再添加，再主动撤下。

如果只是想让统计对象被多人共享，引入 kobject 还会带来命名、登记和失败清理责任，而不会改善原来的所有权证明。继续使用 kref 即可；已有类型封装也可以直接使用 refcount 原语。是否需要对象模型，由真实登记与组织需求决定。

------

### 11.6.2\_误解二\_device\_就是内嵌\_kref\_的对象

内嵌只描述内存布局。device 的初始化、添加、注销和最终 release 共同规定引用责任，不能由调用者直接操作内部 kref 替换。回想完整设备模块：`device_unregister` 消费初始化份额，独立观察者最后 `put_device` 才完成外壳清理。如果照自定义列表的模板在 unregister 后再 put 一次“创建者引用”，就可能提前清掉观察者仍需要的存储。

同样，设备地址仍有效不表示它仍绑定驱动，也不表示硬件或 devm 资源可继续使用。存储引用、业务关闭和资源排空必须分别判断。

------

### 11.6.3\_误解三\_class/bus\_是\_device\_的总引用管理器

同一设备可以同时有功能分类、匹配规则和父子关系，这些是不同的组织轴。公共 class/bus_type 描述与 core 内部对象也未必在同一块分配。若把它们画成“大引用拥有小引用”的单一树，就无法解释为什么内部 bus_release 只释放内部私有对象，或者为什么 class.class_release 与 class.dev_release 针对不同对象。

判断退出顺序时，应回到真实注册关系和使用者协议。注销分类或总线不是替任意设备驱动自动停止业务的命令；普通使用者也不应去操纵内部 kset 引用来代替公开接口。

------

### 11.6.4\_误解四\_release\_都是\_kfree

release 是协议选定的最后清理入口，工作内容取决于对象。它可能释放独立资源或归还子对象份额，也可能把实际回收交给 RCU 回调。会话示例中的 release 先释放会话，再归还设备；设备框架还要按自己的规则处理内部状态。回调参数是 kref、kobject 还是 device，也决定了它从哪一条协议进入。

但是不能把“可能清理很多东西”理解成“什么退出动作都可以拖到最后 put”。撤下公开入口、阻止新操作、取消或排空异步活动，往往必须先完成，才能证明会到达最后一份。若异步活动自己持有引用，却等到最后 release 才取消它，就可能根本等不到 release。若回调在持锁或不可睡眠上下文执行，随意等待还会引入阻塞或死锁。

本章的 kobject 示例区分主动撤下与最终类型清理；设备示例区分注销与最终回收；会话示例把关闭门放在最后释放以前。清理责任可以分布在这些阶段，但每一项必须有确定的执行者与配对依据。`CONFIG_DEBUG_KOBJECT_RELEASE` 还能改变实际类型清理的时点，所以示例明确拒绝该配置，而不是宣称所有最后 put 都立即执行全部清理。

------

### 11.6.5\_误解五\_get\_device\_可以替代私有\_kref

如果私有字段和 device 同属一块外壳，并由设备 release 唯一回收，那么设备引用可以维持这块外壳的存储；不应仅因为字段叫“私有”就再加一套计数。但字段指向的另一块会话、请求或文件上下文分配，不会因为设备仍在就自动有效，其寿命要另行证明。

完整会话程序采用独立分配，所以需要会话自己的引用协议。即使持有会话引用，也仍需锁来读取统计、需 closing 来决定是否接受请求。引用数量不编码这些业务状态。

------

### 11.6.6\_误解六\_私有\_kref\_可以替代\_get\_device

私有引用不能凭空维持另一块设备存储。它可以通过一条明确的持有关系间接保证设备有效：本章每个会话整体拥有设备一份，私有计数非零时该份额还没有归还，因此会话方法可以沿 owner 访问设备锁。这是应用建立的桥接协议，不是 kref 自动理解了 owner 指针。

没有这份桥接引用时，仅把 `struct device *` 保存进会话还不足以延长设备寿命。若设计选择借用而非拥有，就必须证明全部会话访问都在一个更短的、由其他设备拥有者保障的期限内。两种协议可以各自成立，但不能在函数返回一个长期使用的会话以后，仍假设调用者最初的临时设备引用永远存在。

------

## 11.7\_选择规则

首先判断是否已有框架对象和公开契约。已经拿到 device 时，应按设备协议取得、归还和注销；不能因为它也出现在 sysfs 就越过设备层选择 kobject API。只有设计新对象时，才从需要的能力选择合适层次。

```mermaid
flowchart TD
    A[确定要延长的是哪块存储] -->|先查现有身份| B{已有框架对象与公开契约吗}
    B -->|是| C[沿device等所属框架接口配对]
    B -->|否| D{需要设备模型登记与协作吗}
    D -->|是| E[按device及所属子系统设计退出]
    D -->|否| F{需要独立名称和对象层次吗}
    F -->|是| G[设计kobject类型和登记责任]
    F -->|否| H{是否是共享寿命计数}
    H -->|是| I[kref或已有refcount类型封装]
    H -->|否| J[按真实状态选择普通或原子变量]
    C -->|跨对象保存指针时| K[明确借用期限或桥接份额]
    E -->|跨对象保存指针时| K
    G -->|跨对象保存指针时| K
    I -->|跨对象保存指针时| K
```
...
| 需求或约束 | 选择依据 | 需要支付的责任 |
| --- | --- | --- |
| 通用原子状态，不表示存储份额 | atomic_t或与场景匹配的同步方式 | 定义状态及内存序，不把它自动当成引用 |
| 已有专用类型封装的引用计数 | refcount_t可以直接作为内部原语 | 统一取得、归还和清理协议 |
| 自定义共享对象且需要统一release形式 | kref | 发布窗口、份额来源、字段同步和退出上下文 |
| 独立命名及对象层次 | kobject | 初始化后的失败责任、登记撤下和类型清理 |
| 已有或新增设备实例 | 所属设备框架接口 | 初始化份额、注册/注销、绑定与资源退出 |
| 新功能分类或匹配体系 | class或bus的实际子系统职责 | 公共描述、内部登记及成员使用者退出 |

这不是要求每个驱动都建一套 class 和 bus。已有子系统能提供所需组织关系时，应使用它；仅为引用计数而增加分类或总线只会扩大退出责任。接口选定后，再决定是否存在独立会话、请求或共享数据块，以及它们怎样引用主对象。

------

## 11.8\_和前面章节的关系

前面建立的“get以前地址有效、取得有依据、每份恰好归还一次”仍适用于框架对象。本章改变的是 **到哪里寻找和履行这些责任**，不是宣称框架对象不受引用原则约束。对私有对象，责任由自己的发布和清理函数定义；对设备，必须把框架包装已经取得或归还的份额计算进去。

第2章的结构模型解释为什么release能够从内嵌成员恢复外壳；第3～4章的生命周期和基本规则说明份额如何闭合；第8～10章的查找窗口、锁和RCU说明取得以前地址为什么有效。本章再加入名字、设备登记、分类和私有会话，沿这些原则检查框架边界。锁或RCU并未因为换成device就自动出现，长期使用指针以前仍要从所属接口获得有效期限。

接下来第12章按具体错误和观察线索定位责任缺口，第13～15章继续进入模板、实验和验收。后续材料须沿用这里已经区分的存储、业务和资源状态，而不是把所有对象重新抽象成一个没有框架约束的my_obj。

------

## 11.9\_本章检查清单

拿一段实际代码，先画分配与持有关系，再逐条回答。结构体嵌套图只能表示布局，必须另外标出谁拥有谁的一份。

1. 每个指针指向哪块分配？它属于私有对象还是已有框架对象？首次进入函数时谁保证地址有效？
2. 各层使用哪个公开取得/归还接口？初始化、注册失败和注销内部已经处理了哪一份？
3. 每块分配由哪个唯一入口回收？如果同一块内存有两个计数，什么关系保证任一方仍在使用时外壳不会先释放？
4. 名称、sysfs、uevent、父关系、分类、总线和驱动绑定中，哪些真实参与本对象？这些状态何时撤下，由谁负责？
5. release参数属于哪种内嵌对象？类型分派如何进入应用回调，回调在何种上下文、何种配置下执行？
6. 私有指针是借用还是拥有？若由会话桥接设备引用，取得和归还是否恰好各一次，有没有反向拥有造成引用环？
7. 设备仍在时，硬件、devm资源和独立子分配是否可能已经退出？哪个锁、关闭门或排空协议使业务访问合法？
8. 最后归还之后是否还读取了原对象或其内嵌锁？模块退出以前是否已经结束使用回调代码的路径？

用三道题检验，而不只默读“我知道”。

**题一：预测。** 在设备示例中，添加成功后取得两个独立观察者，调用一次unregister，再分别归还观察者。假定没有其他引用，设备release发生在哪一步？先写出3→2→1→0：unregister把3变2，第一个观察者把2变1，第二个才触发最后清理。若注销后立刻再put“初始份额”，就是重复消费。

**题二：解释。** 会话程序的最后release为什么先保存设备地址，释放会话，再put设备？会话存储被释放后不能再读取owner；提前保存的地址仍由尚未归还的桥接份额保证有效。put设备可能释放设备外壳，所以它之后也不能再访问该设备。这是一条有先后顺序的责任链，不是两个任意排列的free。

**题三：修改设计。** 想让设备退出后还可读取会话统计，但不希望会话延长设备外壳寿命，能否直接删去get/put_device？不能：当前会话仍会访问owner.lock和closing。必须先改数据所有权，例如将最终统计和会话锁放在独立会话内，并设计退出时移交结果、切断owner访问的同步协议；只有证明再无设备访问后才能取消桥接。单删一对引用操作不等于完成寿命解耦。

------

## 11.10\_本章小结

本章从普通计数工具走到命名对象、设备框架和独立会话。核心原则可以沿三个实例重新复述：撤下命名对象不等于归还本对象的全部引用；注销设备已经消费初始化份额，但独立观察者仍可能存在；会话引用通过明确持有的一份设备引用连接存储寿命，业务关闭仍由锁和状态决定。

引用原语、框架对象和业务状态分别回答不同的问题。使用kref并不要求创建kobject，操作device应沿设备框架接口；class和bus的公共描述与内部对象要按真实登记和清理关系理解。对于两种计数共享一块分配的设计，必须证明唯一最终销毁协议，不能用“两层生命周期”这句话代替它。

读到这里，应当能够为新对象选择所需层次，并沿成功、失败和退出路径把份额配平。还不能仅凭“没崩溃”证明实现正确；下一章从多put、少put、首次取得窗口和业务退出混淆等故障出发，追踪应该观察什么、每种工具能证明到哪里。
...
下一篇：[典型错误模式与调试线索](P12_典型错误模式与调试线索.md#12.1_本章导读_把错误模式归类排查)。
