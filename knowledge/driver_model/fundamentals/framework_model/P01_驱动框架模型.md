...
把下面的完整程序保存为 sysfs_view.py，或取用[同名材料](../../../../labs/kernel/driver_entries/materials/sysfs_view.py)。它只读目录，默认列前五个 platform 设备；第二行给出解析后的对象位置，第三行给出当前观察到的驱动名。
...
```bash
python3 sysfs_view.py
```

机器上的设备名由实际环境决定，所以不提供一组假定每台机器都应出现的名字。如果看到 object 指向 /sys/devices，而 driver 有一个名称，说明这次读取看到了设备拓扑与驱动关联。若是 no driver link，可能尚未成功绑定，也可能正在转换状态；仅凭这条输出无法确定原因，更不能宣布硬件不存在。
