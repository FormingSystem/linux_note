...
## 6.1\_设备生命周期\_分配\_/\_注册\_/\_注销\_open/close\_devres\_vs\_非\_devres

### 6.1.1\_是什么\_Input\_设备生命周期的\_主干线
...
### 6.1.2\_分配\_/\_注册\_/\_注销三件套\_四连问总表
...
### 6.1.3\_input\_allocate\_device()\_/\_devm\_input\_allocate\_device()\_四连问
...
### 6.1.4\_input\_register\_device()\_/\_input\_unregister\_device()\_四连问
...
### 6.1.6\_devres\_vs\_非\_devres\_接口对比与适用场景

#### (1)\_devres\_与非\_devres\_的角色划分
...
### 6.1.7\_小结\_把生命周期\_API\_固定成可复用\_骨架
...
## 6.2\_能力与属性声明\_evbit/keybit/absbit/propbit\_与\_DIRECT/POINTER
...
### 6.2.1\_概念总览\_能力位图\_vs\_轴参数\_vs\_事件流
...
### 6.2.2\_evbit\_声明事件类型(EV\_KEY/EV\_ABS/EV\_REL/EV\_SW)
...
### 6.2.3\_keybit\_声明按键\_/\_按钮集合与\_语义空间
...
### 6.2.4\_absbit\_声明绝对轴集合(与\_6.3\_的连接点)
...
### 6.2.5\_propbit\_属性声明与\_DIRECT/POINTER
...
### 6.2.6\_能力组合模板\_键盘\_/\_鼠标\_/\_触摸屏\_/\_摇杆
...
### 6.2.8\_小结\_先声明\_能干什么\_再谈\_怎么干得好
...
#### (2)\_驱动侧映射\_硬件\_to\_(type,\_code,\_value)
...
## 6.3\_绝对轴参数族\_min/max/fuzz/flat/res(重点)
...
### 6.3.3\_设备物理空间与\_ABS\_参数\_计数\_像素\_物理量
...
### 6.3.4\_触摸屏\_vs\_摇杆\_fuzz/flat/res\_的典型配置差异
...
### 6.3.5\_可视化\_ABS\_参数从驱动到用户态的使用链路
...
### 6.3.6\_示例代码\_驱动和用户态各一段

#### (1)\_驱动侧\_触摸屏\_ABS\_参数初始化
...
#### (2)\_用户态\_读取\_EVIOCGABS\_+\_使用\_resolution\_做物理换算
...
### 6.3.7\_调试与验证\_如何判断\_ABS\_参数配置正确
...
### 6.3.8\_小结\_把\_ABS\_参数当作\_设备物理描述的一部分
...
### 6.4.1\_单点\_vs\_多点\_为什么需要\_MT-B

#### (1)\_是什么\_MT-A\_/\_MT-B\_的区别
...
### 6.4.2\_槽位(slot)与\_tracking\_id\_核心抽象
...
### 6.4.4\_MT-B\_的设计边界\_驱动与用户态各自负责什么
...
### 6.5.1\_采集可睡\_上报不睡\_上下文与调用边界

#### (1)\_引入\_为什么要把\_采集\_和\_上报\_拆成两个阶段
...
#### (2)\_数据结构视角\_input\_core\_内部的\_短临界区
...
##### 2)\_传统\_bottom\_half\_softirq\_/\_tasklet(不可睡)
...
##### 3)\_线程化\_IRQ\_handler\_request\_threaded\_irq()\_的\_thread\_fn(可睡)
...
##### 5)\_进程上下文\_probe/open/close/ioctl\_等(可睡)
...
#### (4)\_用户/平台视角\_延迟\_vs\_抖动与\_CPU\_负载
...
#### (6)\_示例代码\_在不同上下文中拆分采集与上报

下面用两个最常见的模式示例化：

##### 1)\_线程化\_IRQ\_中断触发采样\_+\_上报(推荐)
...
### 6.5.2\_设备级顺序\_input\_core\_自旋锁与处理层环形缓冲
...
#### (2)\_数据结构视角\_input\_dev\_与\_handler\_的关键字段
...
#### (3)\_开发者视角\_哪些顺序内核帮你兜底\_哪些需要你自己保证
...
#### (4)\_用户\_/\_平台视角\_read()\_/\_poll()\_看到的顺序
...
#### (5)\_可视化\_驱动\_to\_input\_core\_to\_evdev\_to\_用户态的顺序链路
...
#### (8)\_小结\_把\_顺序\_留给内核\_自己只负责\_单线程上报
...
### 6.5.3\_帧一致性与\_TOCTOU\_帧号\_/\_前后检查\_data-ready\_+\_有限重读
...
#### (3)\_开发者视角\_两种常见防\_TOCTOU\_方案

##### 1)\_方案\_A\_帧号(frame\_id)前后一致性检查
...
##### 2)\_方案\_B\_data-ready\_前后检查\_+\_有限重读(无帧号控制器)
...
#### (4)\_用户/平台视角\_为什么\_丢帧\_比\_乱帧\_更容易处理
...
### 6.5.4\_关机/休眠顺序\_disable\_irq\_sync()\_to\_改电源/状态\_to\_唤醒源配置
...
#### (2)\_数据结构视角\_suspend\_状态与\_IRQ/工作队列的关联
...
#### (3)\_开发者视角\_推荐的\_suspend\_/\_resume\_顺序

以下针对“系统 suspend / resume”（`struct dev_pm_ops` 的 `.suspend` / `.resume`）给出推荐顺序。runtime PM 可以在此基础上做裁剪。

##### 1)\_设备\_suspend\_停事件\_to\_停\_IRQ/工作\_to\_改电源\_to\_配唤醒
...
##### 2)\_设备\_resume\_复位电源/状态\_to\_重新启用\_IRQ/工作\_to\_解锁上报
...
#### (5)\_可视化\_系统\_suspend/resume\_与\_IRQ/电源的顺序关系
...
#### (6)\_示例代码\_demo\_触摸屏的\_suspend\_/\_resume\_骨架
...
#### (2)\_数据结构与机制\_evdev\_环形缓冲与\_SYN\_DROPPED
...
#### (6)\_示例代码\_用户态处理\_SYN\_DROPPED\_的最小骨架
...
### 6.5.6\_并发语义交付核对表与调试\_Checklist
...
#### (4)\_用户/平台视角\_API\_使用\_Checklist
...
#### (5)\_可视化\_并发语义整体视图(上下文\_+\_帧一致性\_+\_PM\_+\_背压)
