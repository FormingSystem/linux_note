...
## 6.1_设备生命周期_分配_/_注册_/_注销_open/close_devres_vs_非_devres

### 6.1.1_是什么_Input_设备生命周期的_主干线
...
### 6.1.3_input_allocate_device()_/_devm_input_allocate_device()_四连问
...
### 6.1.4_input_register_device()_/_input_unregister_device()_四连问
...
### 6.1.6_devres_vs_非_devres_接口对比与适用场景

#### (1)_devres_与非_devres_的角色划分
...
## 6.2_能力与属性声明_evbit/keybit/absbit/propbit_与_DIRECT/POINTER
...
### 6.2.2_evbit_声明事件类型(EV_KEY/EV_ABS/EV_REL/EV_SW)
...
### 6.2.3_keybit_声明按键_/_按钮集合与_语义空间
...
### 6.2.4_absbit_声明绝对轴集合(与_6.3_的连接点)
...
### 6.2.5_propbit_属性声明与_DIRECT/POINTER
...
### 6.2.6_能力组合模板_键盘_/_鼠标_/_触摸屏_/_摇杆
...
#### (2)_驱动侧映射_硬件_to_(type,_code,_value)
...
## 6.3_绝对轴参数族_min/max/fuzz/flat/res(重点)
...
### 6.3.3_设备物理空间与_ABS_参数_计数_像素_物理量
...
### 6.3.4_触摸屏_vs_摇杆_fuzz/flat/res_的典型配置差异
...
### 6.3.6_示例代码_驱动和用户态各一段

#### (1)_驱动侧_触摸屏_ABS_参数初始化
...
#### (2)_用户态_读取_EVIOCGABS_+_使用_resolution_做物理换算
...
### 6.3.8_小结_把_ABS_参数当作_设备物理描述的一部分
...
### 6.4.1_单点_vs_多点_为什么需要_MT-B

#### (1)_是什么_MT-A_/_MT-B_的区别
...
### 6.4.2_槽位(slot)与_tracking_id_核心抽象
...
### 6.5.1_采集可睡_上报不睡_上下文与调用边界

#### (1)_引入_为什么要把_采集_和_上报_拆成两个阶段
...
##### 2)_传统_bottom_half_softirq_/_tasklet(不可睡)
...
##### 3)_线程化_IRQ_handler_request_threaded_irq()_的_thread_fn(可睡)
...
##### 5)_进程上下文_probe/open/close/ioctl_等(可睡)
...
#### (6)_示例代码_在不同上下文中拆分采集与上报

下面用两个最常见的模式示例化：

##### 1)_线程化_IRQ_中断触发采样_+_上报(推荐)
...
### 6.5.2_设备级顺序_input_core_自旋锁与处理层环形缓冲
...
#### (2)_数据结构视角_input_dev_与_handler_的关键字段
...
#### (4)_用户_/_平台视角_read()_/_poll()_看到的顺序
...
#### (5)_可视化_驱动_to_input_core_to_evdev_to_用户态的顺序链路
...
### 6.5.3_帧一致性与_TOCTOU_帧号_/_前后检查_data-ready_+_有限重读
...
#### (3)_开发者视角_两种常见防_TOCTOU_方案

##### 1)_方案_A_帧号(frame_id)前后一致性检查
...
##### 2)_方案_B_data-ready_前后检查_+_有限重读(无帧号控制器)
...
### 6.5.4_关机/休眠顺序_disable_irq_sync()_to_改电源/状态_to_唤醒源配置
...
#### (2)_数据结构视角_suspend_状态与_IRQ/工作队列的关联
...
##### 2)_设备_resume_复位电源/状态_to_重新启用_IRQ/工作_to_解锁上报
...
#### (5)_可视化_系统_suspend/resume_与_IRQ/电源的顺序关系
...
#### (6)_示例代码_demo_触摸屏的_suspend_/_resume_骨架
...
#### (2)_数据结构与机制_evdev_环形缓冲与_SYN_DROPPED
...
#### (6)_示例代码_用户态处理_SYN_DROPPED_的最小骨架
...
#### (5)_可视化_并发语义整体视图(上下文_+_帧一致性_+_PM_+_背压)
