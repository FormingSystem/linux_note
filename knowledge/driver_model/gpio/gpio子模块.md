...
# 第1章\_GPIO\_总览与生态演进(以\_6.1\_为基线\_按时间线叙述)
...
## 2.4\_用户视角(他们能看到/怎么验证)

### 2.4.1\_从运行系统导出\_DT\_并检查
...
### 2.4.2\_用\_libgpiod\_验证\_DT\_与标注
...
#### (1)\_[struct\_pinctrl\_desc](#struct\_pinctrl\_desc)(定义一个\_pinctrl\_控制器)
...
#### (2)\_[struct\_pinctrl\_state](#struct\_pinctrl\_state)(一组\_状态设置\_如\_default/sleep)
...
#### (3)\_[struct\_gpio\_chip](#struct\_gpio\_chip)(GPIO\_控制器抽象)
...
# 第5章\_字符设备\_ABI\_与\_libgpiod(用户态)
...
### 5.5.2\_单线输入\_+\_边沿事件(RISING/FALLING)
...
# 第6章\_GPIO\_中断(IRQ)与\_gpio-keys\_/\_自定义驱动
...
## 6.4\_开发者视角(二)\_自定义\_platform\_驱动\_+\_threaded\_IRQ

当你要在**内核驱动里**把 GPIO 中断与设备逻辑深度耦合（不是 input 事件），用以下模板。

### 6.4.1\_设备树(与第\_3\_章风格一致)
...
## 7.4\_gpio-keys(按键\_to\_input\_事件)
...
## 7.7\_regulator-fixed(GPIO\_使能的稳压器)
...
## 7.8\_gpio-mux(用\_GPIO\_选择外部多路器通道)
...
## 7.9\_gpio-poweroff\_/\_gpio-restart(GPIO\_控制关机/重启)
...
### 8.2.2\_消费者节点(consumer)与\_<name>-gpios
...
### 8.2.3\_flags\_的作用域(与\_pinconf\_的关系)
...
### 8.2.4\_gpio-ranges\_与\_pinctrl\_的对齐
...
## 8.3\_开发者视角(通用写法与最小示例)

### 8.3.1\_模式\_A\_单根线(必选)
...
### 8.3.4\_模式\_D\_开漏(Open-Drain)与极性
...
### 8.3.5\_模式\_E\_gpio-hog(默认拉低外设使能)
...
### 8.3.6\_模式\_F\_gpio-line-names(统一命名与测试)
...
### 8.4.3\_交叉验证\_pinctrl(与第\_3\_章配合)
...
# 第9章\_GPIO\_扩展器与桥接器(I²C\_/\_SPI\_/\_PMIC)
...
## 9.3\_设备树绑定(Provider\_与\_Consumer)

> 下面是**典型片段**，用于说明结构与关键属性；请按目标器件的 binding 文档具体调整。

### 9.3.1\_I²C\_MCP23017(带中断)
...
### 9.3.3\_I²C\_PCF8574(无专用中断\_适合轮询或简化输入)
...
## 9.4\_开发者视角\_驱动使用范式(\_cansleep\_+\_threaded\_IRQ)

### 9.4.1\_获取与读写(逻辑值\_线程上下文)
...
### 9.4.2\_扩展器线级中断(threaded\_IRQ\_模板)
...
## 10.6\_用户态路径(libgpiod\_2.x)性能要点
...
### 10.10.1\_ftrace/trace-cmd\_快速测时
...
### 11.2.2\_状态机(pinctrl)与驱动(PM)的协作
...
## 11.3\_开发者视角(DTS\_+\_驱动模板)

### 11.3.1\_设备树\_default\_/\_sleep\_两态示例(i.MX6ULL\_为例)
...
### 11.3.2\_平台驱动\_PM\_+\_pinctrl\_+\_IRQ\_wake(可编可跑骨架)
...
### 11.3.3\_可选\_运行时\_PM(autosuspend\_节电但不改变系统休眠流程)
...
## 11.4\_Bootloader\_to\_内核\_引脚接管一致性
...
### 11.6.1\_系统休眠/唤醒时序(sequenceDiagram)
...
## 12.3\_开发者视角\_最小可用操作清单

### 12.3.1\_pinctrl\_与\_GPIO\_快速体检(必做)
...
### 12.3.2\_dynamic\_debug\_只在需要时\_开口说话
...
### 12.3.3\_tracefs/ftrace\_事件与时序测量
...
### 12.3.4\_/proc/interrupts\_与亲和/优先级
...
### 12.3.6\_kselftest\_/\_gpio-sim(自动化)
...
### 12.5.2\_事件观测时序(sequenceDiagram)
...
# 第13章\_迁移与最佳实践(sysfs\_to\_chardev\_libgpiod\_v1\_to\_v2\_整数\_API\_to\_描述符\_API)
...
## 13.4\_生产脚本\_sysfs\_to\_libgpiod\_工具
...
## 13.5\_用户态\_C\_libgpiod\_v1\_to\_v2(对照与示例)
...
## 13.6\_内核驱动\_整数\_API\_to\_描述符\_API
...
## 13.7\_权限与部署\_从\_root-only\_到\_udev\_规则
...
### 13.9.2\_用户态事件管线(sequenceDiagram)
...
## 14.3\_版型\_A\_板载\_LED\_+\_按键\_+\_复位钩子\_最小实现

### 14.3.1\_设备树(LED\_+\_按键\_+\_复位钩子)
...
## 14.4\_版型\_B\_传感器(VDD\_使能\_+\_复位\_+\_唤醒按钮)组合

### 14.4.1\_设备树(regulator-fixed\_+\_复位\_+\_唤醒)
...
## 14.5\_版型\_C\_gpio-mux\_复用切换(两根位选\_to\_四路通道)
...
### 14.5.2\_极简\_policy\_驱动(sysfs:\_channel)
...
## 14.6\_版型\_D\_生产测试脚本(零侵入\_依赖\_line-names)
...
### 14.7.1\_传感器上电与事件时序(sequenceDiagram)
...
## 15.3\_设备树片段速查(Cookbook)

> 片段只示范结构与关键属性；电气参数请放到 **pinctrl/pinconf**（见第 3 章）。

### 15.3.1\_Provider\_基本形态(以\_i.MX\_为例)
...
### 15.3.5\_gpio-keys\_/\_gpio-leds(优先用子模块)
...
### 15.3.6\_regulator-fixed(GPIO\_使能电源)
...
## 15.4\_Kconfig\_/\_Kbuild\_模板(可直接用)
...
## 15.5\_Mermaid\_图元库与约定(Typora\_兼容)

> 结合你前面的反馈，这里列出**可直接复制**的图元，并强调**语法细节**，避免渲染错误。

### 15.5.1\_流程图(flowchart)
...
# 第16章\_附录\_A\_数据结构(kernel\_6.1\_为基准)

## 16.1\_struct\_gpio\_chip
