...
# 第4章\_Provider(控制器)驱动编写

我是 **GPT-5 Thinking**。

------

## 4.1\_设备树\_GPIO\_绑定通用范式
...
#### (2)\_消费者节点(consumer)与\_<name>-gpios
...
#### (3)\_flags\_的作用域(与\_pinconf\_的关系)
...
#### (4)\_gpio-ranges\_与\_pinctrl\_的对齐
...
#### (6)\_模式\_F\_gpio-line-names(统一命名与测试)
...
## 4.2\_GPIO\_扩展器与桥接器(I²C\_/\_SPI\_/\_PMIC)
...
### 4.2.3\_设备树绑定(Provider\_与\_Consumer)

> 下面是**典型片段**，用于说明结构与关键属性；请按目标器件的 binding 文档具体调整。

#### (1)\_I²C\_MCP23017(带中断)
...
#### (3)\_I²C\_PCF8574(无专用中断\_适合轮询或简化输入)
...
### 4.2.4\_开发者视角\_驱动使用范式(\_cansleep\_+\_threaded\_IRQ)
...
### 4.3.6\_用户态路径(libgpiod\_2.x)性能要点
...
### 4.3.8\_波形/比特翻转(bit-banging)的边界
...
### 4.4.3\_开发者视角(DTS\_+\_驱动模板)

#### (1)\_设备树\_default\_/\_sleep\_两态示例(i.MX6ULL\_为例)
...
#### (2)\_平台驱动\_PM\_+\_pinctrl\_+\_IRQ\_wake(可编可跑骨架)
...
#### (3)\_可选\_运行时\_PM(autosuspend\_节电但不改变系统休眠流程)
...
### 4.4.4\_Bootloader\_to\_内核\_引脚接管一致性
...
### 4.4.5\_用户视角(验证低功耗与唤醒)

#### (1)\_检查\_pinctrl\_状态切换
