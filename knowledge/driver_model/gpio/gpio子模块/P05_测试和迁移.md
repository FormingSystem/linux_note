...
### 5.1.3_开发者视角_最小可用操作清单

#### (1)_pinctrl_与_GPIO_快速体检(必做)
...
## 5.2_迁移与最佳实践(sysfs_to_chardev_libgpiod_v1_to_v2_整数_API_to_描述符_API)
...
### 5.2.4_生产脚本_sysfs_to_libgpiod_工具
...
### 5.2.5_用户态_C_libgpiod_v1_to_v2(对照与示例)
...
### 5.2.7_权限与部署_从_root-only_到_udev_规则
...
### 5.3.3_版型_A_板载_LED_+_按键_+_复位钩子_最小实现

#### (1)_设备树(LED_+_按键_+_复位钩子)
...
### 5.3.4_版型_B_传感器(VDD_使能_+_复位_+_唤醒按钮)组合

#### (1)_设备树(regulator-fixed_+_复位_+_唤醒)
...
### 5.3.5_版型_C_gpio-mux_复用切换(两根位选_to_四路通道)
...
#### (2)_极简_policy_驱动(sysfs:_channel)
...
### 5.3.6_版型_D_生产测试脚本(零侵入_依赖_line-names)
...
### 5.4.3_设备树片段速查(Cookbook)

> 片段只示范结构与关键属性；电气参数请放到 **pinctrl/pinconf**（见第 3 章）。

#### (1)_Provider_基本形态(以_i.MX_为例)
...
#### (5)_gpio-keys_/_gpio-leds(优先用子模块)
...
### 5.4.4_Kconfig_/_Kbuild_模板(可直接用)
...
### 5.4.5_Mermaid_图元库与约定(Typora_兼容)
