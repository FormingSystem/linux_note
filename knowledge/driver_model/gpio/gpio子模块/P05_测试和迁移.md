---
id: knowledge.driver_model.gpio.gpio子模块.p05_测试和迁移
title: "测试和迁移"
kind: subsystem
status: evolving
domains:
...
### 5.1.3\_开发者视角\_最小可用操作清单

#### (1)\_pinctrl\_与\_GPIO\_快速体检(必做)
...
#### (2)\_dynamic\_debug\_只在需要时\_开口说话
...
## 5.2\_迁移与最佳实践(sysfs\_to\_chardev\_libgpiod\_v1\_to\_v2\_整数\_API\_to\_描述符\_API)
...
### 5.2.4\_生产脚本\_sysfs\_to\_libgpiod\_工具
...
### 5.2.5\_用户态\_C\_libgpiod\_v1\_to\_v2(对照与示例)
...
### 5.2.6\_内核驱动\_整数\_API\_to\_描述符\_API
...
### 5.2.7\_权限与部署\_从\_root-only\_到\_udev\_规则
...
### 5.3.3\_版型\_A\_板载\_LED\_+\_按键\_+\_复位钩子\_最小实现

#### (1)\_设备树(LED\_+\_按键\_+\_复位钩子)
...
### 5.3.4\_版型\_B\_传感器(VDD\_使能\_+\_复位\_+\_唤醒按钮)组合

#### (1)\_设备树(regulator-fixed\_+\_复位\_+\_唤醒)
...
### 5.3.5\_版型\_C\_gpio-mux\_复用切换(两根位选\_to\_四路通道)
...
#### (2)\_极简\_policy\_驱动(sysfs:\_channel)
...
### 5.3.6\_版型\_D\_生产测试脚本(零侵入\_依赖\_line-names)
...
### 5.4.3\_设备树片段速查(Cookbook)

> 片段只示范结构与关键属性；电气参数请放到 **pinctrl/pinconf**（见第 3 章）。

#### (1)\_Provider\_基本形态(以\_i.MX\_为例)
...
#### (5)\_gpio-keys\_/\_gpio-leds(优先用子模块)
...
### 5.4.4\_Kconfig\_/\_Kbuild\_模板(可直接用)
...
### 5.4.5\_Mermaid\_图元库与约定(Typora\_兼容)
