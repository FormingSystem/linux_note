...
## 1.1\_GPIO\_总览与生态演进(以\_6.1\_为基线\_按时间线叙述)
...
### 1.1.3\_开发者视角(今天应该怎样写)

#### (1)\_驱动迁移\_三步走\_(按时间线给出依据)
...
#### (2)\_用户写入到\_LED\_的时序(sequenceDiagram)
...
#### (1)\_内核\_Consumer(仅演示\_描述符\_API\_用法)
...
### 1.2.2\_数据结构视角(DT\_基本语义)

#### (1)\_Provider\_侧常用属性(GPIO\_控制器节点)
...
#### (2)\_Consumer\_侧约定(外设节点如何引用\_GPIO)
...
#### (3)\_flags\_常用取值(来自\_dt-bindings/gpio/gpio.h)
...
## 1.3\_pinctrl\_/\_pinmux\_与\_GPIO\_的关系
...
##### 1)\_struct\_pinctrl\_desc(定义一个\_pinctrl\_控制器)
...
##### 2)\_struct\_pinctrl\_state(一组\_状态设置\_如\_default/sleep)
...
##### 3)\_struct\_gpio\_chip(GPIO\_控制器抽象)
...
### 1.3.4\_实战\_最小可运行驱动(pinctrl\_+\_gpiod\_+\_PM\_+\_sysfs)
...
### 1.3.5\_用户视角(验证是否配置正确)

#### (1)\_查看\_pinctrl\_控制器及引脚状态(debugfs)
