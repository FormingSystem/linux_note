...
fd = os.open("/dev/note_iter", os.O_RDONLY)
try:
    first = bytearray(5)
    rest = bytearray(11)
    done = os.readv(fd, [first, rest])
    print(done, bytes(first), bytes(rest))
    print("end:", os.read(fd, 1))
    print("positioned:", os.pread(fd, 5, 0))
    print("still end:", os.read(fd, 1))
    os.lseek(fd, 0, os.SEEK_SET)
    print("rewound:", os.read(fd, 5))
finally:
    os.close(fd)
PY
...
第一行应是 16、b'hello' 与 b' from iter\n'；接着 end 是空字节串。pread 从位置 0 读到 hello，下一次普通 read 仍是空，因为 pread 没有改变共享位置。最后 lseek 明确改变共享位置，再读才得到 hello。
