...
构建沿用[模块工程](../../../engineering/build/kernel_modules/大纲.md)，在完整材料目录设置 KDIR 为匹配目标的已构建内核目录。交叉参数按该工程设置，在目标机执行装载和 Python 观察：
...
def flushes(fd):
    with open(f"/proc/self/fdinfo/{fd}", encoding="ascii") as info:
        return next(line.strip() for line in info if line.startswith("note_flushes:"))

descriptors = []
try:
    first = os.open("/dev/note_session", os.O_RDONLY)
    descriptors.append(first)
    alias = os.dup(first)
    descriptors.append(alias)
    separate = os.open("/dev/note_session", os.O_RDONLY)
    descriptors.append(separate)
    print("before:", flushes(alias), flushes(separate))
    descriptors.remove(first)
    os.close(first)
    print("after:", flushes(alias), flushes(separate))
finally:
