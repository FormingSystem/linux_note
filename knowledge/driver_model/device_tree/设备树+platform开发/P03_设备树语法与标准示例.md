---
id: knowledge.driver_model.device_tree.设备树_platform开发.p03_设备树语法与标准示例
title: "设备树语法与标准示例"
kind: subsystem
status: evolving
domains:
...
### 3.1.3\_PowerPC\_与\_OpenFirmware
...
#### (1)\_内核文档\_(Linux\_Kernel\_Documentation)
...
#### (3)\_社区规范\_(Devicetree\_Specification)
...
#### (4)\_#address-cells\_与\_#size-cells
...
#### (6)\_request\_mem\_region()\_与冲突检测
...
#### (6)\_示例\_B\_偏移映射(节点本身\_<2,1>\_父\_<1,1>)
...
#### (7)\_示例\_C\_上一级\_cells\_改变(祖先/父\_<2,2>\_节点本身\_<2,1>)
...
#### (9)\_示例\_E\_PCIe(child=3\_parent=2\_size=2\_to\_每段\_7\_cells)
...
### 3.5.6\_interrupts\_与\_interrupt-parent
...
##### 2)\_常用触发类型宏(来自\_<dt-bindings/interrupt-controller/irq.h>)
...
#### (9)\_为什么自定义的\_xxx\_remove()\_没有中断的显式回收
...
##### 1)\_devm\_request\_irq()\_的自动回收机制
...
##### 2)\_为什么\_platform\_get\_irq()\_不需要释放
...
##### 1)\_gpio-controller\_与\_#gpio-cells
...
###### b)\_of\_get\_named\_gpio()\_传统\_GPIO\_编号接口
...
#### (10)\_GPIO\_中断\_从\_GPIO\_到\_GIC\_的中断解析
...
#### (12)\_驱动中断与\_GPIO\_的关系\_GPIO\_to\_GIC
...
##### 1)\_示例\_GPIO\_to\_GIC\_中断的设备树配置
...
### 3.5.8\_regulator\_与\_power\_管理
...
#### (6)\_驱动示例\_与设备树\_regulator\_配置的交互
...
###### a)\_时钟获取\_devm\_clk\_get(dev,\_"spi-core")
...
###### b)\_时钟使能\_clk\_enable(spi->core\_clk)
...
##### 1)\_按路径查找节点\_of\_find\_node\_by\_path
...
##### 2)\_按兼容属性查找节点\_of\_find\_compatible\_node
...
##### 3)\_遍历子节点\_of\_get\_next\_child
...
##### 1)\_解析字符串属性\_of\_property\_read\_string
...
##### 2)\_解析整数属性\_of\_property\_read\_u32
...
##### 3)\_解析整数数组\_of\_property\_read\_u32\_array
...
##### 4)\_检查属性是否存在\_of\_property\_read\_bool
...
##### 1)\_解析寄存器地址\_of\_address\_to\_resource
...
##### 1)\_检查节点是否兼容\_of\_device\_is\_compatible
...
### 3.6.4\_用户视角\_验证设备树解析结果

虽然of_xxx函数主要在驱动中运行，但用户可通过以下方式验证设备树解析结果，间接了解这些函数的工作效果。

#### (1)\_查看/sys/firmware/devicetree/base
...
### 3.6.5\_调试与验证\_of\_xxx函数的常见问题与排查
