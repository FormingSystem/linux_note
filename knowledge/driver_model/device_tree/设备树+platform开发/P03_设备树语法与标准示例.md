...
#### (1)_内核文档_(Linux_Kernel_Documentation)
...
#### (3)_社区规范_(Devicetree_Specification)
...
#### (4)_#address-cells_与_#size-cells
...
#### (6)_示例_B_偏移映射(节点本身_<2,1>_父_<1,1>)
...
#### (7)_示例_C_上一级_cells_改变(祖先/父_<2,2>_节点本身_<2,1>)
...
#### (9)_示例_E_PCIe(child=3_parent=2_size=2_to_每段_7_cells)
...
### 3.5.6_interrupts_与_interrupt-parent
...
##### 2)_常用触发类型宏(来自_<dt-bindings/interrupt-controller/irq.h>)
...
#### (9)_为什么自定义的_xxx_remove()_没有中断的显式回收
...
##### 2)_为什么_platform_get_irq()_不需要释放
...
##### 1)_gpio-controller_与_#gpio-cells
...
###### b)_of_get_named_gpio()_传统_GPIO_编号接口
...
#### (10)_GPIO_中断_从_GPIO_到_GIC_的中断解析
...
#### (12)_驱动中断与_GPIO_的关系_GPIO_to_GIC
...
###### a)_时钟获取_devm_clk_get(dev,_"spi-core")
...
###### b)_时钟使能_clk_enable(spi->core_clk)
...
##### 1)_按路径查找节点_of_find_node_by_path
...
##### 2)_按兼容属性查找节点_of_find_compatible_node
...
##### 1)_解析字符串属性_of_property_read_string
...
##### 2)_解析整数属性_of_property_read_u32
...
##### 3)_解析整数数组_of_property_read_u32_array
...
##### 4)_检查属性是否存在_of_property_read_bool
...
##### 1)_解析寄存器地址_of_address_to_resource
...
##### 1)_检查节点是否兼容_of_device_is_compatible
