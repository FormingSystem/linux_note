...
### 2.2.2\_从\_DTB\_到\_device\_node(解析阶段)
...
### 2.2.3\_从属性到资源\_是否\_必须\_转成\_struct\_resource

先回答大问题：**不是强制必须**。驱动可以直接从 `device_node` 读取属性，但**工程上更推荐转成 `resource`**。下面分两路对比。

#### (1)\_A\_路\_直接读取属性(不转\_resource)
...
#### (2)\_B\_路\_转换为\_resource(推荐的工程化路线)
...
### 2.2.4\_自动生成\_platform\_device\_与填充\_resource数组\_调用链
...
### 2.3.3\_驱动\_probe\_从\_device\_node\_到\_resource
...
### 2.3.7\_匹配表与\_MODULE\_DEVICE\_TABLE
...
#### (2)\_struct\_of\_device\_id\_的定义位置与字段
...
#### (1)\_MODULE\_DEVICE\_TABLE\_的兄弟宏
...
#### (4)\_of\_match\_ptr()\_与\_CONFIG\_OF
...
#### (6)\_binding\_文档与\_compatible\_规范
...
#### (1)\_/proc\_与\_/sys\_的目录映射(从\_DTS\_到驱动绑定)
...
#### (2)\_匹配与资源生成流程图(DTS\_to\_resource\_to\_probe)
...
#### (3)\_模块自动装载与\_probe\_调用时序(含\_modalias\_/\_alias)
