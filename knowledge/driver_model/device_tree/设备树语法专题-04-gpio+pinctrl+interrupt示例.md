...
## 1.5\_i.MX6ULL\_与\_RK356x\_平台特性对比
...
## 2.2\_数据结构视角\_GPIO\_框架核心机制

### 2.2.1\_GPIO\_控制器的核心结构体
...
## 2.3\_开发者视角\_GPIO\_控制器的\_DTS\_语法

### 2.3.1\_i.MX6ULL\_GPIO\_控制器语法
...
## 2.5\_工程实例二\_GPIO\_控制电源(regulator\_子系统)

### 2.5.1\_典型应用\_3.3V\_模块电源
...
## 3.3\_工程语法\_pinctrl\_节点结构与配置项

### 3.3.1\_i.MX6ULL\_的\_pinctrl\_写法
...
## 3.6\_工程实例三\_Wi-Fi\_模块的引脚状态与上电时序

### 3.6.1\_RK3568\_+\_AP6256\_模块
...
# 第4章\_interrupt\_子系统的工程语法与级联机制

*(参考平台：NXP i.MX6ULL / Rockchip RK356x · Linux ≥ 6.1)*

------

## 4.1\_主题引入\_事件如何从脚到\_CPU
...
## 4.2\_数据结构视角\_irqdomain\_把不同控制器\_装进同一地图
...
## 4.4\_工程实例一\_按键(gpio-keys)从\_GPIO\_域到\_GIC

### 4.4.1\_i.MX6ULL\_版本(GPIO1\_IO05\_为下降沿触发)
...
### 4.4.2\_RK356x\_版本(GPIO2\_C1\_上升沿)
...
## 4.5\_工程实例二\_多中断源设备(READY\_+\_FAULT)

某 SPI 传感器有两根中断脚：`READY`（上升沿），`FAULT`（下降沿），分别接在不同 GPIO bank。

### 4.5.1\_DTS(通用写法\_i.MX6ULL\_举例)
...
## 4.6\_工程实例三\_I/O\_扩展器(MCP23017\_/\_PCA9539)级联中断

扩展器自己是 **GPIO 控制器 + 中断控制器**，它的 **INT 引脚** 再接到主控的某个 GPIO。

### 4.6.1\_i.MX6ULL\_版本(PCA9539\_举例)
...
## 4.8\_工程实例五\_唤醒中断(Wakeup\_IRQ)与电气一致性

### 4.8.1\_场景

- Wi-Fi `HOST_WAKE` / 触摸屏 `INT` 需要在系统挂起时唤醒 SoC。

### 4.8.2\_DTS\_关键点(i.MX6ULL\_举例)
...
## 5.3\_工程实例一\_Wi-Fi\_模块完整组合(REG\_ON\_+\_HOST\_WAKE)

### 5.3.1\_i.MX6ULL\_平台\_DTS
...
## 5.4\_工程实例二\_摄像头模块(PWDN\_+\_RESET\_+\_IRQ)
...
## 5.5\_工程实例三\_RS485/UART\_半双工方向控制
...
## 6.3\_DTS\_层差异\_属性语法与引脚配置对比

### 6.3.1\_pinctrl\_节点差异
...
# 第7章\_调试与验证技巧\_从引脚到中断的全链路追踪

------

## 7.1\_主题引入\_驱动成功加载\_≠\_驱动真正工作
...
## 7.3\_数据结构视角\_如何在运行时定位引脚信息

### 7.3.1\_设备树\_to\_pinctrl\_解析路径
...
## 7.4\_用户视角\_可视化验证引脚行为

### 7.4.1\_引脚电平验证(示波器\_/\_逻辑分析仪)
...
### 7.5.2\_tracepoint\_GPIO\_&\_IRQ
