...
### 3.1.4\_建模方式二\_共享\_cdev\_+\_i\_cindex
...
## 3.2\_附录\_A\_精讲「多实例」的\_3\_种常见写法
...
## 3.3\_附录\_B\_一个小小用户态测试程序(对比\_shell)
...
`poll`、`mmap` 与 DMA 会引入等待通知、VMA 生命周期和 DMA API 等额外契约，不应直接堆叠在本章的基础读写模型中；请按[专题大纲](大纲.md)进入对应机制。
