...
这里不是 VFS 的完整教程。VFS 权威专题还必须独立解释文件系统注册、挂载、路径查找、缓存和通用生命周期；本章只完整展开 **VFS 与字符设备相交的调用链**，保证字符设备主线不断。
...
- `fs/open.c`：`do_sys_openat2()`、`do_dentry_open()`、`vfs_open()`；
- `fs/namei.c`：`do_filp_open()` 和路径打开状态机；
- `fs/char_dev.c`：`def_chr_fops`、`chrdev_open()`、`cdev_map`；
- `include/linux/fs.h`：inode、file、file_operations；
- `include/linux/cdev.h`：cdev 对象。
