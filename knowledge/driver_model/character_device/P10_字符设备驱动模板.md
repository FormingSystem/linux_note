...
## 10.1\_代码上手\_单/多实例\_+\_静/动设备号\_+\_自动/手工节点

### 10.1.1\_最小可运行模板

本节先用单实例、动态设备号和自动节点建立最小可运行模板，再说明怎样演进到多实例和阻塞 I/O。
...
> **阻塞 `read()` 的返回边界：** `wait_event_interruptible()` 返回 0，只表示等待宏观察到业务条件成立，调用者仍应在正确的锁与生命周期保护下重新检查条件；返回负值表示等待被信号打断，应将错误返回 VFS，不能继续按读取成功处理。通用状态转换、`-ERESTARTSYS`、超时和检查—睡眠窗口见[等待队列](../../linux/synchronization_and_asynchrony/synchronization/waiting_notification/P01_等待队列.md)，字符设备的并发与注销边界见[文件操作并发与生命周期](P04_文件操作并发与生命周期.md)。
...
最小模板用于验证注册、打开和基本读写路径。正式驱动应根据设备语义补充对象生命周期、并发、阻塞取消和移除路径，不能仅通过堆叠回调把教学模板直接当成项目模板。
...
```mermaid
flowchart TD
    Start([Start]) --> CheckReq{req == 0?}
    CheckReq -- Yes --> Return0[/return 0/]
    CheckReq -- No --> ComputeShouldBlock[compute should_block]
    ComputeShouldBlock --> TryLock[lock interruptible]
    TryLock -- Interrupted --> ReturnERESTARTSYS[/return -ERESTARTSYS/]
    TryLock -- Acquired --> LoopStart([while req])

    LoopStart --> ReadAttempt[read from ring buffer]
    ReadAttempt --> FaultCheck{n == -EFAULT?}
    FaultCheck -- Yes --> SetDoneEFAULT[done = -EFAULT] --> BreakOut
    FaultCheck -- No --> NPositive{n > 0?}

    NPositive -- Yes --> UpdatePtrs[advance pointers; done += n] --> WakeWrite[wake writers] --> BreakOut
    NPositive -- No --> DoneCheck{done > 0?}

    DoneCheck -- Yes --> BreakOut
    DoneCheck -- No --> EOFCheck{eof_on_empty?}

    EOFCheck -- Yes --> SetDoneZero[done = 0] --> BreakOut
    EOFCheck -- No --> NonBlockCheck{should_block == false?}

    NonBlockCheck -- Yes --> SetDoneEAGAIN[done = -EAGAIN] --> BreakOut
    NonBlockCheck -- No --> UnlockAndWait[unlock; wait on read queue]

    UnlockAndWait --> WaitInterrupted{wait interrupted?}
    WaitInterrupted -- Yes --> ReturnERESTARTSYS
    WaitInterrupted -- No --> ReLock[re-lock interruptible]
    ReLock -- Interrupted --> ReturnERESTARTSYS
    ReLock -- Acquired --> LoopStart

    BreakOut --> UnlockBeforeReturn[unlock] --> ReturnDone[/return done/]
```
...
flowchart TD
    ...
    LoopStart --> WriteAttempt[write to ring buffer]
    WriteAttempt --> FaultCheck{n == -EFAULT?}
    FaultCheck -- Yes --> SetDoneEFAULT[done = -EFAULT] --> BreakOut
    FaultCheck -- No --> NPositive{n > 0?}

    NPositive -- Yes --> UpdatePtrs[advance pointers; done += n] --> WakeRead[wake readers] --> ContinueLoop([continue])
    ContinueLoop --> LoopStart
    NPositive -- No --> NonBlockCheck{should_block == false?}

    NonBlockCheck -- Yes --> SetDoneMaybeEAGAIN[done = done_or_minus_EAGAIN] --> BreakOut
    NonBlockCheck -- No --> UnlockAndWait[unlock; wait on write queue]
