...
> **阻塞 `read()` 的返回边界：** `wait_event_interruptible()` 返回 0，只表示等待宏观察到业务条件成立，调用者仍应在正确的锁与生命周期保护下重新检查条件；返回负值表示等待被信号打断，应将错误返回 VFS，不能继续按读取成功处理。通用状态转换、`-ERESTARTSYS`、超时和检查—睡眠窗口见[等待队列](../../linux/waiting_notification/P01_等待队列.md)，字符设备的并发与注销边界见[文件操作并发与生命周期](P04_文件操作并发与生命周期.md)。
