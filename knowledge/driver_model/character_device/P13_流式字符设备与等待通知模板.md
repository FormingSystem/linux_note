...
两处设备号应相符；没有节点先按第十一章检查 devtmpfs 或手工建立本次实验的入口，不往缺失路径重定向。下面的完整 C11 程序使用两个独立打开文件和非阻塞读写，先观察确定的回绕关系。程序保留每次短传输的返回值，不用循环补齐掩盖驱动的数组边界。

`O_RDONLY`、`O_WRONLY` 分别请求只读和只写打开，均叠加本章的非阻塞标志。S_ISCHR 继续检查字符设备类型；POLLIN 是用户空间的可读事件位。poll 的第三个参数为 0 时只查询当前就绪状态，不等待未来事件，所以适合逐步观察每次读写怎样改变条件。pollfd.events 保存请求的事件，revents 保存实际结果；POLLERR、POLLHUP、POLLNVAL 分别表示错误、挂断和无效描述符，本例遇到它们立即停止，不能把它们当成“暂时不可读”。
...
保存为 `stream_probe.c`，编译后运行：

```bash
cc -std=c11 -Wall -Wextra -Werror -pedantic stream_probe.c -o stream_probe
sudo ./stream_probe
```

预期只有最后一行输出。它要求本次装载后队列为空且没有其他读写者。检查失败时保留现场，先按 `head/tail/used` 重建已成功步骤，不反复运行一个假设“初始为空”的程序。
