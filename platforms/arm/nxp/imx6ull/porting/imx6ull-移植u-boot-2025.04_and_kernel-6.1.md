...
# 第5章\_虚拟机\_NFS\_和\_tftp\_配置

虚拟机ubuntu22中配置nfs和tftp，参考[正点原子Linux之ARM裸机篇视频](https://www.bilibili.com/video/BV1yE411h7uQ?vd_source=b387713a15d6517575ab4761525174e7)。
...
4. 修改uboot的顶层Makefile文件，在下面添加对应的架构和交叉编译器的配置
   ...
   ![image](../../../../../assets/images/uboot/nxp/root_makefile_change.png)

## 8.1\_添加开发板对应的xxx\_defconfig文件
...
2. 修改 `mx6ull_test_emmc_defconfig` 文件的一些内容
   ...
   ![image](../../../../../assets/images/uboot/nxp/xxx_defconfig_change.png)

   笔者来对比下2025年和2016年的xxx_defconfig的区别，下面图片展示的是mx6ull_14x14_evk_emmc_defconfig文件的对比详情：

   ![image](../../../../../assets/images/uboot/nxp/xxx_defconfig_compire.png)
...
![image](../../../../../assets/images/uboot/nxp/mx6ullevk.h_compire.png)
...
### 8.3.1\_新增mx6ull\_test\_emmc文件夹
...
### 8.3.5\_imximage.cfg修改

将mx6ull_test_emmc目录下imximage.cfg内容更改为如下内容，其中包含DDR的校验信息，DDR的校验请查阅[DDR配置更新章节](#8.5_DDR配置更新)：
...
### 8.3.7\_mx6ull\_test\_emmc/Kconfig修改
...
### 8.3.9\_mx6ull\_test\_emmc/Makefile修改
...
### 8.3.10\_mx6ull\_test\_emmc.c修改
...
![image](../../../../../assets/images/uboot/nxp/kconfig_add_new_board.png)

如下所示，这是新增加配置路径部分：

![image](../../../../../assets/images/uboot/nxp/kconfig_add_new_board_kconfig.png)
...
#### (1)\_imx6ull-14x14-evk.dts\_复制+重命名
...
#### (4)\_向\_arch/arm/dts/Makefile\_新增设备树文件
...
![image](../../../../../assets/images/uboot/nxp/add_new_dtb.png)



## 8.7\_编译新加入的开发板配置的uboot
...
![image](../../../../../assets/images/uboot/nxp/new_board_compile.png)

这是编译完成后的示例：

![image](../../../../../assets/images/uboot/nxp/new_board_compile_end.png)
...
![image](../../../../../assets/images/uboot/nxp/dtb_lcdif_config.png)

#### (3)\_lcdif设备节点属性与lcd屏幕属性映射关系
...
![image](../../../../../assets/images/uboot/nxp/rgb_lcd_elc_route.png)
...
![image](../../../../../assets/images/uboot/nxp/lcd_driver_change.png)
...
![image](../../../../../assets/images/uboot/nxp/xxx_defconfig_net_change.png)

#### (2)\_硬件连接

enet1的硬件图：

![image](../../../../../assets/images/uboot/nxp/enet1_elc.png)

enet2的硬件图

![image](../../../../../assets/images/uboot/nxp/enet2_elc.png)

网口复位引脚，如下图所示的SNVS_REMPER7(pin 23) 和 SNVS_REMPER8(pin 26) ：

![image](../../../../../assets/images/uboot/nxp/enet_reset_pin_elc.png)

#### (3)\_修改phy芯片设备树描述

##### 1)\_切换phy网卡驱动到LAN8720A

在 `drivers/net/phy/smsc.c` 文件中，有关于LAN8720A的芯片设备id说明，如下所示：

![image](../../../../../assets/images/uboot/nxp/lan8720_id_description.png)
...
![image](../../../../../assets/images/uboot/nxp/enet_type_choose_in_official.png)
...
![image](../../../../../assets/images/uboot/nxp/enet_define_in_official.png)
...
* 再解决如何适配：将设备树的 `fec` 节点下的 `mdio` 节点的 `compatible` 值选做LAN8720A的配置，它的配置定义在 `drivers/net/phy/smsc.c` 文件中，内容如下：
  ...
  ![image](../../../../../assets/images/uboot/nxp/enet_dtb_add_lan8720a.png)
...
![image](../../../../../assets/images/uboot/nxp/enet_reset_pin_dtb_config_1.png)
...
![image](../../../../../assets/images/uboot/nxp/enet_reset_pin_conflict.png)
...
![image](../../../../../assets/images/uboot/nxp/enet_dtb_add_reset_pin.png)

#### (4)\_为LAN8720A驱动添加新的复位操作

接下来，进入最为重要的一步骤，为LAN8720A添加软复位。它的参考非常意外，在一个[uboot移植网络攻略](https://blog.csdn.net/ZHONGCAI0901/article/details/118310802)的评论区：

![image](../../../../../assets/images/uboot/nxp/enet_sw_reset_config.png)

那么我们追踪到drivers/net/phy/phy.c，添加依据软复位代码：

![image](../../../../../assets/images/uboot/nxp/enet_sw_reset_driver.png)
...
参考[uboot和kernel获取](#第6章_uboot和kernel获取)章节，将kernel版本切换到 `origin/lf-6.1.y` 分支，笔者接下来将会分析lf-6.1.y分支的kernel移植操作：
...
## 9.1\_修改顶层Makefile\_添加交叉编译器前缀和架构
...
![image](../../../../../assets/images/kernel/nxp/top_makefile_add_arch.png)
...
2. `make imx_v7_defconfig` ：导入imx6ull基本配置。这里有个 `imx_v7_defconfig` 这个配置，笔者需要讲解下它的来源。正常操作是直接搜索 `CONFIG_SOC_MX6UL` 选项，在 `xxx_defconfig`，看下谁含有这个配置项就基本能够锁定对应的 `xxx_defconfig`。
   ...
   ![image](../../../../../assets/images/kernel/nxp/choose_xxx_defconfig.png)
...
4. `make -j$(nproc)` ：多线程编译kernel。
   ...
   ![image](../../../../../assets/images/kernel/nxp/build_kernel_success_log.png)
...
#### (1)\_.missing-syscalls.d是什么文件

这个问题问得很细 👍。我来详细解释一下：

------

##### 1)\_.missing-syscalls.d\_文件是什么
...
### 9.4.1\_解决\_imx/sdma/sdma-imx6q.bin\_报错问题

#### (1)\_在xxx\_defconfig添加配置
...
![image](../../../../../assets/images/kernel/nxp/kernel_soc_config.png)
...
![image](../../../../../assets/images/kernel/nxp/kernel_sdma_bin_config.png)

#### (2)\_下载\_sdma-imx6q.bin\_固件
...
### 9.5.1\_imx6ull-14x14-evk-emmc.dts\_复制+重命名
...
![image](../../../../../assets/images/kernel/nxp/kernel_dtb_config_1.png)

### 9.5.2\_imx6ull-14x14-evk.dts\_复制+重命名
...
![image](../../../../../assets/images/kernel/nxp/kernel_dtb_config_2.png)

### 9.5.3\_imx6ull.dtsi\_复制+重命名

复制一份 `imx6ull.dtsi`，然后将其重命名为 `imx6ull-test.dtsi`。不做修改

### 9.5.4\_imx6ul-14x14-evk.dtsi\_复制+重命名
...
![image](../../../../../assets/images/kernel/nxp/kernel_dtb_lcd_config_1.png)
...
![image](../../../../../assets/images/kernel/nxp/kernel_dtb_lcd_config_2.png)
...
![image](../../../../../assets/images/kernel/nxp/kernel_net_reset_sw.png)

### 9.7.3\_添加开发板对应的设备树文件

配置好设备树文件之后，还需要修改文件 `arch/arm/boot/dts/Makefile`，找到 `dtb-$(CONFIG_SOC_IMX6ULL)` 配置项，在此配置项中加入 `imx6ull-14x14-test-emmc.dtb`，如下所示：

![image](../../../../../assets/images/kernel/nxp/kernel_add_dtb_into_makefile.png)

### 9.7.4\_添加smsc公司phy配置
...
![image](../../../../../assets/images/kernel/nxp/add_smsc_phy_config.png)



## 9.8\_修改cpu频率为performance
...
![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_1.png)

![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_2.png)

![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_3.png)

![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_4.png)

设置之后将配置进行保存，它会自动更新到 `.config` 文件中：

![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_5.png)

### 9.8.2\_将配置到imx\_v7\_test\_defconfig

将 `arch/arm/configs/imx_v7_test_defconfig` 文件打开，我们从 `.config` 配置中将文件中的配置复制到 `arch/arm/configs/imx_v7_test_defconfig` ：

![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_6.png)

然后重新编译即可更新cpu主频：

![image](../../../../../assets/images/kernel/nxp/kernel_performance_config_7.png)
...
## 10.2\_获取\_BusyBox\_根文件系统

BusyBox 可以在其官网下载到，官网地址为：https://busybox.net/。下面是下载指引：

![image](../../../../../assets/images/rootfs/get_rootfs_source.png)

笔者这里采用的是比较新的版本：https://busybox.net/downloads/busybox-snapshot.tar.bz2

![image](../../../../../assets/images/rootfs/busybox_version.png)
...
![image](../../../../../assets/images/rootfs/compiler_config.png)

## 10.4\_busybox\_中文字符支持
...
![image](../../../../../assets/images/rootfs/rootfs_config_1.png)
...
![image](../../../../../assets/images/rootfs/rootfs_config_2.png)
...
![image](../../../../../assets/images/rootfs/rootfs_config_3.png)

![image](../../../../../assets/images/rootfs/rootfs_config_4.png)
...
![image](../../../../../assets/images/rootfs/rootfs_config_5.png)

![image](../../../../../assets/images/rootfs/rootfs_config_6.png)
...
![image](../../../../../assets/images/rootfs/rootfs_config_7.png)
...
### 10.7.1\_复制$(cross\_compile)/libc/lib
...
![image](../../../../../assets/images/rootfs/rootfs_lib_files.png)

### 10.7.3\_向$(root\_fs)/usr/lib\_目录添加库文件
...
![image](../../../../../assets/images/rootfs/rootfs_usr_lib_files.png)至此，根文件系统的库文件就全部添加好了。

### 10.7.4\_解决无\_rcS\_文件的错误与hotplug错误
...
# 第11章\_buildroot\_下载工具

首先声明：笔者这里将不会教怎么打包一体化采用buildroot来编译所有环境，只是在使用上述环境的时候如何为现有环境添加新kernel所适配的工具，以libgpiod工具为主要讲解方式。

## 11.1\_下载buildroot

下载buildroot源码到ubuntu。[buildroot官网下载](https://buildroot.org/download.html)：![image](../../../../../assets/images/buildroot/external_toolchain/buildroot_download.png)
...
进入到buildroot源码目录，执行 `make menuconfig` shell命令，添加芯片架构的配置信息。我们采用的imx6ull，配置如下：![image](../../../../../assets/images/buildroot/external_toolchain/buildroot_target_config.png)
...
进入 Toolchain，配置交叉编译器的位置，前缀，编译器属性等:![image](../../../../../assets/images/buildroot/external_toolchain/buildroot_target_config_whole.png)
...
![image](../../../../../assets/images/buildroot/external_toolchain/tool_libgpiod_add.png)
